...
estimateBootstraps <- function(iseed, biters, estimate.list,
  ...
  sEst <- function(sMestimate, i, sEstimate, sbmin, sbmax, DataMatrix.list, SelParmValues, Csites.weights.list,
    ...
    ErrorSingular <- grep("singular", try(
      {
  ...
  for (iter in 1:biters) {
    ...
    sEst.list <- sEst(
      sMestimate, iter, sEstimate, sbmin, sbmax, DataMatrix.list, SelParmValues, Csites.weights.list,
      estimate.input.list, dlvdsgn, H
    )
    ...
    Resids[iter, ] <- estimateFeval(
      BetaEst,
      DataMatrix.list, SelParmValues, Csites.weights.list,
      estimate.input.list, dlvdsgn
    ) # executed with monitoring load adjustment and thinned set of parameters
