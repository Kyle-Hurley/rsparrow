...
#' @return `AreaRch`  data.frame containing 'waterid' from `indata` and `accum_elements` columns
#'                   from `indata` accumulated incrementally and output as `accum_names` for
#'                   column names. `AreaRch` contains same number of rows as `indata`
#'
#' @keywords internal
#' @noRd
...
accumulateIncrArea <- function(indata, accum_elements, accum_names) {
  ...
  for (c in c("termflag", "fnode", "tnode", accum_elements)) {
    data[[c]] <- ifelse(is.na(data[[c]]), 0, data[[c]])
  }
