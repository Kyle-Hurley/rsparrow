...
#'       separating sites
#' @return `Csites.weights.list` list of objects related to regression weights as proportional
#' to incremental area size including
#' \tabular{ll}{
#' `NLLS_weights` \tab character RSPARROW control setting where "default" indicates `weights=1`
#' unweighted, "lnload" indicates weights expressed as reciprocal of variance proportional to
#' log of predicted load, and "user" indicates weights assigned by user in userModifyData.R
#' script expressed as the reciprocal o fthe variance proportional to the user-selected
#' variables. \cr
#' `tiarea` \tab numeric vector of incremental area \cr
#' `count` \tab data.frame of `staidseq` sequence of staids and `nirchs` number of unique staids \cr
#' `weight` \tab numeric vector of weights for NLLS estimation \cr
#' }
#' @keywords internal
#' @noRd
