...
estimateNLLSmetrics <- function(if_estimate, if_estimate_simulation, if_sparrowEsts, sparrowEsts,
  ...
  if (dynamic) {
    if ("season" %in% names(subdata)) {
      if (sum(ifelse(!is.na(subdata$season), 1, 0)) == length(subdata$season)) {
        dynseas <- TRUE
      }
    }
    if ("year" %in% names(subdata)) {
      if (sum(ifelse(!is.na(subdata$year), 1, 0)) == length(subdata$year)) {
        dynyear <- TRUE
      }
    }
  }
  ...
  for (k in 1:length(classvar)) {
    for (i in 1:nrow(sitedata)) {
      class[i, k] <- as.numeric(sitedata[[classvar[k]]][i])
    }
  }

  # contiguous class variables by reach
  classrch <- as.numeric(subdata[[classvar[1]]]) # used to compute RMSE by class
  ...
  jtotarea <- data.index.list$jtotarea
  ...
  betamin <- SelParmValues$betamin
  betamax <- SelParmValues$betamax
  betatype <- SelParmValues$betatype
    ...
    if (k != npar) {
      ...
      if (length(ErrorSolve) > 0) {
        if (ErrorSolve == 1) {
          ...
          stop("Error in estimateNLLSmetrics.R. Run execution terminated.")
        }
      }
  ...
  if (ifHess == "yes" & if_estimate_simulation == "no") {
    ...
    Hssobj <- function(Estimate,
      ...
      crossprod(estimateFeval(
        Estimate,
        DataMatrix.list, SelParmValues, Csites.weights.list,
        estimate.input.list, dlvdsgn
      )) # changed beta0 to Estimate 9-6-16
    }
  ...
  if (dynamic) {
    if (dynseas & dynyear) { # seasonal model
      ANOVAdynamic.list <- named.list(
        dynsites, dynyearmin, dynyearmax, cyear, cseason,
      ...
      if (dynseas) {
        ANOVAdynamic.list <- named.list(
          dynsites, dynyearmin, dynyearmax, cseason,
          smobs, sSSE, sRSQ, sRSQ_YLD, sPBias,
          psSSE, psRSQ, psRSQ_YLD, psPBias
        )
      } else {
  ...
  ANOVA.list <- named.list(
    mobs, npar, DF, SSE, MSE, RMSE, RSQ, RSQ_ADJ, RSQ_YLD, NSeff, PBias,
    pSSE, pMSE, pRMSE, pRSQ, pRSQ_ADJ, pRSQ_YLD, pNSeff, pPBias
  )
  Mdiagnostics.list <- named.list(
    Obs, predict, yldobs, yldpredict,
