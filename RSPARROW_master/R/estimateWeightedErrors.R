...
estimateWeightedErrors <- function(file.output.list, xrun_id, pre_run_id, nreaches, calsites) {
  ...
  filename <- paste0(
    path_user, .Platform$file.sep, results_directoryName, .Platform$file.sep, pre_run_id, .Platform$file.sep,
    "estimate", .Platform$file.sep, pre_run_id, "_residuals.csv"
  )

  oResids <- read.csv(filename,
  ...
  filename <- paste0(
    path_user, .Platform$file.sep, results_directoryName, .Platform$file.sep, xrun_id, .Platform$file.sep,
  ...
  xy <- xy[order(x), ]
  plot(xy$x, xy$y,
    ylab = "Squared Log Residuals & Normalized Weights", xlab = "Log Predicted Load",
  ...
  legend("topright",
    cex = 1.1, c("Nonlinear LS fit to log residuals", "Normalized weights"),
