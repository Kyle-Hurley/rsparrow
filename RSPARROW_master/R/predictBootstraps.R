...
predictBootstraps <- function(iseed, biters, estimate.list, estimate.input.list, predict.list, BootResults,
  ...
  # Extract from predict.source.list
  predict.source.list <- predict.list$predict.source.list
  srclist_total <- predict.source.list$srclist_total
  srclist_mtotal <- predict.source.list$srclist_mtotal
  srclist_inc <- predict.source.list$srclist_inc
  srclist_nd_total <- predict.source.list$srclist_nd_total
  srclist_yield <- predict.source.list$srclist_yield
  srclist_myield <- predict.source.list$srclist_myield
  srclist_yldinc <- predict.source.list$srclist_yldinc
  srclist_yldinc_deliv <- predict.source.list$srclist_yldinc_deliv

  # Extract from file.output.list
  path_results <- file.output.list$path_results
  run_id <- file.output.list$run_id
  confInterval <- file.output.list$confInterval
  #################
  # SETUP VARIABLES
  #################
  ...
  for (iter in 1:biters) {
    ...
    bootcorrectionR <- 1
    predBoots.list <- predictBoot(
      oEstimate, estimate.list, estimate.input.list, bootcorrectionR,
    ...
    if (j >= index.mpload.start & j <= index.mpload.end) { # select only the monitored load and source predictions
      ...
      for (i in 1:nnrows) {
        if (bootpred[i, j, 1] > 0) {
          ...
          if (j == 2) {
            sample.error.var.boots[i] <- sample.error.var
          } # reach error variance for total load
  ...
  boparmlist <- c(
    "waterid", mean1, mean2, mean3, mean4, mean5, mean6, mean7, mean8, mean9,
