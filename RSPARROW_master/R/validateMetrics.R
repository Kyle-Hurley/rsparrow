...
validateMetrics <- function(classvar, estimate.list, dlvdsgn, Vsites.list, yieldFactor,
  ...
  for (k in 1:length(classvar)) {
    for (i in 1:nrow(vsitedata)) {
      class[i, k] <- as.numeric(vsitedata[[classvar[k]]][i])
    }
  }
  ...
  # Extract needed columns from subdata
  vdepvar <- subdata$vdepvar
  vstaid <- subdata$vstaid
  lat <- subdata$lat
  lon <- subdata$lon
  ...
  for (k in 1:nreach) {
    if (vdepvar[k] > 0) {
      ...
      pyldobs[ins] <- Obs[ins] / data[k, data.index.list$jtotarea] * yieldFactor
      pyldpredict[ins] <- ppredict[ins] / data[k, data.index.list$jtotarea] * yieldFactor
      pratio.obs.pred[ins] <- Obs[ins] / (ppredict[ins])

      xlat[ins] <- lat[k]
      xlon[ins] <- lon[k]
      xstaid[ins] <- vstaid[k]
      tarea[ins] <- data[k, data.index.list$jtotarea]
  ...
  vMdiagnostics.list <- named.list(
    Obs, xstaid, tarea, xlat, xlon,
