...
verifyDemtarea <- function(if_verify_demtarea, data1, compareData,
  ...
  if (if_verify_demtarea == "yes") {
    ...
    hs_data <- data.frame(waterid, demtarea_new, hydseq_new, headflag_new)
    ...
    sub1.plot <- qplot(sub1$demtarea, sub1$demtarea_new,
      log = "xy",
    ...
    if (nrow(DAreaFailCheckObj) > 0) {
      DAreaFailCheckMessage <-
        paste0(
          "Number of reaches with different (>1%) total drainage areas (see records in DAreaFailCheckObj): ",
    ...
    if (DAreaFailCheckMessage != " ") { # Map mis-matched reaches and diagnostics
      if (nrow(DAreaFailCheckObj) > 0) {
        ...
        outDir <- paste0(path_results, .Platform$file.sep, "estimate",
                         .Platform$file.sep, "diagnostic_darea_mismatches_maps",
                         .Platform$file.sep)
        ...
        checkDrainageareaErrors(
          file.output.list, mapping.input.list, data_names,
