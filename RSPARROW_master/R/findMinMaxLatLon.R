...
findMinMaxLatLon <- function(sitedata, mapping.input.list) {
  ...
  if (identical(NA, mapping.input.list$lat_limit)) {
    mapping.input.list$lat_limit <- c(latmin - 2, latmax + 2)
  }
  if (identical(NA, mapping.input.list$lon_limit)) {
    mapping.input.list$lon_limit <- c(lonmin - 2, lonmax + 2)
  }

  return(list(sitegeolimits = sitegeolimits, mapping.input.list = mapping.input.list))
} # end function
