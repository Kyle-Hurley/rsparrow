...
#' @return `dlvdsgn` data.frame containing the design matrix imported from
#' design_matrix.csv for selected parameters
#' @keywords internal
#' @noRd
...
selectDesignMatrix <- function(SelParmValues, betavalues, dmatrixin) {
  ...
  d2matrixin <- suppressWarnings(matrix(unlist(as.data.frame(dmatrixin)), ncol = adel, nrow = asrc))
