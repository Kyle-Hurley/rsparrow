...
getVarList <- function() {
  reqNames <- c(
    "waterid", "fnode", "tnode", "frac", "iftran",
    "demiarea", "hydseq", "termflag",
    "rchtype", "calsites"
  )

  fixNames <- c(
    "rchname", "demtarea", "length", "headflag", "meanq", "rchtot",
  ...
  explanations <- structure(list(
    sparrowNames = structure(c(
      25L, 16L, 6L, 24L,
      7L, 11L, 3L, 2L, 13L, 10L, 15L, 17L, 9L, 23L, 22L, 8L, 18L, 19L,
      20L, 4L, 5L, 12L, 14L, 21L, 1L
    ), .Label = c(
      "calsites", "demiarea",
      ...
      "target", "termflag", "tnode", "waterid"
    ), class = "factor"),
    explanation = structure(c(
      12L, 15L, 11L, 18L, 25L, 1L, 19L,
      13L, 14L, 6L, 7L, 17L, 3L, 16L, 24L, 5L, 20L, 10L, 2L, 8L,
      9L, 21L, 22L, 23L, 4L
    ), .Label = c(
      "\"if transport\" indicator (0=no; 1=yes; nontransport, shoreline segments should be set to 0)",
  ...
  ), .Names = c("sparrowNames", "explanation"), class = "data.frame", row.names = c(NA, -25L))
