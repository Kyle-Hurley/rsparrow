...
#' @return `data1`  data1 input data.frame with missing variables from dataDictionary
#'                 and/or parameters added
#' @keywords internal
#' @noRd



checkMissingData1Vars <- function(data1) {
  # get missing values
  missing <- checkingMissingVars(data1,
    data_names = NA, betavalues = NA,
    types = c("datalstCheck"), allMissing = TRUE,
    returnData = TRUE
  )
  k <- missing$k
  datalstMissingdata <- missing$datalstMissingdata
  if (!is.null(missing$data1)) data1 <- missing$data1
