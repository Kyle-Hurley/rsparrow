...
#' @return `Choices`  data.frame of parameter metrics with columns `category` ("Load
#' Predictions", "Yield Predictions", or "Data Dictionary Variable"), `variable` character
#' vector of RSPARROW variables for each `category`, and `definition` character vector
...
createInteractiveChoices <- function(SelParmValues, existPredict, subdata, data_names, map_uncertainties) {
  if (existPredict) {
    ...
    choices <- data.frame(
      category = rep("Load Predictions", length(choices)),
      variable = choices,
    ...
    definitions <- c(
      "Concentration based on decayed total load and discharge",
    ...
    choices <- data.frame(
      category = rep("Yield Predictions", length(choices)),
      variable = choices,
      definition = definitions
    )
    ...
    if (!identical(map_uncertainties, NA)) {
      ...
      definitions <- c(
        "Standard error of the total load (percent of mean)",
      ...
      choices <- data.frame(
        category = rep("Prediction Uncertainties", length(choices)),
        variable = choices,
        definition = definitions
      )
      Choices <- rbind(Choices, choices)
    ...
    for (n in c("demtarea", names(subdata)[which(regexpr("waterid", names(subdata)) < 0 &
      ...
      if (class(subdata[, n]) == "numeric") {
        ...
        choices <- data.frame(
          category = c("Data Dictionary Variable"),
          variable = n,
          definition = def
        )
        Choices <- rbind(Choices, choices)
