    ...
    if (identical(button, "savePDF") | input$batch == "Batch") {
      ...
      if (input$mapType == "Stream") {
        ...
        if (!dir.exists(paste0(
          path_results, .Platform$file.sep, "scenarios", .Platform$file.sep, compiledInput$scenarioName, .Platform$file.sep,
    ...
    if (input$batch != "Batch") {
      if (!identical(button, "savePDF")) {
        if (input$mapType == "Stream" | input$mapType == "Catchment") {
          ...
          p <- predictMaps(
            compiledInput, NA, output_map_type, TRUE,
            file.output.list,
            data_names, mapping.input.list,
            subdata,
            # scenarios
          ...
          unPackList(
            lists = list(agg_map.list = agg_map.list),
            parentObj = list(NA)
          )
          ...
          if (length(names(mapdata)[names(mapdata) %in% c("lat", "lon")]) != 0) {
            ...
            unPackList(
              lists = list(map_loop.list = map_loop.list),
              parentObj = list(NA)
            )
            p <- pa
            assign("p", p, envir = .GlobalEnv)
        ...
        if ((class(p)[1] == "gg" | class(p)[1] == "gtable") & input$enablePlotly == "static") {
          pdf(filename)
      ...
      save(
        list = c("path_main", "batchFilename", "RSPARROW_errorOption"),
