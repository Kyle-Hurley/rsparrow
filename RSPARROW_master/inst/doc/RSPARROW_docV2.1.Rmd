---
title: "RSPARROW 2.1:  An R System for SPARROW Modeling"
author: "Lillian Gorman Sanisaca, Kyle Hurley, and Richard B. Alexander, U.S. Geological Survey"
date: "`r format(Sys.time(), '%d %B, %Y')`(Version 2.1.0)"
output: 
  pdf_document: 
...
RSPARROW is a system of R scripts and functions for executing and evaluating SPARROW models that generates graphical, map, and tabular output (Alexander and Gorman Sanisaca, 2019). Users operate the system within RStudio from a single control script (*sparrow_control.R*) that accesses the supporting input files and functions.

This documentation describes the steps necessary to estimate the original static version of the model (long-term mean annual loads; e.g., Smith et al., 1997), as presented in the RSPARROW 1.1 version (Alexander and Gorman Sanisaca, 2019), as well as new material that describes the steps for estimating a statistical dynamic (time-dimensioned) version of the SPARROW model (R. Smith, USGS, written comm., 2022) for simulating mean annual or mean seasonal loads. Both models provide reach-level predictions (and uncertainties) of the mean water-quality loads, yields, and flow-weighted concentrations. The predictions also include the shares of the load attributable to individual upstream sources and predictions of the mass quantities of the total load and individual sources that are delivered to downstream water bodies. The input data of time-varying conditions for the statistical dynamic model can be expressed as mean seasonal (four mean seasonal values at each monitoring site, computed as seasonal averages of the observations across multiple years), mean annual (multiple continuous or discrete mean annual values, computed as mean averages of the observations across multiple seasons), or a time series of seasonal observations across multiple years and seasons.

The dynamic model, available for the first time in RSPARROW 2.0, is a simplified statistical dynamic specification of a SPARROW model for simulating temporally varying stream water-quality loads. The statistical estimation of the model coefficients empirically incorporates the implicit effects of contaminant storage processes in an aggregate manner into the model predictions and the estimated coefficients for sources, land-to-water delivery, and aquatic decay. However, the model lacks structural components (e.g., coefficients or input data) that would provide a quantifiable and more realistic physical representation of storage processes (e.g., in soils and groundwater), which are known to cause appreciable temporal lags in contaminant transport within watersheds (e.g., Schmadel et al., 2021). Therefore, contaminant storage in watersheds is not explicitly identified or quantified in the statistical dynamic model, with temporal changes in storage assumed to be zero. Although the model accurately represents the contaminant mass balance in watersheds over time, it does not provide any quantifiable or interpretable information about the timing of contaminant storage and release over time.

The scenario evaluation capability in the RSPARROW Decision Support System (DSS) has been revised in the 2.0 version to support its use with statistical dynamic models, including applications based on user-provided forecasts of changed watershed conditions (e.g., land use, climate), on time scales ranging from near-term seasonal time periods to annual and seasonal periods in future years. However, because the RSPARROW specification of the dynamic model lacks any explicit physically based components to account for storage, scenario outcomes provide only a very approximate indicator of how stream water-quality might respond to managed or natural changes in watersheds (e.g., land use, climate) at some future date, subject to the assumption that changes in storage are zero (see tutorial illustrations in Chapter 7). Users are cautioned that the lack of contaminant storage components in the model limits the interpretability of the outcomes of these scenarios, which do not precisely quantify the effects of changed model variables on the timing of future changes in stream water quality. A more physically based representation of storage processes in the model would be necessary to provide a more realistic forecast of future changes in stream water quality in response to managed or natural changes in watershed sources, land use, or climate.

Contaminant storage processes have been simulated in newer, modified versions of the SAS SPARROW model (R. Smith, USGS, written comm., 2022;  Miller et al., 2024; Schmadel et al., 2024) by adding a first-order lag process, using one or more estimated model coefficients for temporally lagged sources, or by adding a storage-related source term for baseflow or groundwater discharge that reflects contaminant transit-time distributions. The inclusion of these storage processes in RSPARROW would provide a more realistic and interpretable dynamic model that could be used to evaluate change scenarios. The inclusion of these process components will be considered for future versions of RSPARROW. The change-scenario features and the temporal data structure in RSPARROW 2.0 have been designed to be fully compatible with future implementations of a more physically based representation of storage in the model.

As an additional simplification, the residuals of the RSPARROW statistical dynamic model are assumed to be temporally independent of one another. The standard errors of the coefficients of the statistical dynamic RSPARROW models are not adjusted for serial correlation that can exist in the time series of the residuals of a dynamic SPARROW model. This can cause somewhat overestimated significance levels (i.e., lower p-values) for SPARROW model coefficients (e.g., Ator et al., 2019; Schmadel et al., 2021). Therefore, the coefficient standard errors and significance levels reported in RSPARROW 2.0 should be considered approximate for statistical dynamic models, but these measures can provide a preliminary identification of the important variables in the model. Statistical adjustments for serial correlation will be considered for a future version of RSPARROW to provide more accurate estimates (e.g., Schmadel et al., 2021) of the coefficient standard errors and significance levels, which can support final user decisions on the selection of model variables.
...
* R functions and scripts, coded according to standardized open-source R protocols, promote work environment efficiencies that encourage the collaborative development of sharable SPARROW methods and functions. Use of the USGS GitLab and GitHub repositories ensures standardized methods of storage, maintenance, documentation, and retrieval of R functions and control scripts. A description of the RSPARROW design elements (Chapter 8) provides an informative guide for experienced R users and developers. Custom string search tools are also provided to developers to ensure that functional dependencies and variable attributes can be readily identified in the RSPARROW library to support the collaborative development of the software.

* Diagnostic metrics and interactive plots and maps, with user-controlled options, enable efficient evaluations of model performance and production of map output for explanatory and response data and model prediction metrics by monitoring sites, stream reaches, catchments, and time periods (dynamic model only). These capabilities are enhanced by use of a data dictionary as a master repository of system and user-defined variable names and attributes. Users also have the option to sequester monitoring (validation) sites to independently assess model performance.

* An RShiny interactive Decision Support System (DSS) allows users to map SPARROW model predictions and geographic data by stream reach, catchment, and monitoring site location. The DSS can also be used to assess the effects of hypothetical natural or managed changes in contaminant sources (e.g., land use, wastewater loads) and/or land-water delivery variables (e.g., climate) on downstream water-quality loads and concentrations (i.e., the evaluation of "change scenarios"). The mapper operates within a user-specified Web browser, with options to display standardized, globally available base maps (e.g., topography, land use, street maps). A batch-mode option supports production of multiple maps in a single execution for model applications to large watersheds that require longer execution times. The DSS executes change scenarios for user-defined conditions, including watershed and reach locations, source types, land-water delivery factors, change magnitudes, and time periods (dynamic model only), with the water-quality effects reported in tabular and mapped output. Scenarios are applicable to modeled sources with inputs expressed in units of mass (e.g., fertilizers) or area (i.e., land-use/land-cover variables). The change magnitudes for the model explanatory variables (sources and/or land-water delivery) may be expressed as (1) user-defined percentages of the base or reference conditions used to calibrate the model (expressed as changes in either the explanatory input data or the model rate coefficients), or (2) user-defined forecasted conditions input from an external CSV file that is prepared by the user.

RSPARROW includes the same analytical capabilities as the original SAS SPARROW version (Schwarz et al., 2006), with the exception of the full bootstrap resampling method for prediction uncertainties, one of two methods in SAS SPARROW for estimating uncertainties. The full bootstrap method is computationally demanding and has been only rarely employed by USGS users. To estimate prediction uncertainties, RSPARROW executes the parametric Monte Carlo resampling methods that are also used in SAS SPARROW; this method provides sufficiently robust estimates with fewer computational demands.

**The 2.0 RSPARROW version (Gorman Sanisaca and Alexander, 2024) provides the following new enhancements to the earlier 1.1 version (Alexander and Gorman Sanisaca, 2019): **

* Executes a statistical dynamic SPARROW model for annually and/or seasonally varying input data. The input data can be expressed as mean seasonal (four mean seasonal values at each monitoring site, computed as seasonal averages of the observations across multiple years), mean annual (multiple continuous or discrete mean annual values, computed as mean averages of the observations across multiple seasons), or a time series of seasonal observations across multiple years and seasons. The model specification is a simplified statistical dynamic version of a SPARROW model that simulates temporal changes in both contaminant sources and land-to-water delivery factors (e.g., climate). The model does not explicitly identify or quantify contaminant storage in watersheds (e.g., soils, groundwater), although the implicit effects of storage processes are empirically incorporated in an unspecified manner into the model predictions and the estimated coefficients for sources, land-to-water delivery, and aquatic decay. Thus, although accurately representing the contaminant mass balance in watersheds, the model does not provide any specific quantifiable information about the timing of contaminant storage and release across the time periods as simulated in the model. 

* Provides graphical displays of temporally varying output to support the dynamic models, including annual and seasonal diagnostic maps and plots as well as interactive multi-panel stream and catchment water-quality prediction map displays for separate seasons and years. 

* Supports the execution of additional types of management scenarios for static and statistical dynamic models in the RShiny Decision Support System (DSS). This includes scenarios that evaluate the stream water-quality effects of:  (1) percentage changes in untransformed land-to-water delivery variables (e.g., temperature, precipitation); and (2) user-supplied forecasted changes in source and/or land-water delivery variables that are input from an external CSV file. The capability to input a user's CSV file allows, for example, the evaluation of the stream water-quality effects of forecasted changes in land use or climate conditions (e.g., temperature, precipitation) as reported by global climate models. User control over forecasted changes input from a CSV file also allows the evaluation of scenarios that entail changes in land-water delivery variables that are transformed in the calibrated model (e.g., logarithmic). In these cases, a relative or absolute change can be externally applied to the untransformed delivery variable by the user, with the variable subsequently transformed and stored in the external CSV file prior to input to the RSPARROW DSS. Users are cautioned that the lack of contaminant storage components in the statistical dynamic model limits the interpretability of the outcomes of change scenarios, which are unable to precisely quantify the effects of changed model variables on the timing of future changes in stream water quality. A more physically based representation of storage processes, such as lagged sources or groundwater discharge with contaminant transit times, would be necessary in the dynamic model to provide a more realistic forecast of future changes in stream water quality in response to managed or natural changes in watershed sources, land use, or climate (such updates are being considered for future RSPARROW versions). 

**The 2.1 RSPARROW version provides the following updates to the earlier 2.0 version (Gorman Sanisaca and Alexander, 2024): **

* Removes the R Markdown (RMD) coding structure that was previously used to produce graphics and report output. 

* Improves the organization and formatting of the model graphical output for *plotly* HTML files, with updated documentation of the model output files (see tables 2 and 4 below). 

* Provides documentation to explain the required UTF-8 encoding of the primary data input file for SPARROW modeling (see subsection 3.1). 

* Includes an updated version of R:  R-4.4.2
...
RSPARROW requires a 64-bit processor on a Windows platform. A minimum of 4GB of RAM or larger is highly recommended. Users may find that 8GB or more of RAM is necessary to obtain acceptable performance when using the RShiny interactive mapper and for model execution with large watersheds (e.g., more than 300,000 reaches). An installation of RStudio is the required interface for using RSPARROW. The software has not been tested on MAC and Unix platforms (note that two features of the current software version have Windows dependencies: the automated pop-up of CSV control files and the batch-mode operation). Our evaluations indicated that the RShiny interactive mapper performed more reliably using Chrome, Microsoft Edge, and Firefox Web browsers as compared with that for Internet Explorer. 
...
Three directories contain the contents of RSPARROW (See Figure \ref{directory1} for the contents of the RSPARROW directories as installed on a user's computer):

* **RSPARROW_master**:  Includes functions, sourced files (DLLs), meta-data, and documentation vignettes. The directory contents should not be modified by users. The documentation, with clickable links in the PDF table of contents, is located in the *"RSPARROW_master/inst/doc"* subdirectory. 

* **UserTutorial**:  Contains static versions of SPARROW total nitrogen models that are used for the tutorials presented in Chapter 6. The *"results"* subdirectory contains a control script (*sparrow_control.R*) and input control files to execute the tutorial models. The control script executes a predetermined sequence of function calls, governed by user settings in the control script (see the *executionTree* section of Chapter 8 for a list of functions). **Once the RSPARROW directories and required software are installed (see subsection 1.3.3), users can begin by running the control script in RStudio (after setting RStudio defaults--see subsection 1.3.4), which will execute the final SPARROW tutorial model 6 (with results written to the "Model6_User" directory) and enable the RShiny DSS.  To verify the software installation and model execution, users can compare the results in the model subdirectories with those provided in the "Model6" directory. ** The results/output (with subdirectories) for other static models (1-5, 7-8) can be obtained by replacing the control script and input control files in the *"results"* directory with copies of those for any of the other static models, followed by execution of the model control script in RStudio.

* **UserTutorialDynamic**:  Contains statistical dynamic versions of SPARROW total phosphorus models that are used for the tutorials presented in Chapter 7. The *"results"* subdirectory contains a control script (*sparrow_control.R*) and input control files to execute the tutorial models. The control script executes a predetermined sequence of function calls, governed by user settings in the control script (see the *executionTree* section of Chapter 8 for a list of functions). **Once the RSPARROW directories and required software are installed (see subsection 1.3.3), users can begin by running the control script in RStudio (after setting RStudio defaults--see subsection 1.3.4), which will execute the  SPARROW tutorial model "TP_season" (with results written to the "TP_season_User" directory) and enable the RShiny DSS.  To verify the software installation and model execution, users can compare the results in the model subdirectories with those provided in the "TP_season" directory. ** The results/output (with subdirectories) for other dynamic models ("TP_MeanAnnual", "TP_MeanSeason") can be obtained by replacing the control script and input control files in the *"results"* directory with copies of those for any of the other dynamic models, followed by execution of the model control script in RStudio.


\begin{figure}
\centering
\includegraphics[width=0.95\linewidth]{figures/FigDirectory1.jpg}
\caption{\textbf{RSPARROW directory structure.} The static and statistical dynamic tutorial models have an identical directory structure, with the exception of four additional subdirectories for the dynamic model. Estimation of a new SPARROW model automatically creates a User Model directory, with the user-defined name "run\_id", where copies of the control files are saved; model output is saved in functionally named subdirectories (as shown in green; the dynamic version of the model automatically creates four additional subdirectories as shown in red). Results of previously estimated models can be accessed for predictions, mapping, or source-change management scenario evaluations, without having to re-estimate models. The "data", "gis", "results", and scenario name directories have user-assigned names as specified in the control settings. The "RSPARROW master" directory and its subdirectories should not be modified by user; no other subdirectories should be created in this directory. }
\label{directory1}
\end{figure}


The directory structure and contents for the tutorial models, including the control script and input control files, can be used as a template to guide the setup and execution of user developed models.  Execution of the RSPARROW library functions outside of the control script is not supported; however, the RShiny interactive mapper can be independently enabled for any previously executed model within an RStudio session (see Chapter subsection 4.4.9.1.3). 
...
RSPARROW 2.1 was tested for compatibility with versions of R, RStudio, and the required R library functions available at the time of public release (April 2025): These include R-4.4.2, RStudio 2024.12.0 (Build 467), and the required versions of R library dependencies that are listed in the DESCRIPTION file located in the "RSPARROW_master" directory. RSPARROW may operate with more current versions of the software, but full compatibility of RSPARROW with newer versions is not guaranteed.


**The required software can be obtained as follows:**

* *R:* The current and past versions are available from the CRAN server at *https://cran.r-project.org/bin/windows/base/old/*. We recommend installation of R-4.4.2, using the zipped version included with the RSPARROW 2.1 download from the USGS GitLab repository. The included R version contains all R libraries and dependencies that are required to execute RSPARROW 2.1. The R directory should be unzipped to a location on your PC that does not require Administrator rights. RStudio access to the R directory is achieved by setting the R directory path in the *"Tools >> Global Options >> R version:"* (see section 1.3.4 below for details). 
...
* *R libraries and dependencies:* We recommend using the R version R-4.4.2, which includes pre-installed RSPARROW R library dependencies. This option will preclude the execution of the auto-installation steps that are contained in the RSPARROW control script (located in the *"UserTutorial/results"* directory). However, advanced R users have the option to install R for versions >=4.4.2 from CRAN and automatically install R dependencies for RSPARROW when executing the RSPARROW control script in RStudio. Execution of the control script automatically installs the most recent versions of the required R library packages and functions in the user's default R library as specified in RStudio (see the next subsections for instructions on setting RStudio defaults and information on the installation of RSPARROW library dependencies). 
...
* For the *"R version"*, select the *"Change..."* button and choose an already installed 64-bit R version or, as we recommend, browse to the *"R.exe"* executable binary object for the R-4.4.2 version (*".../R-4.4.2/bin/x64/R.exe"*) and select the binary object.   

* For the *"Default working directory"*, select the *"Browse..."* button to identify the current default working directory. The user should either change the default working directory to the directory where the *"RSPARROW_master"* directory resides or ensure that the *"RSPARROW_master"* directory is located in the current default working directory. Either option is consistent with the use of a relative path name for the `path_master` setting in the RSPARROW control script (section 11). 
...
Users are not required to update R packages to versions that are more recent than those listed for the RSPARROW compatibility versions in the DESCRIPTION file. Note that RSPARROW compatibility with updated R package versions (more recent than those listed in the DESCRIPTION file) is not guaranteed.

More advanced users, who choose to manually download R from [CRAN](https://cran.r-project.org/) and install the RSPARROW R library dependencies, should be attentive to the following messages and prompts associated with the R package installations when opening and executing the RSPARROW control script in RStudio:
...
* *R package installation and updating:*  The required RSPARROW library dependencies (and the most recent library versions) are automatically installed when the control script is initially executed with a newly installed version of R that excludes the RSPARROW R library dependencies (>=R-4.4.2). In subsequent executions of the control script, no updates are made to the library dependencies (see Chapter subsection 4.4.12 for details).

* *R package installation from source:*  If the following question appears in a pop-up box in RStudio during the installation of R library dependencies, users should select *"No"* as the response: *"Do you want to install from sources the packages which need compilation?"*. A *"No"* response installs the most current R packages from binary versions. The selection of *"Yes"* requires that users have installed [*Rtools42*](https://cran.r-project.org/bin/windows/Rtools/) (an R developer package) on the Windows operating system; this enables the compilation and installation of R packages from their source.

* *Resolution of library installation problems:* Users have periodically encountered R library dependencies that will not install properly in RStudio. The problem is often resolved by:  (1) deleting the current R version and installing a new R base version (i.e., >=4.4.2), and (2) executing the RSPARROW control file, which will auto-install the required library dependencies to the new R version. Alternatively, users can install the version of R-4.4.2 with RSPARROW R library dependencies that is included with the RSPARROW download.


Finally, note that the following warning message may appear when the RSPARROW library is loaded to an RStudio session. The warning is of no concern because the cited S3 method is not used by RSPARROW. 
```{r, eval=FALSE}
...
The RSPARROW installation includes three upper-level directories as shown in Figure \ref{directory1}, including two user tutorial directories with examples of static and dynamic models as well as a master directory that contains the RSPARROW software and supporting documentation. The directories may be located parallel to one another or placed in other directory locations, provided that the directories are not nested within each other. Additionally, no changes should be made to the master directory structure and contents. Advanced users and developers should consult chapter 8 to ensure that any updates to the Master files will not interfere with the operation of the RSPARROW system. 

When developing new SPARROW models, users should create an upper-level User Directory for each model that contains three parallel subdirectories *"data"*, *"gis"*, and *"results"*; the names of these subdirectories may be defined by users in the control script (see section 11 of the script). This directory structure and the required input datafiles are provided with the SPARROW tutorial models (Chapter 6 and 7). 

The *"results"* subdirectory contains the control script and supporting input control files that are required to operate the RSPARROW program. We recommend that users operate RSPARROW in RStudio by opening and executing the control script in the *"results"* subdirectory of any User Directory. Execution of the control script in an RStudio session makes the *"results"* subdirectory active by storing its path name internally in R for use in managing file input/output. The names of the control script and the associated input control files are fixed and should not be changed by users.
...
RSPARROW can only be executed in RStudio via user settings in the control script, *sparrow_control.R*, and supporting input control files located in the *"results"* subdirectory (Table 1). The script consists of system variable settings, functionally organized in 12 sections (see Chapter 4, subsection 4.1). Details are given in subsection 4.2 on how to get started with executing the control script, including a checklist with an ordered sequence of steps to guide the setup and testing of system settings and new models (see subsection 4.2.4). An overview of some of the key features of the control script operation is provided in this section.  
...
Stream and catchment maps of model predictions, or other variables listed in the *dataDictionary.csv*, may be created by users with the `master_map_list` setting. Using the `enable_plotlyMaps` setting (section 8 of the control script), maps are stored with either non-interactive (`no`) or interactive (`yes`) features in HTML files in the *"maps"* subdirectory. Alternatively, the map setting `enable_shinyMaps<-"yes"` (section 9 of the control script) can be used to enable the RShiny mapper, allowing users to interactively select and view predictions and optionally save maps of prediction metrics or other variables in the *"maps/Interactive"* subdirectory.  

To enhance the computational speeds for model development and mapping output, binary R objects are created and stored in the *data*, *results*, and *gis* directories (see Tables 1, 3). Users are required to use the control file settings to create GIS R binary objects to support all mapping functions (see Chapter subsection 4.4.8.1). 

Results of the execution of user-defined management scenarios are output to the *"scenarios"* directory. The results are saved as CSV files and HTML maps in a user-assigned subdirectory that is identified by the setting `scenario_name<-"user named scenario directory"`, allowing users to archive the results of multiple scenarios, provided the model is not re-estimated (NOTE: if model estimation is executed, then all prior scenario results are deleted). The RShiny interactive mapper can also be used, and is encouraged, to execute management scenarios and create CSV and PDF/HTML output; the mapped results can be displayed interactively in the RShiny mapper and optionally output to PDF/HTML files or batch processed from the menu and output as PDF/HTML files. The RShiny batch mode is recommended for modeled spatial domains with many reaches (> ~300-500K) due to longer processing times.
...
--------------  --------------------------- ---------------------------------------------------
 data            *data1.csv*,**data1**      Reach-level attribute data required for SPARROW
                                            model calibration and prediction.
                                            \textcolor{white}{.}
 gis             GIS shape files,           Geographic data to support the mapping of 
                   **GeoLines**,            metrics by monitoring site, reach, or catchment. 
                   **lineShape**,           Users are required to create the binary files 
                   **polyShape**            to execute mapping functions.
                                            \textcolor{white}{.}
 results         *sparrow_control.R*        R script to control the specification and 
                                            execution of RSPARROW models and all other 
                                            operations, with user settings organized by 
                                            12 topical categories.
                                            \textcolor{white}{.}
 results        *userModifyData.R*          R script supports user-supplied R data calculations 
                                            and conditional statements with existing and newly-
                                            created variables prior to model execution.                                             
                                            \textcolor{white}{.}
 results        *dataDictionary.csv*        Repository for the variable definitions of all 
                                            RSPARROW input data. These are required to support 
                                            variable calculations, model execution, and user-
                                            controlled mapping of station and reach metrics. 
                                            The file provides a crosswalk between user-defined 
                                            variable names in the *data1.csv* and the required 
                                            RSPARROW system variable names. 
                                            \textcolor{white}{.}
 results        *master_dataDictionary.csv* A continuously updated version of the 
                                            *dataDictionary.csv* that is revised with each model 
                                            execution to record any user edits to the 
                                            *dataDictionary.csv* file. 
                                            \textcolor{white}{.}
 results        *parameters.csv*            Repository of the candidate explanatory variables 
                                            for specifying RSPARROW models. Columns include 
                                            initial parameter values, their minimum and maximum 
                                            bounds, and a flag to identify variables to include
                                            in bivariate correlations of explanatory variables. 
                                            User settings for the minimum and maximum bounds 
                                            control the selection of parameters for model 
                                            calibration.
                                            \textcolor{white}{.}
 results        *design_matrix.csv*         Interaction matrix for the source and land-to-water 
                                            delivery variables.
                                            \textcolor{white}{.}
 results        **data1_priorImport**       Updated *data1.csv* input file with 
                                            *dataDictionary.csv* variable names and user-
                                            selected network attribute updates. Created with 
                                            the execution of the control script, following 
                                            input of the *data1.csv* file (facilitates the 
                                            speed up of data input in subsequent executions of 
                                            the control script for different model 
                                            specifications). 
--------------  --------------------------- ---------------------------------------------------
...
**Table 2. Listing of the RSPARROW input and output datafiles and R objects for static and dynamic User Model subdirectories.** File names shown in parentheses are user defined model names (i.e., "run_id"), scenario and variable names, or labels assigned during execution ("time"). R objects (shown in **bold**) are stored as binary files; the objects contain dataframes or list objects within the R environment. Creation of output files requires that all settings are turned on; otherwise only selected files are output (see Table 6 in Chapter 4.3 for a listing of the crosswalk between the control settings and output of specific datafiles and objects).

User Model Directory and Subdirectory Names   Datafile/**R Object** Name                                          
--------------------------------------------  ----------------------------------------------------------                                                     
...
\textcolor{white}{.}
 (run_id)/data                                **subdata**                                                      
                                              ...
                                              **vsitedata**      
\textcolor{white}{.}
 (run_id)/estimate                            AreaRatio_NewOld.html                                             
...(/diagnostic_darea_mismatches_maps)        demtarea.html
                                              hydseq.html
\textcolor{white}{.}
...
\textcolor{white}{.}
 (run_id)/estimate/mapSiteAttributes          (run_id)_Calibration_Site_Attribute_Maps.html                                               
\textcolor{white}{.}
 (run_id)/estimate/residualMaps               (run_id)_Diagnostics_Residuals_Maps.html 
                                              (run_id)_Validation_Residuals_Maps.html
\textcolor{white}{.}
 (run_id)/estimate/summaryCSV                 ClassRegionModelPerformance_MonitoringAdj.csv
                                              ClassRegionModelPerformance_NoMonitoringAdj.csv
                                              DesignMatrix.csv
                                              EigenvaluesEigenvectors.csv
                                              EigenValueSpread.csv
                                              LargestSqResid.csv
                                              LogResid_StationQuantiles.csv
                                              ModelPerformanceMonitoringAdj.csv
                                              ModelPerformanceNoMonitoringAdj.csv
                                              ObservedYieldPercentiles.csv
                                              ParameterCorrelations.csv
                                              ParameterCovariances.csv
                                              ParameterEstimates.csv
                                              ParameterSummary.csv
                                              PredictedYieldPercentiles.csv
                                              RatioObsToPredLoad_StationQuantiles.csv
                                              StandardResid_StationQuantiles.csv
\textcolor{white}{.}                                                               
 (run_id)/maps...(/Stream)...(/Catchment)     (run_id)_(variable_name).html                           
\textcolor{white}{.}
...
\textcolor{white}{.}
 (run_id)/scenarios/(scenario_name)           (scenario_name)_(run_id)_predicts_load_scenario.csv               
                                              ...
                                              **batch**_**(time).RData**           
.
 (run_id)/scenarios/(scenario_name)           (scenario_name)\_(run_id)\_(variable_name).html                                                
  ...(/Stream)...(/Catchment)                          
\textcolor{white}{.}
 (run_id)/batchSessionInfo                    **(run**_**id).RData**                   
                                              (run_id)_log.txt    
\textcolor{white}{.}
 **Dynamic Model Subdirectory Names**         **Dynamic Model Datafile Names**
\textcolor{white}{.}
 (run_id)/estimate                            **(run_id)\_ANOVAdynamic.list**
\textcolor{white}{.}
 (run_id)/estimate                            CDF_of_Station_Euclidean_Distances.html                                             
.../diagnostic_spatialautocor_dynamic         CDF_of_Station_Hydrological_Distances.html
                                              Morans_I_by_Class_variable.html
                                              Morans_I_by_river_basins.html
\textcolor{white}{.}
 (run_id)/estimate                            DiagSensitivity_by_Param.html                                            
.../diagnostic_sensitivity_dynamic            DiagSensitivity1%_by_Param.html
                                              DiagSensitivity1%_by_Param_logScale.html
\textcolor{white}{.}
 (run_id)/estimate                            ModEstimation_4panel_Classvar.html                                            
.../diagnostic_PlotsNLLS_dynamic              ModEstimation_Box_and_Quantile_Resid.html
                                              ModEstimation_Classvar_Decile_Box.html
                                              ModEstimation_Conditioned_v_Unconditioned_loads.html
                                              ModEstimation_Correlations_ExplanitoryVariables.html
                                              ModEstimation_DrainageArea_DecileClass_Box.html
                                              ModEstimation_LanduseClass_Decile_Box.html
                                              ModEstimation_Obs_v_Pred.html
                                              ModSimulation_4panel_Classvar.html
                                              ModSimulation_Box_and_Quantile_Resid.html
                                              ModSimulation_Classvar_Decile_Box.html
                                              ModSimulation_Correlations_ExplanitoryVariables.html
                                              ModSimulation_DrainageArea_DecileClass_Box.html
                                              ModSimulation_LanduseClass_Decile_Box.html
                                              ModSimulation_Obs_v_Pred.html
                                              Obs_v_Pred_TimeSeriesPlots.html
\textcolor{white}{.}                          
 (run_id)/estimate                            (run_id)_Calibration_Site_Attribute_Maps.html                                              
.../diagnostic_PlotsNLLS_dynamic              
/mapSiteAttributes
\textcolor{white}{.}                          
 (run_id)/estimate                            Model Estimation Log Residuals.html                                             
.../diagnostic_PlotsNLLS_dynamic              Model Estimation Obs-Pred.html
/residualMaps                                 Model Estimation Standardized Residuals.html
                                              Model Simulation Log Residuals.html
                                              Model Simulation Obs-Pred.html
\textcolor{white}{.}
 (run_id)/estimate                            ModSimulation_4panel_Classvar.html
.../validation_plots_dynamic                  ModSimulation_Box_and_Quantile_Resid.html
                                              ModSimulation_Classvar_Decile_Box.html
                                              ModSimulation_DrainageArea_DecileClass_Box.html
                                              ModSimulation_LanduseClass_Decile_Box.html
                                              ModSimulation_Obs_v_Pred.html
\textcolor{white}{.}                          
 (run_id)/estimate                            Model Simulation Log Residuals.html                                            
.../validation_plots_dynamic                  Model Simulation Obs-Pred.html 
/residualMaps                               
 ...
 **data1**                *data1.csv*             none                              `input_data_fileName`    
                                                                                      (section 1) 
                                                                                      \textcolor{white}{.}
 **data1_priorImport**    *data1.csv*             *data1UserNames* replaced         `load_previousDataImport` 
                          *dataDictionary.csv*     with *sparrowNames* from              (section 1)            
                                                   *dataDictionary.csv* in                             
                                                   **data1** object
                                                   \textcolor{white}{.}
...
For the statistical dynamic SPARROW models, the stream reach records in the *data1.csv* file must be both hydrologically ordered (upstream to downstream) and chronologically ordered according to season and/or year. Thus, the *waterid* and *hydseq* variables reflect a unique reach and time period combination, such that the stream records are hydrologically ordered within each consecutive time period in the data1 master data file. Additionally, the variables *season* and/or *year* must be defined in the *data1.csv* file. See the following section for details on the specification of these variables in the dynamic models. Note that the temporal data structure implemented in RSPARROW version 2.0 has been designed to be compatible with the structure required for a fully dynamic SPARROW model in future versions of RSPARROW that would include a more physically based representation of contaminant storage.
...
In the initial execution of the control script, a binary version of the *data1.csv* file, which includes the results of data checks and any user-specified updates from section 2 of the control script, is automatically created in the "User Directory/results" subdirectory and called "(input_data_fileName)_*priorImport*". This allows users to speed up data input in subsequent executions of the control script (see control setting details described in section 1 of the control script or Chapter subsection 4.4.1).

During the execution of an RSPARROW model in an RStudio session, a modified version of the *data1.csv*, the R object *subdata*, is created and serves as the data input for model estimation, prediction, and mapping during the RStudio session; the records are sorted in hydrological order (from upstream to downstream)--i.e., ascending order by the system variable *hydseq* (See Table 4 in section 3.2 for explanation). The object is modified from the *data1.csv* based on user-defined filters, using the control setting `filter_data1_conditions` (section 2 of the control script) or R commands in the *userModifyData.R* script (see subsection 3.5). All variables in the *subdata* object are also contained in the *sitedata* object, with the number of records equal to the number of calibration sites and sorted in hydrological (from upstream to downstream reach) order using the *hydseq* system variable. The *subdata* and *sitedata* objects are used to manage model estimation and prediction as well as the processing of model residuals and diagnostics plots and maps and are stored in the "User Directory/results/(run_id)/data" directory. See Chapter subsection 5.1 for additional details on the contents of the R objects. 

**Required UTF-8 encoding format for the data1.csv file**

The *data1.csv* input file is required to be in UTF-8 format (i.e., Unicode Transformation Format - 8 bits), a universally used text encoding standard for Unicode, which provides unique character to binary translations. The presence of non-UTF-8 characters in the data1 file can cause the loss of data records and the substitution of special characters (e.g., Spanish accents) with other randomly assigned characters (e.g., backslashes). 

The RSPARROW data input function is designed to automatically scan the *data1.csv* file for non-UTF-8 compatible character strings. If such strings are detected, then the following steps are taken:  (1) prints a warning message to the RStudio console that lists the first 6 rows of the data file in which non-UTF-8 strings occur (plus 1 header row); (2) prints a count of the total number of rows of the data file with non-UTF-8 strings; (3) outputs a table of the most likely file text encodings for the user's *data1.csv* file; and (4) stops RSPARROW execution. These tasks are performed by the following RSPARROW functions:  *readData.R*, *mod_read_utf8.R*, and *checkFileEncoding.R*. Note that the detection of file encoding is similar to the read_utf8() function of the xfun package, and that the file encoding returned is only an approximation.

In cases where non-UTF-8 characters are detected, users have the option to attempt a manual data conversion of their *data1.csv* file to UTF-8 format; for example, using the *iconv* command in R as illustrated below. The command should be executed as a Windows system command, and therefore, must be executed in the RStudio *Terminal* window. 

```{r, eval=FALSE}
# Example R command to convert a data file to UTF-8 format, executed in the 
#   RStudio Terminal window
iconv -c -t UTF-8 [insert full pathname]/data1.csv > [insert full pathname]/data1_utf8.csv

```

## *dataDictionary.csv*:  Active repository of the system and user-defined variable names 
...
The *dataDictionary.csv* ensures global system-wide access to the *sparrowNames* variables (Table 4) during an RStudio session. By executing the control script within an RStudio session, variables listed in the *dataDictionary.csv*, including those modified in *userModifyData* script, are stored internally and are available for RSPARROW operations, including estimation, prediction, mapping, and source-change scenario evaluations (see subsection 3.5).
...
**The execution of dynamic models requires the following variable specifications (See Table 4 for additional explanations):**  

* Three additional FIXED variables must include the *mapping_waterid*, *year* and/or *season*.

* The numeric values of the *waterid* and *hydseq* variables must reflect a unique reach and time period combination, based on reach records in the *data1.csv* file that are chronologically ordered according to *season* and/or *year*, with all reach records hydrologically ordered within each time period. It is recommended that the *waterid* variable use the *mapping_waterid* as the base number (i.e., the unique reach location identifier) with the *year* and/or *season* added to the numeric value. Users have the option to create the *hydseq* variable automatically for a dynamic model (see Chapter 4.4.2); note that to manually assign *hydseq* values, it is recommended that users assign the record ID for a *data1.csv* file that is chronologically ordered with records hydrologically sorted within each time step.  


Table 4 identifies five types of explanatory variables that can be used to create the parameter files (*parameters.csv*, *design_matrix.csv*) for an initial SPARROW model (see subsection 4.4.1). These variables include the SOURCE (contaminant sources or surrogates, such as land use), DELIVF (land-to-water delivery factors, such as climatic variables), STRM (stream decay), RESV (reservoir decay), and OTHER (additional variables for use in the stream or reservoir decay). At least one SOURCE variable is required to estimate an RSPARROW model. A DELIVF variable must also be defined in the data dictionary and parameter control files for RSPARROW to operate properly, although the DELIVF variable does not have to be included in the model estimation. 
...
---------- ------------------- ------------------- ----------------------------------------
 REQUIRED  *waterid*           user_id             Unique reach identification (ID) number. 
                                                   For the dynamic model, the *waterid* 
                                                   reflects a unique reach and time period 
                                                   combination. (*waterid* values are 
                                                   renumbered if the number of values 
                                                   exceeds 1M, with the original *waterid* 
                                                   values saved in the *mapping_waterid*
                                                   variable in the *data1.csv* file).
                                                   \textcolor{white}{.}
 REQUIRED  *fnode*             user_fnode          Reach from (upstream) node
                                                   \textcolor{white}{.}
 REQUIRED  *tnode*             user_tnode          Reach to (downstream) node
                                                   \textcolor{white}{.}
 REQUIRED  *frac*              user_frac           Reach transport fraction, ranging from 
                                                   0 to 1 (1=no diversion of water/mass) 
                                                   \textcolor{white}{.}
 REQUIRED  *iftran*            user_iftran         "if transport" indicator (0=no; 1=yes); 
                                                   nontransport reaches, such as coastal 
                                                   and lake shoreline segments, should be 
                                                   set to zero   
                                                   \textcolor{white}{.}
 REQUIRED  *demiarea*          user_area           Reach incremental drainage area 
                                                   \textcolor{white}{.}
 REQUIRED  **hydseq**          user_hydseq         Unique hydrological sequence number; 
                                                   used to sort the  *data1.csv* reach file 
                                                   (and *subdata* object file) in 
                                                   hydrological order (from upstream to 
                                                   downstream) to enable the summation of 
                                                   area and constituent mass 
                                                   \textcolor{white}{.}
 REQUIRED  *termflag*          user_termflag       Reach terminal flag indicator (1=stream 
                                                   reach; 3=coastal shoreline reach) 
                                                   \textcolor{white}{.}
 REQUIRED  *rchtype*           user_rchtype        Reach type indicator (0=reach; 1=reservoir 
                                                   internal reach; 2=reservoir outlet reach; 
                                                   3=coastal segment) 
                                                   \textcolor{white}{.}
 REQUIRED  *calsites*          user_calsites       Calibration site flag that identifies user 
                                                   selected sites (0=not selected; 1=selected) 
                                                   for model calibration (`if_estimate`<-"yes") 
                                                   and model simulation (`if_estimate_`
                                                   `simulation`<-"yes"). Selected sites must 
                                                   have positive load values (*depvar*>0).  
                                                   One site per reach is allowed.  
                                                   \textcolor{white}{.}
 FIXED     *target*            user_target         Terminal target reach (1=target; 0=non 
                                                   target) for computing load delivery from 
                                                   each upstream reach to the nearest 
                                                   downstream target reach (e.g., input to 
                                                   estuary or reservoir)
                                                   \textcolor{white}{.}
 ...
 FIXED     **demtarea**        user_demtarea       Reach total drainage area 
                                                   \textcolor{white}{.}
 FIXED     **headflag**        user_headflag       Reach headwater flag (1=headwater reach; 
                                                   0=other reach) 
                                                   \textcolor{white}{.}
 FIXED     *length*            user_length         Reach length                                   
                                                   \textcolor{white}{.}
 FIXED     *meanq*             user_mean_flow      Mean annual streamflow; needed for 
                                                   estimating concentration and stream decay  
                                                   \textcolor{white}{.}
 FIXED     *rchtot*            user_rchtot         Reach time of travel; needed for estimating 
                                                   stream decay 
                                                   \textcolor{white}{.}
 FIXED     *hload*             user_hload          Areal hydraulic load for impoundments; 
                                                   needed for estimating aquatic decay 
                                                   \textcolor{white}{.}
 FIXED     *staid*             user_staid          Unique station ID sequence number; an 
                                                   updated hydrologically ordered sequence 
                                                   number is automatically assigned based on 
                                                   user-selected calibration sites (*calsites*=1)  
                                                   \textcolor{white}{.}
 ...
 FIXED     *depvar_se*         user_depvar_se      Mean annual load standard error 
                                                   \textcolor{white}{.}
 FIXED     *weight*            user_weight         Optional residual weights used to execute a 
                                                   weighted nonlinear RSPARROW model 
                                                   (default=1.0) 
                                                   \textcolor{white}{.}
 FIXED     *mapping_waterid*   user_id             Unique reach identification (exclusive of 
                                                   time) that is used to merge geographic shape 
                                                   files and execute geographic upstream search 
                                                   algorithms in the RShiny DSS (note that the 
                                                   variable is unique only to the reach and not 
                                                   the time period). This is required for use 
                                                   with a dynamic model.  
                                                   \textcolor{white}{.}
 FIXED     season              user_season         Character identifier of the season time step 
                                                   (‘winter’,’spring’,’summer’,’fall). This is 
                                                   required for use with a dynamic seasonal model. 
                                                   RSPARROW internally sorts records according to 
                                                   the sequential order of ‘winter’ to ‘fall’. The
                                                   variable should be present in the *data1.csv* 
                                                   file to fully enable use of the stream network 
                                                   verification tasks (as enabled in section 2 of 
                                                   the control script). 
                                                   \textcolor{white}{.}
 FIXED     year                user_year           Numeric identifier for the year time period. 
                                                   This is required for use with dynamic annual 
                                                   and seasonal models. The variable should be 
                                                   present in the *data1.csv* file to fully enable 
                                                   use of the stream network verification tasks 
                                                   (as enabled in section 2 of the control script). 
                                                   \textcolor{white}{.}
 ...
 RESV      reservoir_variables user_reservoir_vars Reservoir decay explanatory variables
                                                   \textcolor{white}{.}
 OTHER     other_variables     user_other_vars     User-specified variables for inclusion in the 
                                                   stream or reservoir decay specifications
                                                   \textcolor{white}{.}
 OPEN      LULC_variables      user_LULC_vars      Variables for the drainage areas of land cover 
                                                   and land-use classes of interest; used for 
                                                   diagnostic metrics and plots (see section 5 of 
                                                   the control script; `class_landuse` setting) 
                                                   \textcolor{white}{.}
 OPEN      CLASS_variables     user_class_vars     A unique (e.g., hierarchical) classification 
                                                   code for spatially contiguous drainage areas, 
                                                   such as hydrologic regions (e.g., HUC in USA), 
                                                   political units, or other ecological or 
                                                   physiographic regions; used for diagnostic 
                                                   metrics and plots (see section 5 of the 
                                                   control script; `classvar` setting) 
                                                   \textcolor{white}{.}
 OPEN      S_source_variables  user_source_vars    User-defined source variables (listed in the 
                                                   `scenario_sources` setting; see section 9 of 
                                                   control script); contains the source-change 
                                                   factors for evaluating hypothetical management 
                                                   scenarios; the prefix "S_" is required 
                                                   \textcolor{white}{.}
 OPEN      user_variables      user_variables      Any variables present in the *data1.csv* file 
                                                   or created in the *userModifyData* script for 
                                                   which global access is required for mapping 
                                                   \textcolor{white}{.}
 OPEN      *valsites*          user_valsites       Validation site flag to support one of two 
                                                   methods for the selection of sites for model 
                                                   validation (0=not selected; 1=selected); 
                                                   selected sites must also have positive load 
                                                   values (*depvar*>0). See explanation for the 
                                                   `if_validate<-"yes"` control setting in 
                                                   subsection 4.4.6; the *valsites* name (i.e., 
                                                   *sparrowNames*) is a unique reserved system 
                                                   variable name that must be used with one of 
                                                   the validation site methods. 
---------- ------------------- ------------------- -------------------------------------------------------
...
See the discussion of the `if_estimate` control setting in Chapter subsection 4.4.4 (section 4 of the control script) for details on recommended parameter settings for the *parameters.csv* file.
...
\textcolor{white}{.}
*parmInit*               Initial value for the nonlinear least squares (NLLS) estimation or the 
                         fixed value for predictions in simulation mode. For SOURCE and aquatic 
                         decay (STRM, RESV) variables, an initial value of zero or a small 
                         positive value is recommended. For the delivery factors (DELIVF), an 
                         initial value of zero is generally recommended, given that the estimated 
                         parameters may be either negative or positive.
\textcolor{white}{.}
*parmMin*                Minimum bound on the estimated or fixed parameter. Typically zero for 
                         SOURCE, STRM, and RESV parameter types and a zero or small negative 
                         value (e.g., -10000) for DELIVF and OTHER parameter types.  A zero or 
                         NA value for both the *parmMin * and *parmMax* will exclude the 
                         parameter from the model.
\textcolor{white}{.}
*parmMax*                Maximum bound on the estimated or fixed parameter. Typically a large 
                         positive value for all parameter types (e.g., 10000), but a zero may be 
                         specified for DELIVF or OTHER parameter types. A zero or NA value for 
                         both the *parmMin * and *parmMax* will exclude the parameter from the 
                         model.
\textcolor{white}{.}
*parmType*               Parameter type (SOURCE, DELIVF, STRM, RESV, OTHER)
\textcolor{white}{.}
*parmCorrGroup*          Parameter selection setting for inclusion or exclusion from diagnostic 
                         plots and matrices of all-possible explanatory variable Spearman's 
                         rank correlations (enter "1" to be included or "0" to be excluded from 
                         the correlations; see explanation of the control setting 
                         `if_corrExplanVars` in section 5 of the control script).
------------------------ -------------------------------------------------------------------------
...
* Define monitoring station attributes to display in the diagnostics output HTML file (setting `map_siteAttributes.list` in the control script section 8):
```{r, eval=FALSE}
...
1. Execute at least one of the tutorial models (Chapter 6 or 7) to ensure that the required RSPARROW software (R, RStudio, R library dependencies) has been properly installed. See Chapter subsection 1.3 for details on the software installation. 

2. For a new model control script, set the path name for the RSPARROW Master directory (i.e., "RSPARROW_master") and User subdirectory names (e.g., "data", "gis", "results") or use a relative path name for the RStudio default working directory (section 11 of the control script). Execution of the control script in an RStudio session will automatically determine the location (path name) of the upper level user directory (e.g., "UserTutorial") and the subdirectory paths (e.g., "results", "gis", "data"). 
...
3. Setup the required control input files and data import options (section 1 of the control script; also see Chapter 3 for guidance on the required content of the control input files). The control settings in the control script section 3 define CSV file delimiters, CSV decimal symbols, and the name of the data1 input file. Optional settings are also available to create the initial versions of the CSV control input files for modeling; these use the column header variable names in the *data1.csv* file to setup columns in the files. 
    ...
    3b. The setting `create_initial_dataDictionary<-"yes"` (see subsection 4.4.1 for details) allows users to automatically create the *dataDictionary.csv* file, using column header names in the *data1.csv* file and internal RSPARROW system variable names. The file will be saved to the "User Directory/results" subdirectory where it must remain to be located by the control script. The file name is also fixed and should not be changed by users. 
    ...
    * Additional editing of the *dataDictionary.csv* is required to fill-in the `NA` values by completing the variable types (*varType*) and internal R variable names (*sparrowNames*). 
    ...
    3c. The setting `create_initial_parameterControlFiles<-"yes"` (see subsection 4.4.1 for details) allows users to automatically create the *parameters.csv* and *design_matrix.csv* control input files from the *dataDictionary.csv* file. The control files will be saved to the "User Directory/results" subdirectory where they must remain to be located by the control script. The file names are also fixed and should not be changed by users. 
...
2. Using the Window's environment, manually copy the R control script and control input files from the previously executed model *"run_id"* subdirectory into the upper-level *"results"* directory.
...
* Set `if_estimate<-"no"` (and `if_estimate_simulation<-"no"`) to use the prior estimated (or simulated) model results to generate new predictions, maps, and management scenarios, or to enable the RShiny interactive mapper. Thus, no model re-estimation is necessary in this case. 
...
**Table 6. General listing of the datafiles and R objects output to RSPARROW directories by the control script settings.** R objects (shown in **bold**) are stored as binary files; the objects contain dataframes or list objects within the R environment. File names shown in parentheses are user defined model names (i.e., "run_id"), scenario and variable names, or labels assigned during execution ("time"). Note that users must specify supplementary settings in many of these sections to obtain complete functionality and to output all of the text and graphics.

Directory Control Setting                               Name of Datafile or R Binary Object                   
------------------------------------------------------- -----------------------------------------------------------------
 Directory: results                                                                      
...
`if_verify_demtarea` (2) new drainage area differs         (run_id)_diagnostic_darea_mismatches.csv     
 from *demtarea* by >1%; `enable_plotlyMaps` (8)           (run_id)_diagnostic_darea_mismatches.html     
...
\textcolor{white}{.}
Directory: results/(run_id)/estimate/mapSiteAttributes     (run_id)_Calibration_Site_Attribute_Maps.html
`if_estimate` or `if_estimate_simulation` (4)                                              
and `enable_plotlyMaps` (8)
\textcolor{white}{.}
Directory: results/(run_id)/estimate/residualMaps           
`if_estimate` or `if_estimate_simulation` (4)              (run_id)_Diagnostics_Residuals_Maps.html
and `enable_plotlyMaps` (8)                                
`if_estimate` and `if_validate` (6)                        (run_id)_Validation_Residuals_Maps.html
and `enable_plotlyMaps` (8)
\textcolor{white}{.}
Directory: results/(run_id)/maps                                                          
 ...(/Stream)...(/Catchment)                                                                     
`master_map_list`, `output_map_type`, and                  (run_id)_(variable_name).html    
  `enable_plotlyMaps` (8)                                           
\textcolor{white}{.}
...
prediction                                                  
`outputESRImaps` (8)                                       lineShape.(dbf,prj,shp,shx), polyShape.(dbf,prj,shp,shx) 
\textcolor{white}{.}
Directory: results/(run_id)/maps/ESRI_ShapeFiles/         
residuals                       
`outputESRImaps` (8)                                       residShape.(dbf,prj,shp,shx)     
\textcolor{white}{.}
Directory: results/(run_id)/maps/ESRI_ShapeFiles/         
siteAttributes
`outputESRImaps` (8)                                       siteAttrshape.(dbf,prj,shp,shx)     
\textcolor{white}{.}
...
Directory:  results/(run_id)/scenarios/(scenario_name)                                                              
...(/Stream)...(/Catchment)                                                               
`if_predict_scenarios` (9), `scenario_name` (9),           (scenario_name)\_(run_id)\_(variable_name).html
...
\textcolor{white}{.}
 **Control Settings for Dynamic Models**                 **Dynamic Model Datafile Names**
\textcolor{white}{.}
Directory: results/(run_id)/estimate                    
To execute dynamic model, FIXED variables must           **(run_id)\_ANOVAdynamic.list**
include:  *mapping_waterid* and *year* and/or *season*,
with *waterid* and *hydseq* reflecting unique reach and
time period combinations
\textcolor{white}{.}
Directory: results/(run_id)/estimate                                                               
/diagnostic_spatialautocor_dynamic                   
`if_estimate` or `if_estimate_simulation` (4)           CDF_of_Station_Euclidean_Distances.html      
 and `if_spatialAutoCorr` (5),                          CDF_of_Station_Hydrological_Distances.html
 and `diagnosticPlots_timestep` (8)                     Morans_I_by_Class_variable.html
                                                        Morans_I_by_river_basins.html                                                        
\textcolor{white}{.}
Directory: results/(run_id)/estimate 
/diagnostic_sensitivity_dynamic
`if_estimate` or `if_estimate_simulation` (4)           DiagSensitivity_by_Param.html 
 and `diagnosticPlots_timestep` (8)                     DiagSensitivity1%_by_Param.html
                                                        DiagSensitivity1%_by_Param_logScale.html
\textcolor{white}{.}
Directory: results/(run_id)/estimate                                                                        
/diagnostic_PlotsNLLS_dynamic                     
`if_estimate` or `if_estimate_simulation` (4)           ModEstimation_4panel_Classvar.html
 and `diagnosticPlots_timestep` (8), `classvar` (5),    ModEstimation_Box_and_Quantile_Resid.html
 `class_landuse` (5)                                    ModEstimation_Classvar_Decile_Box.html
                                                        ModEstimation_Conditioned_v_Unconditioned_loads.html
                                                        ModEstimation_Correlations_ExplanitoryVariables.html
                                                        ModEstimation_DrainageArea_DecileClass_Box.html
                                                        ModEstimation_LanduseClass_Decile_Box.html
                                                        ...
                                                        ModSimulation_LanduseClass_Decile_Box.html
                                                        ModSimulation_Obs_v_Pred.html
`if_estimate` or `if_estimate_simulation` (4)           Obs_v_Pred_TimeSeriesPlots.html                                            
and `diagnostic_timeSeriesPlots` (8)                                                       
\textcolor{white}{.}                          
Directory: results/(run_id)/estimate                                                                          
/diagnostic_PlotsNLLS_dynamic/mapSiteAttributes              
`if_estimate` or `if_estimate_simulation` (4)           (run_id)_Calibration_Site_Attribute_Maps.html
and `enable_plotlyMaps` (8)
\textcolor{white}{.}                          
Directory: results/(run_id)/estimate                                                                         
/diagnostic_PlotsNLLS_dynamic/residualMaps                     
`if_estimate` or `if_estimate_simulation` (4)           Model Estimation Log Residuals.html  
 and `enable_plotlyMaps`, `map_years`, `map_seasons`    Model Estimation Obs-Pred.html  
`mapsPerPage`, `mapPageGroupBy` (8)                     Model Estimation Standardized Residuals.html                  
                                                        Model Simulation Log Residuals.html            
                                                        Model Simulation Obs-Pred.html
\textcolor{white}{.}
Directory: results/(run_id)/estimate                            
/validation_plots_dynamic                     
`if_estimate` and `if_validate` (4)                      ModSimulation_4panel_Classvar.html
 and `diagnosticPlots_timestep` (8)                     ModSimulation_Box_and_Quantile_Resid.html
 and `classvar`, `class_landuse` (5)                    ModSimulation_Classvar_Decile_Box.html
                                                        ModSimulation_DrainageArea_DecileClass_Box.html
                                                        ModSimulation_LanduseClass_Decile_Box.html
                                                        ModSimulation_Obs_v_Pred.html
\textcolor{white}{.}                          
Directory: results/(run_id)/estimate                                                                     
/validation_plots_dynamic/residualMaps                   
`if_estimate` and `if_validate` (4)                     Model Simulation Log Residuals.html 
 and `enable_plotlyMaps`, `map_years`, `map_seasons`    Model Simulation Obs-Pred.html  
`mapsPerPage`, `mapPageGroupBy` (8)
------------------------------------------------------- -----------------------------------------------------------------
...
The setting allows users to automatically create the *dataDictionary.csv* file, using column header names in the *data1.csv* file and internal RSPARROW system variable names. The *dataDictionary.csv* will be saved to the "User Directory/results" subdirectory where it must remain to be located by the control script. The file name is also fixed and should not be changed by users. 
...
```     
The setting allows users to automatically create the *parameters.csv* and *design_matrix.csv* control input files from the *dataDictionary.csv* file. The *parameters.csv* and the *design_matrix.csv* will be saved to the "User Directory/results" subdirectory where they must remain to be located by the control script. The file names are also fixed and should not be changed by users.

Execution of the control script will create the parameter and design matrix files and fill-in in the *sparrowNames*, *parmType*, units, and description columns using the *dataDictionary.csv* definitions.  
    
Prior to executing this setting with the control script, users should edit the *dataDictionary.csv* to fill-in the *varType* for all variables. A minimum of one SOURCE *varType* is required to execute the control setting. In addition, one DELIVF *varType* is required to subsequently execute steps 5 and 6 in the checklist in subsection 4.2.4. Users should assign the *data1UserNames* variable name to missing entries of the *sparrowNames* for all variables other than those with a *varType* designation of REQUIRED or FIXED. 
...
Additional editing of the columns and cells in the newly-created parameter control files (*parameters.csv*, *design_matrix.csv*) will not be necessary to complete the setup and verification in steps 5 and 6 in the checklist in subsection 4.2.4. However, additional editing will be required to the columns in these files to execute user-specified models (see subsection 4.4.4.3). The *parmUnits* and *description* columns in the *parameters.csv* file are optional but will be automatically filled according to values in the *dataDictionary.csv*.
...
#-------------------------------------------------------------------------------     
   ...
   # "no" imports the data1 csv file defined in 'input_data_fileName' and saves 
   #      it as a binary file, overwriting any prior version of the binary file
...
```   
This setting executes the function *userModifyData.R* (located in the "User Directory/results" subdirectory). The function supports user-defined data calculations and conditional statements with existing or newly-created variables in the `input_data_fileName` file (e.g., *data1.csv*). Users may also use R statements in the function to filter calibration sites (e.g., eliminate sites or their associated loads that are deemed unacceptable for use in the model calibration). See Chapter subsection 3.5 for details on the setup and use of this function, including examples of R statements that can be used with RSPARROW models.  
...
The evaluations of reach connectivity described in this section can be optionally executed for static and dynamic models.  For dynamic models, where the reach network is associated with multiple time steps (i.e., annual and/or seasonal), the evaluations of reach connectivity can be optionally applied to all or selected time steps as specified by the four mapping control settings described in Chapter section 4.4.8.4. Users also have the option to create the *hydseq* variable automatically for a dynamic model that includes a unique reach and timestep order as required for estimation of the model; this procedure, as executed by settings in section 2 of the control script, automatically sorts the reach records hydrologically within each timestep. 
...
For the control setting `if_verify_demtarea<-"yes"`, the messages below are written to the RStudio Console window. This includes a listing of the number of reaches with differences in drainage areas, based on a comparison of the newly-computed areas to the user-provided total drainage area.  
```{r, eval=FALSE} 
...
Writing results from drainage area comparisons (CSV, HTML maps) in estimate directory...
```
...
calculate_reach_attribute_list <- c("hydseq","headflag","demtarea")
calculate_reach_attribute_list <- NA  # no calculation is requested
```
Users can optionally use RSPARROW to determine values for as many as three reach attributes, *hydseq*, *headflag*, and *demtarea*. The values are determined using the RSPARROW "hydseq" function (*hydseq.R*), an R translation of a SAS subroutine available with the SAS SPARROW software (G.E. Schwarz, 2017, personal communication). These attributes are defined as:  *demtarea* is the RSPARROW total drainage area, *hydseq* is the unique hydrological sequence number, and *headflag* is the headwater identifier value (1=headwater reach; 0=non-headwater reach). 
...
#-------------------------------------------------------------------------------      
  ...
  #  Example filtering of reaches based on drainage density conditions:
  filter_data1_conditions <- c("data1$drainden > 0 & !is.na(data1$drainden)")
  filter_data1_conditions <- NA  # no filtering selected
```
...
**The execution of dynamic models requires the following variable specifications:**  (1) Three additional FIXED variables must include the *mapping_waterid*, *year* and/or *season*; and (2) The numeric values of the *waterid* and *hydseq* variables must reflect a unique reach and time period combination, based on reach records in the *data1.csv* file that are chronologically ordered according to *season* and/or *year*, with all reach records hydrologically ordered within each time period. See Chapter section 3.2 (setup of the *dataDictionary.csv*) for an explanation of the creation and specification of these variables. 
...
Subsequent to model execution, the mean-adjusted land-to-water delivery variables (and all other variables required to execute a model) are stored in the *estimate* subdirectory in the R binary object *DataMatrix.list* (see Chapter subsection 5.2.3.1 for details on the content of the object). 
...
#-------------------------------------------------------------------------------
  ...
  #   where the OTHER index references the parameter vector sequence, corresponding 
  #   to the order of the OTHER variables listed in the parameters.csv file
```
...
Model estimation uses the conditioned model predictions, which are calculated by substituting the observed station load for the model predicted load on reaches with monitoring stations (Schwarz et al., 2006). Model performance (see subsection 4.4.4.5) is reported for both the conditioned and unconditioned (simulated) model predictions. Model prediction output also includes both conditioned and unconditioned predictions (for details on the prediction types, see subsection 4.4.7).  
...
**Table 7. Recommended parameter settings in the *parameters.csv* file.** Note that at least one of the initial values for the model must be non-zero for the *nlfb* function to execute without error. *Fixed Parameters* are set to a constant non-zero value and included when making model predictions.  The following header definitions apply:  *parmType* (Parameter Type), *parmInit* (Initial parameter value), *parmMin* (Parameter minimum or lower bound), *parmMax* (Parameter maximum or upper bound).

*parmType*               *parmInit*                    *parmMin*            *parmMax*      
----------------------- ------------------------------ -------------------- ---------------------
*Estimated Parameters*  
 DELIVF                  0                             large negative value large positive value  
                                                       (e.g., -10000)        (e.g., 10000)    
 SOURCE                  small positive value          0                    large positive value 
                                                                             (e.g., 10000)                              
 STRM                    small positive value          0                    large positive value  
                                                                             (e.g., 1000)
 RESV                    small positive value          0                    large positive value 
                                                                             (e.g, 1000)
\textcolor{white}{.}
...
Performance measures are reported in the *(run\_id)\_summary.txt* file (subsection 5.2.4.2) and saved as CSV files in "summaryCSV" subdirectory. These measures describe the general acceptability of the overall fit of the model to the observations. Users should also conduct more detailed assessments of whether the model assumptions are satisfied as described in subsections of Chapter 4.4 (subsections 4.4.4.6 to 4.4.4.8 and 4.4.5.1). 
...
The above performance measures are reported for *conditioned* (monitoring-station adjusted) and *unconditioned* (simulated) predictions (also see discussion of model predictions in subsection 4.4.7). 
...
The bias-correction factor is reported as the `Mean Exp Weighted Error` in the model summary *(run\_id)\_summary.txt* file (see subsection 5.2.4.2). 
...
#-------------------------------------------------------------------------------
  ...
  # "yes" indicates that all estimation, prediction, maps, and scenario files 
  #   from the subdirectory with the name = run_id will be deleted 
  #   and only regenerated if settings are turned on. This will over-ride a selection 
  #   of if_estimate<-"yes".
  # "no" indicates the model should be run in simulation mode, without formal NLLS 
  #   parameter estimation, using the user-defined initial values (parmInit) in the 
  #   parameters.csv file as fixed parameter values.
if_estimate_simulation<-"no"
```
This setting executes the model in simulation mode, without formal NLLS parameter estimation, using the user-defined initial values (*parmInit*) in the *parameters.csv* file as fixed parameter values during model execution. This setting can be used to test the model calculations and predictions (e.g., summation of mass loads in the river network) prior to estimating the model (also specify the control setting `if_predict<-"yes"`). The setting can also be used, for example, to execute a simple land-use model with literature export coefficient values, which may be informative to establish baseline information about constituent loads in streams. Note that simulation performance diagnostics are output (see sections 5.2.4.2 and 5.2.4.9) for monitoring sites with positive load values (*depvar*>0).
...
#-------------------------------------------------------------------------------
  ...
  # Options: 
  # "yes" computes the second derivative Hessian standard errors for the estimated 
  #   model coefficients. These calculations can be more computationally demanding 
  #   than Jacobian estimates.
...
In cases of severe multi-collinearity that results in a singular (ill-conditioned) Hessian matrix, RSPARROW terminates the model execution. The Hessian matrix will be singular (with a determinant equal zero) if at least one of the explanatory variables can be expressed as a linear combination of one or more of the other explanatory variables. See subsection 4.4.4.8 for details. 

Note that the standard errors of the coefficients of statistical dynamic RSPARROW models are not adjusted for serial correlation, which can exist in the time series of the model residuals. Serial correlation in dynamic model residuals can cause somewhat underestimated coefficient standard errors, which will result in overestimates of the significance levels (i.e., lower p-values) of SPARROW model coefficients (e.g., Ator et al., 2019; Schmadel et al., 2021). 
...
#-------------------------------------------------------------------------------
  ...
  # Options: 
  # "yes" executes a Moran's I test and renders diagnostics_spatialautocorr.html
...
Test results are reported in the *(run\_id)\_diagnostic_spatialautocor.html* and  *(run\_id)\_diagnostic_spatialautocor.txt* files in the "(run_id)/estimate" subdirectory. Separate test results are reported using distance weights specified according to Euclidean and hydrologic distances between the calibration stations (Fig. \ref{SpatialAutocor2}). The results are reported for the following three spatial domains:  
...
* The full modeled domain and regions, based on the user-specified contiguous drainage areas (Fig. \ref{SpatialAutocor2}c,d) as defined by the first classification variable listed in the `classvar` setting (described below in subsection 4.4.5.2), using Euclidean distance weights. 
...
#-------------------------------------------------------------------------------
  ...
  # "no" does not split the monitoring sites and skips rendering validation 
  #   reports
  if_validate <- "yes"
...
Both site selection methods can be used when executing a dynamic SPARROW model. For dynamic models, the first site selection method randomly selects reaches from the full set of monitored network reaches without constraints on the site ID or the time period. Therefore, this method has the disadvantage of potentially allowing the same site to be used for validation and calibration, with the validation set of mean loads (annual and/or seasonal) randomly selected from the monitoring site time periods. Alternatively, it is recommended that users employ the second site selection method to manually identify the monitoring sites and time periods that are to be used for validation purposes. 
...
#-------------------------------------------------------------------------------
  ...
  add_vars<-NA
  add_vars<-c("huc4") # For dynamic model, "season" and/or "year" added by default
...
When a dynamic model is executed, the time-related variables *year* and/or *season* will be added by default to the list of additional variables for output. The user can also optionally add these variables to the list as shown above.  
...
```{r, eval=FALSE}
  ...
  # Indicate whether shape files are to be converted to binary.  NOTE: This is required 
  # to be executed at least once to use any RSPARROW mapping feature.
  # Options: 
...
Note that only geographic (latitude-longitude) projections are currently supported for RSPARROW map output to HTMLS, PDFs, and in RShiny map displays (i.e., `CRStext <- "+proj=longlat +datum=NAD83"`). The maps render using a Mercator projection. Note that the *leaflet* option in the RShiny mapper provides a Google Mercator projection, with a map display CRS=EPSG:3857 and a data CRS=EPSG:4326. No edits should be made to the `CRStext` setting in the current RSPARROW version.
...
```{r, eval=FALSE}
  ...
  # (4) site attributes - monitoring site variables in map_siteAttributes.list 
  #                       in section 8
  # Options: 
...
```
This setting controls the production and output of ESRI shape files, which can be optionally specified for four types of map output (see Chapter subsection 5.3.4 for details on the directory structure and output file names). The shape files may include: 

* Streams and catchments:  This controls the stream and catchment maps of the variables specified in the `master_map_list` setting. These variables may include any of the standard SPARROW prediction variables (listed above in Chapter subsection 4.4.7), the prediction uncertainties (as described in Chapter subsection 5.2.2.3), or any of the *sparrowNames* variables listed in the *dataDictionary.csv* file. 
...
```{r, eval=FALSE}
  #-------------------------------------------------------------------------------
  # Attribute maps of model predictions and dataDictionary variables
...
```  
These settings allow model predictions (listed above in Chapter subsection 4.4.7) or any of the numeric *sparrowNames* variables listed in the *dataDictionary.csv* file to be specified (`master_map_list`) and mapped to streams and/or catchments as selected in the `output_map_type` setting. The maps are output to HTML files, with non-interactive or interactive features, according to the control settings described in Chapter subsection 4.4.8.4 below. 
...
```{r, eval=FALSE}
  ...
  # Set integer indicating the number of decimal places to be rounded for 
  # prediction maps. NOTE: Rounding to a negative number of digits means 
  # rounding to a power of ten, so for example, rounding to -2 rounds to 
  # the neared hundred. 
...
# Set integer indicating the number of decimal places to be rounded for site 
# attribute maps. NOTE: Rounding to a negative number of digits means rounding 
# to a power of ten, so for example, rounding to -2 rounds to the neared hundred.
...
```{r, eval=FALSE}
  ...
  # Controls reports of diagnostic, spatial autocorrelation, validation, and 
  #   sensitivity plots by time steps
  # Options: 
...
For dynamic models, the setting `diagnosticPlots_timestep` controls the timestep that is used for the diagnostic plots that are output within the upper-level files in the "…/estimate/diagnosticPlotsNLLS_dynamic" directory. Choices include "season" and "year", with the former outputting the plots of the calibration observations for all years by each season and the latter outputting the plots of the calibration observations for all seasons by each year. Plots are not executed when a setting of "NA" is specified. Note that files output to the subdirectories "mapSiteAttributes" and "residualMaps" are not controlled by this setting. 

The setting `diagnostic_timeSeriesPlots` controls the output of time series plots of the predicted and observed loads for the calibration period of record for each of the calibration monitoring. The plots appear in the output file *Obs_v_Pred_TimeSeriesPlots.html*.  Users who are interested in outputting additional time series plots of the diagnostic variables and observations can import the *(run\_id)\_residuals.csv* file in the "…/estimate" subdirectory to R or another software package. The residuals file includes variables for the predicted loads, observed loads, time period (season and/or year), model residuals, and other attributes associated with the model performance for the monitoring site data. 
...
```{r, eval=FALSE}
  ...
  # NOTE: if residual_map_breakpoints is set to NA, then breakpoint defaults 
  #       will be applied
  ...
  # NOTE: residualColors must be length = 8 corresponding to 
  #       residual_map_breakpoints
  residualColors <- c(
  "red", "red", "gold", "gold", "dark green", "dark green", "blue", "blue"
  )

  # residualPointStyle must be length = 8 corresponding to 
  #     residual_map_breakpoints
  # Examples: 
  ...
  # residualPointSize_breakpoints must be length = 8 corresponding 
  #    to breakpoints
  residualPointSize_breakpoints <- 
    c(0.75, 0.5, 0.4, 0.25, 0.25, 0.4, 0.5, 0.75)
...
```{r, eval=FALSE}
  ...
  # NOTE: when "yes", maps will require substantially more computational time, 
  #  system memory, and disc space compared to "no". When "yes", many maps will 
  #  be saved to their own file, outside of their associated report, to increase 
  #  likelihood of successful rendering.
  enable_plotlyMaps <- "yes"

  # Define which dataDictionary variables are displayed in the interactive hover 
  #   text.  NOTE: For dynamic models, "year" and/or "season" are added by default. 
  #   For dynamic models, each variable must be unique fr each reach and timestep 
  #   else the variable is removed. Latitude and longitude are automatically 
  #   included for site attribute maps.
...
For dynamic models, variables that are listed for the `add_plotlyVars` setting **MUST** be unique for each reach and timestep or the variable will be removed from the list during execution. Also note that the variables “year” and/or “season” are added to the list by default if a dynamic model is being executed. 
...
# NOTE: Fewer maps may appear per page if the mapPageGroupBy selection contains less 
#   available data for maps than mapsPerPage selection
mapsPerPage <- 4
...
# NA No grouping
# Examples: 
# GroupBy year Example: If mapsPerPage is 4 and the year 2001 has only 3 seasons of 
#   data, setting the mapPageGroupBy to "year" will result in a 3-panel map for 2001 
#   followed by a 4 panel plot for 2002 if 4 seasons exist for 2002
# GroupBy season Example: If mapsPerPage is 4 and the year 2001 has only 3 seasons 
#   of data, then grouping by "season" will result in only 1 season being displayed 
#   per page. If mapping only 2 years (2001 and 2002), 3 2-panel maps would be 
#   generated for each of the 3 seasons that are present in 2001 and 2002, and 1 
#   1-panel map for the season present in 2002
# No Grouping Example: If mapsPerPage is 4, mapPageGroupBy is NA, and the year 2001 
#   has only 3 seasons of data, the result will be that every page contains 4 maps 
#   and the last page of maps would have any remaining maps. There would be 3 maps 
#   from 2001 (one for each season) and 1 map from 2002 to form the first 4-panel 
#   map and then the last page would contain a 3-panel map of the remaining seasons 
#   in 2002, assuming 2002 is the last year being mapped.
mapPageGroupBy <- "year"

```
For dynamic models, the above four control settings manage the annual and/or seasonal observations that are displayed in separate *plotly* maps in the HTML files. The settings control the format of HTML files output to six subdirectories: 

  * *"…/estimate/diagnosticPlotsNLLS_dynamic/residualMaps"*;
  
  * *"…/estimate/diagnosticPlotsNLLS_dynamic/mapSiteAttributes"*; 
  
  * *"…/estimate/validation_plots_dynamic/residualMaps"*;
  
  * *"…/estimate/diagnostic_darea_mismatches_maps"*; 
  
  * stream and catchment model predictions in *"…/maps/streams"* or *"…/maps/catchment"*, with the prediction variable list for output controlled by the `master_map_list` setting; and 
  
  * stream and catchment model scenario output in *"…/scenarios/streams"* or *"…/scenarios/catchment"*.

Among the options for `map_years` and `map_seasons`, the `“all”` option outputs a map of the observations for each year and/or season in chronological order, whereas `“mean”` outputs a map of the average of the observations across all years and/or seasons. Note that if the `“mean”` setting is selected, an internal setting of `“all”` is forced for the output of the residual maps, given that a mean calculation on the residuals is not informative. The “NA” option causes no seasonal observations to be mapped (e.g., as for a static model). 

Selection of a summary metric option for `map_years` (i.e., `“mean”`, `“median”`, `“min”`, `“max”`) will compute a mean or other summary metric across all years for each season that is listed for the `map_seasons` control setting (e.g., `“all”`; `c(“summer”,”fall”)` ), whereas selection of a summary metric option for `map_seasons` will compute a mean or other summary metric across all seasons for each year listed for the `map_year` control setting (e.g., `“all”`; `c(2002, 2004)`). The “NA” option causes no annual observations to be mapped (e.g., as for a static model).

The `mapsPerPage` setting specifies the maximum number of maps that can be displayed on each page, with values ranging from 1 to 4. Note that fewer maps may appear per page if the `mapPageGroupBy` selection contains less available data for maps than the `mapsPerPage` selection. The "NA" option allows the default of one map per page to apply (also, as preferred for a static model). 

The `mapPageGroupBy` setting specifies the temporal organization (i.e., “grouping”) of the map displays on each page. The “year” option displays all seasonal maps on a page for each year in chronological order, whereas the “season” option displays all yearly maps on a page (or multiple pages) for each season.  The “NA” option imposes no grouping on the displays. 

Table 8 provides a detailed illustration of the types of plots that are output by different combinations of these four control settings. The settings control the format of HTML files output to the three subdirectories listed above.

As an illustration of the four display settings, consider the following example with `map_years<-c(2001,2002)` and `map_seasons<-“all”` in which 2001 has only three seasons of data and 2002 has four seasons (note that this example applies to the attributes of the dynamic tutorial model): 

* For the setting `mapsPerPage<-4`, the option of `mapPageGroupBy<-"year"` displays a three-panel map for 2001 followed by a four-panel plot for 2002. 

* The option of `mapPageGroupBy<-"season"` will display each season on a single page, with the example displaying three two-panel maps for each of the three seasons with observations for 2001 and 2002 and one one-panel map for 2002 for the remaining season that is missing data for 2001. 

* The option of `mapPageGroupBy<-NA` will display four maps chronologically per page with the last page displaying any remainder. Thus, for this setting, three seasonal maps from 2001 would be display together with one seasonal map from 2002 to form a four-panel plot; the last page would contain a three-panel plot of the remaining seasonal maps for 2002, assuming 2002 is the last year being mapped.  


&nbsp;

**Table 8. Summary of dynamic mapping display options for four control settings that manage the time periods and display format for HTML output to three subdirectories.** The heading of the first four columns shows the control setting name. See the text for a listing of the three subdirectories where HTML output is directed.

 `map_seasons`       `map_years`       `mapsPerPage`   `mapPageGroupBy`  Plot Description
-------------- ---------------------  --------------- ------------------ ------------------------------------
 "all"          "all"                            4            "year"     Four-panel maps displaying values 
                                                                         for all seasons in each year grouped 
                                                                         by “year”
                                                                         \textcolor{white}{.}
 c(2001,2002)    "all"                            2         "season"     Two-panel maps displaying values for 
                                                                         2001 and 2002 grouped by season
                                                                         \textcolor{white}{.}
 c(2001,2002)    "summer"                         4           NA         Four-panel maps displaying summer 
                                                                         values for years 2001 and 2002, maps 
                                                                         grouped in 4’s by consecutive years
                                                                         \textcolor{white}{.}
 "mean"              4                        "all"           NA         One four-panel plot displaying all 
                                                                         seasonal means, computed across all 
                                                                         years for each season
                                                                         \textcolor{white}{.}
 "mean"        c(“winter”,”spring”,             4              NA        One three-panel plot displaying the 
                  "fall")                                                seasonal means for three seasons, 
                                                                         computed across all years for each 
                                                                         season
                                                                         \textcolor{white}{.}
  "all"           "mean"                         4             NA        Four-panel maps displaying annual 
                                                                         means for each year, computed across 
                                                                         all seasons in each year; maps 
                                                                         grouped in 4’s by consecutive years
                                                                         \textcolor{white}{.}
 c(2001,2002)    "mean"                          4             NA        Two-panel map displaying annual 
                                                                         means for two years, computed across 
                                                                         all seasons in each year; maps 
                                                                         grouped by consecutive years
                                                                         \textcolor{white}{.}
  "mean"          "mean"                         1            NA         One map displaying the long-term 
                                                                         mean, computed across all years and 
                                                                         seasons
                                                                         \textcolor{white}{.}
  "all"              NA                          4            NA         Four-panel plots displaying 
                                                                         consecutive seasonal values (yearly 
                                                                         data are not output)
                                                                         \textcolor{white}{.}
   NA              "all"                         4            NA         One four-panel plot displaying all 
                                                                         yearly values (seasonal values are 
                                                                         not output)
                                                                         \textcolor{white}{.}
   NA                NA                          1             NA        Static model, no timesteps available 
                                                                         in the data
-------------- ---------------------  --------------- ------------------ ------------------------------------
...
# "yes" Automatically runs the RShiny DSS at the completion of model execution 
...
# path_shinyBrowser <- "C:/Windows/SystemApps/Microsoft.MicrosoftEdge_
#        8wekyb3d8bbwe/MicrosoftEdge.exe"
# path_shinyBrowser <- NA      # user's default browser

```
The `enable_shinyMaps<-"yes"` setting enables an RShiny interactive mapping interface within a Web browser window. The browser type is specified in the `path_shinyBrowser` control setting. From RShiny, users can map RSPARROW predictions and variables (*sparrowNames*) in the data dictionary by stream reach, catchment, and monitoring site location and interactively define and execute hypothetical source-change management scenarios (see Chapter subsection 4.4.9.2). A separate *Rscript.exe* window also opens in a black box during the RShiny session, where message output is written including warning messages. If execution errors occur, the black box will close and the application window will become inoperable and grey in color; an error message will be written to the *error.log* file. 

The RShiny mapper is not enabled if a user executes a model in batch mode (i.e., `batch_mode<-"yes"`; section 11 of the control script). Note that the RShiny mapper contains a batch-mode option to support map production for larger watersheds that require longer execution times. This option allows users to output maps of multiple variables (i.e., prediction, uncertainty, and *dataDictionary*) in a single execution.


\begin{figure}
\centering
\includegraphics[width=2.0\linewidth]{figures/FigRShinyMap.jpg}
\caption{\textbf{RShiny interactive mapper interface and example map for the static SPARROW model of total nitrogen yield for streams in the Midwest. } The leaflet map includes five optional base maps, and is shown for the tutorial model 6. The menu and map panels appear side-by-side in the actual browser display. }
\label{RShinyInteractiveMap}
\end{figure}



##### RShiny mapper menu options (static and dynamic models)

&nbsp;

The RShiny menu options will change with the user's selections, from top to bottom, with different input boxes appearing based on each upper level selection by the user. An example of the RShiny mapper interface is shown in Figure \ref{RShinyInteractiveMap} for the map of total nitrogen yield for streams in the Midwest. The menu options described in this section apply to both static and dynamic models; the subsequent section describes additional menu options that apply only to dynamic models.  

The interface provides the following options: 

(1) **Output Mode:  *"Interactive"* or *"Batch"* ** - Interactive mode allows users to map one variable at a time, with output directed to CSV files and optionally to PDF or HTML files. Batch mode is recommended for large model domains (e.g., >300K stream reaches), which may require considerable time to fully display. Batch mode allows users to select multiple variables and to execute these in the background with the map output directed to PDF or HTML files (and optionally to ESRI shape files in batch mode). In batch mode, the following windows must remain open:  (a) the RStudio session, which is unavailable for use during map production; (b) the *Rscript.exe* window running the shinyApp; and (c) the *Rscript.exe* window running the batch execution.
...
\caption{\textbf{Summary of the features and drawbacks for the three "Output Map Format" options in the RShiny interactive browser interface. } Note that only geographic (latitude-longitude) map projections are currently supported in RSPARROW.  }
...
(3) **Map Type:  *"Stream"*, *"Catchment"*, *"Site Attributes"*, or *"Management-Change Scenarios"* ** 
...
* For *"Stream"* or *"Catchment"* map types, users can select from among the following four options for *"Mapping Variable Type"*: *"Load Predictions"*, *"Yield Predictions"*, *"Prediction Uncertainties"*, *"Data Dictionary Variables"*. 
    ...
    * For the first three prediction-related options for *"Mapping Variable Type"*, users may select a *"Mapping Variable"* for any of the prediction metrics using the names shown in subsection 4.4.7.1 above; these can only be mapped for a stream or catchment *"Map Type"*. 
...
* For the *"Scenarios for Changes in Sources and/or Delivery Variables"* map type, users can display and save the results of management-change scenarios for streams or catchments based on choices for the *"Select output map type"* option (see examples of the menu options and output in Chapter subsection 6.2.9). Users have control over the selection of source variables, land-to-water delivery variables, change factors, reach locations, a name for each scenario, and time periods for the dynamic model (see Chapter subsection 4.4.9.2 below for details on the decision support methods).   
    ...
    * The setting *“Run Scenario using forecasting file input”* allows users the option of inputting an external CSV file with forecasted changes in source and/or land-to-water delivery variables. An entry of *“yes”* requests that the user enter the CSV file name in response to the prompt *“Enter forecast scenario filename”*. The file should be located in the user’s RSPARROW “…/data” directory, the most secure location to avoid the overwriting of files. A copy of the file is stored in the *“…/results/scenarios/(scenario_name)/”* directory when the scenario is executed, but may be overwritten if a scenario with the same name is subsequently executed. We recommend that user’s adopt file naming conventions that identify unique features of the scenario or changed model variables. The variables in the external scenario file should have either the *sparrowNames* or *data1UserNames* listed as the column header (a check box on the menu is used to select the name type). RSPARROW automatically performs various tasks on the data input from an external CSV forecast file to ensure consistency with features of the calibrated model and internal data files. See section 4.4.9.2.1 below for details on the creation and use of the external CSV forecast file option.

    * A response of *“no"* to the *“Run Scenario using forecasting file input”* request prompts the display of menu options that require the user to specify the geographic areas, the change percentages, and the time periods (for dynamic model) that are applied to one or more of the source and/or delivery explanatory variables in the scenario.  This entails selecting the target (downstream) reaches for the watershed, where the scenario is applied, as well as the percentage change factors that are applied to the model explanatory data and/or model coefficients. Change factors can be applied separately to the source and land-water delivery model variables, including the option to sub-select reaches using the *dataDictionary.csv* variables (for dynamic models, this includes access to the *season* and *year* variables to sub-select timesteps for execution in scenario model simulations). 
      ...
      * The setting *"Select reaches for applying the scenario (within targeted watersheds)"* allows users to select *"all reaches"* or *"selected reaches"* to apply the scenario. This enables tabular entry of the details of the change scenario attributes, including variable names and conditions for the sources and/or delivery variables (*Percent Change (+/-) Factors*), and reach locations (and timesteps for the dynamic model) in the targeted watersheds where the scenarios apply (a right click allows the addition of rows to the table to select additional sources or reach locations--see examples in Chapter subsection 6.2.9).  
      
      * The choice to apply scenarios to *"selected reaches"* provides users with two options for entering the scenario attributes via the setting *"Apply same reach selection criteria to all selected changed variables (yes/no)"*. A *"yes"* response applies the same reach (and timestep) selection to all sources and/or delivery variables using two separate tables (one for sources and delivery variables and one for the reach selection criteria). A *"no"* response allows users to apply different reach (and timestep) selection criteria and change percentages to each model variable in one table; the use of one table allows the user the greatest flexibility to change any given model variable in different ways in different areas. 
    
    * The mapping output for the scenario is controlled by three settings:
    
      * The setting *"Mapping Variable Type"* provides users with three options, including *"Load Predictions for Changed Modeled Variables"*, *"Yield Predictions for Changed Modeled Variables"*, and *"Relative Change in Load"*. The former two options display absolute measures of the stream load, yield, or concentration associated with a changed source(s) and/or land-to-water delivery variable(s), whereas the latter option displays a relative measure of the change in stream load. 
      
      * The setting *"Mapping Variable"* allows users to select absolute measures of change for prediction metrics for any of the named variables shown in subsection 4.4.7.1 above, or to select one of four options for the relative measure of change (*"ratio_total"*, *"ratio_inc"*, *"percent_total"*, *"percent_inc"*). The relative measure is expressed as the ratio of the changed model prediction of incremental or total load (with source or delivery variable changes applied) to the baseline (unchanged) incremental or total load prediction (the ratio measure can also be optionally expressed as a percentage). 
    
      * Scenario results are output to a subdirectory with the user-specified name (*"Enter scenario name"* option), located within the upper-level *scenarios* directory.


(4) **Customization settings for map displays:** - A variety of functional settings (*"Mapping Settings"*) are provided (see Fig. \ref{RShinyInteractiveMap}) to allow users to customize the map and legend background color, binning breakpoints and intervals, symbol colors, numerical rounding, and text and symbol/line sizes. The customized settings for these controls, shown in the previous sections for the non-interactive production of maps, are loaded as the default settings in the RShiny mapper.

(5) **Generate and save map output:** - The *"Generate Map"* button displays the map according to the user-defined settings. The *"Save Map"* button outputs the map according to the user-defined settings to appropriate subdirectories in the "maps" or "scenarios" directories (Figure \ref{directory1}). The output format is controlled by the *"Output Map Format"* setting, with a PDF file output for the *"static"* option (with non-interactive features) and a HTML file output (with interactive features) for the *"plotly"* and *"leaflet"* options. The *"plotly"* map option is not available for the display of catchment metrics because of the lengthy rendering time. Note that a map does not have to be generated (*"Generate Map"*) prior to saving (*"Save Map"*). 



##### RShiny mapper menu options (dynamic models only)

&nbsp;

All of the menu displays and options available for static models are also available for dynamic models. 

The DSS for dynamic models provides an additional menu display that appears following the *“Map Type”* option (see Figure \ref{RShinyInteractiveMap2}) that's labeled  *“Dynamic Mapping Display Settings”*, with *“year(s)”* and/or *“season(s)”* dropdown menu options (one or both options are displayed depending on the detected timestep of the dynamic model). The check-box dropdown menus allow users to specify the time periods or summary statistics for the variables that are mapped and displayed in the DSS window; a maximum of four maps can be displayed for the screen page (i.e., four-panel display). 

The *“year(s)”* option allows the selection of multiple map displays (using a check box) for a variable for summary metrics, including the mean, median, minimum, and maximum (computed for the calibration time period), as well as for individual years in the calibration time period.

The *“season(s)”* option allows the selection of multiple map displays (using a check box) for a variable for summary metrics (computed for the calibration time period) as well as for individual seasons in the calibration time period. 

Note that a summary metric option for *"year(s)"* (i.e., *“mean”*, *“median”*, *“min”*, *“max”*) computes a mean or other summary metric across all years for each season that is selected (up to the maximum of four seasons) for the *"season(s)"* setting.  By contrast, selection of a summary metric option for *"season(s)" computes a mean or other summary metric across all seasons for each year that is selected, up to the maximum of four years for the *"year(s)"* setting (note that batch mode allows more years to be selected). See Table 8 in Chapter subsection 4.4.8.4 for guidance and examples of the options for these settings. 

&nbsp;

**Execution of management-change scenarios**

When executing management-change scenarios (i.e., *“Map Type”* selection of *“Scenarios for Changes in Sources and/or Delivery Variables”*), options for the *“year”* and/or *“season”* settings control only the mapping display. These display settings need to be selected to be consistent with the model simulation timestep. The simulation timestep is associated with the base or reference conditions of the explanatory variable(s) during a calibration time period that are either altered (i.e., as with  percentage-change scenarios) or are used as a benchmark to facilitate comparisons with future water-quality predictions based on forecasted changes in watershed conditions (i.e., as with scenarios based on use of an external forecasting CSV file). 

The menu options as described for static models in the previous Chapter subsection 4.4.9.1.1 can be used to execute a change scenario expressed as a percentage change in the explanatory variables or input from an external forecasting CSV file. 

&nbsp;

*Percentage-change scenarios*

When the scenario is expressed as a percentage change, users should specify the *"year(s)"* and/or *"season(s)"* that are consistent with the base/reference conditions for the simulation timestep during the calibration period, which are entered as reach (and timestep) selection criteria in the section of the RShiny menu entitled *“Select Target Reach Watersheds”*. This section appears in the menu when a *“no”* response is selected for the *“Run Scenario using forecasting file input”* option.

To specify the simulation timestep (year and/or season) for the scenario, the following menu entries are required:

* Select the option *“selected reaches”* for the menu setting labeled *“Select reaches for applying the scenario (within targeted watersheds)”*. Note that selection of the option *“all reaches”* for this setting does not allow the user to enter values for the timestep. 

* Select the option *“yes”* or *“no”* for the menu setting labeled *“Apply same reach selection criteria to all selected changed variables (yes/no)”*. 

  * A *“yes”* entry displays two tables in the menu. The first table allows users to select the one or more model explanatory variables and enter the percentage quantity for application of the percentage change (table label *“Select Model Explanatory Variables and Percent Change Factors”*). The second table allows users to specify reach criteria for the scenario, including timestep entries (table label *“Reach Selection Criteria”*), which apply to all model explanatory variables (i.e., sources and/or delivery variables).  For example, to enter the timestep for a seasonal model, two rows are required, one for the year and a second for the season, with the *“AND”* separator selected for the first row.

  *	A *“no”* entry results in the display of only one table. This option provides greater flexibility, allowing users to apply different reach (and timestep) selection criteria and change percentages to each model variable. For example, to enter the timestep for a seasonal model, two rows are required, one for the year and a second for the season, with the *“AND”* separator selected for the first row. Note that the changed model variable and the change percentage must be entered on both rows for the table entries to be accepted (*this does NOT cause any replication of the application of the change percentage*). Also, note that each variable that is changed requires its own timestep specification; for example, four separate rows would be required in the tutorial seasonal model to specify a scenario with a precipitation reduction in the winter of 2014 and a precipitation increase in the summer of 2014. 

  * This menu setting also allows scenario evaluations of a percentage change in one or more model coefficients associated with explanatory variable values for a base or reference timestep, such as for scenarios where a hypothetical change in the mass loading from a land-use based source is evaluated (e.g., see example in Chapter subsection 6.2.9.4 for the static tutorial model). This is implemented by specifying *"yes"* for the *"ChangeCoefficient"* column that appears in the tabular displays. Each changed model coefficient that is evaluated in a scenario would require a timestep specification to identify the time period of the explanatory variable (e.g., land use) that is to be used as the base or reference condition for expressing the percentage change. 

&nbsp;

*Change scenarios input from an external CSV file*

In cases where the changed values of one or more explanatory variables are input from an external forecasting CSV file, the file records should be unique for both the reach ID and timestep (i.e., the *sparrowNames* *waterid* variable). The user should specify the timestep and reach ID *a priori* as part of the process of manually creating the forecasting file. Therefore, the *"year(s)"* and/or *"season(s)"* settings selected for the DSS map display should be consistent with these timesteps to properly display the results of the simulation of changed model predictions. 

See the Chapter section 4.4.9.2.1 below and the Chapter subsection 7.2.2.2 (dynamic tutorial model example) for details on the creation and use of the external forecasting CSV file option. Chapter section 4.4.9.2.1 also gives details on the various tasks that are performed automatically by RSPARROW on the data input from an external CSV forecast file to ensure consistency with features of the calibrated model and internal data files. 


\begin{figure}
\centering
\includegraphics[width=1.2\linewidth]{figures/FigRShinyMap2.jpg}
\caption{\textbf{RShiny interactive mapper interface and example map for the dynamic SPARROW model of total phosphorus yield for streams in the Tampa Bay watershed. } The leaflet map includes five optional base maps. The tutorial total phosphorus model is described in Chapter 7. The menu and map panels appear side-by-side in the actual browser display. }
\label{RShinyInteractiveMap2}
\end{figure}
...
To enable the RShiny mapper for a previously executed model within an existing or new RStudio session, users should run the following statements in the RStudio Console window. The `runBatchShiny` control setting will enable the mapper for the results of a prior model in a specified browser (e.g., Chrome), according to the full path name that is listed in the function argument, as illustrated below. 
...
#   executed model (e.g., model 6) by running the following function in the Console window:
runBatchShiny(path_PastResults = "C:/UserTutorial/results/Model6", path_shinyBrowser = 
...
#### Simulation of management-change scenarios in the DSS mapper
...
The control settings in this section provide a decision-support tool for executing user-defined change scenarios. These include simulations of the effects on stream water quality of natural or managed changes in the values of one or more explanatory variables in the model. The changes may be hypothetical (e.g., a uniform percentage change) or may reflect model-based forecasts of future watershed conditions (e.g., land use, climate).  The results of the scenarios can be output in tabular (CSV files) and mapped forms (HTML). Note that the scenario settings are only applicable to RSPARROW models for which model estimation and predictions have been previously executed (see `if_estimate` and `if_predict` control settings).

The DSS tool allows users to evaluate the effects of changes (increases or decreases) in source inputs, expressed in units of *mass* (e.g., fertilizer use) or *area* (e.g., land use/cover), as well as changes in land-to-water delivery variables on the water-quality loads and concentrations in downstream reaches. For scenarios with area-based land-use sources, users have the option to convert the land area from one land-use type to another, or users can change the land-use source loading per unit area through changes in the mean estimates of the land-use model coefficients (this latter option can only be executed in the RShiny interactive mapping interface; see description in subsection 4.4.9.2).

Two modes are available to specify changes in land-water delivery variables and/or source variables that are evaluated in a scenario:  (1) **“change-percentage scenarios”**—menu entries allow users to specify the magnitude of a hypothetical percentage change in the values of one or more explanatory variables or model coefficients; and (2) **“forecasting file change scenarios”**—users input an external CSV file to the DSS, which contains new reach-level values of one or more of the model explanatory variables that reflect hypothetical or model-forecasted changes in watershed conditions (this mode should be used to evaluate changes in transformed land-to-water delivery variables—see discussion in section 4.4.9.2.1 below). 

**Note that the scenarios input from an external forecast file can only be executed for dynamic models currently.** However, a workaround method is available for static models. See section 4.4.9.2.1 for details.

As we noted in section 1.2, users are cautioned that the lack of contaminant storage components in the statistical dynamic model limits the interpretability of the outcomes of change scenarios, which do not precisely quantify the effects of changed model variables on the timing of future changes in stream water quality. A more physically based representation of storage processes in the dynamic model, such as lagged sources or groundwater discharge with contaminant transit times, would be necessary (and is being considered for future RSPARROW versions) to provide a more realistic forecast of future changes in stream water quality in response to managed or natural changes in watershed sources, land use, or climate.

Although users can execute change scenarios using the control script settings described in this subsection, we recommend that users execute scenarios in the RShiny interactive mapping interface (activated by the `enable_shinyMaps<-"yes"`) because:  (1) it provides considerable flexibility and is easier to use than the control script settings, and (2) the *"leaflet"* mapping option (with standardized, globally available base maps) is only supported in the RShiny mapper. Users should consult Chapter subsections 6.2.9 (static model) and 7.2.2.2 (dynamic model) for scenario tutorials that illustrate the use of the RShiny mapper.

Nevertheless, users of the RShiny mapper should also consult the sections below to obtain a general understanding of the methods for identifying watersheds and reaches for scenarios, the limitations of scenarios with land-use sources, and the output produced for scenario results. 


&nbsp;


##### Use of an external CSV file to identify locations and conditions for applying scenarios 
...
# Specify if sparrowNames (TRUE) or data1UserNames (FALSE) are used in the 
#   forecast CSV file
# Options: 
...
```
Users have the option to specify management-change scenario conditions for reaches and timesteps (dynamic model), including sources and/or land-to-water delivery factors, in an external CSV file, with the name entered using the `forecast_filename` setting. 

**The external CSV file option should be used for scenarios involving changes in land-to-water delivery variables that are expressed as transformed variables in the calibrated model.** The alternative use of the `scenarios_factor` control setting or **“Percent Change Factors”** in the RShiny DSS to evaluate a hypothetical percentage change in a transformed delivery variable are potentially problematic. A percentage change in a transformed variable (e.g., logarithmic) implicitly assumes a disproportionate change in the original units of the variable relative to the specified percentage; this can result in erroneous interpretations of the magnitudes of the simulated changes in stream water-quality loads. 

**Note that the scenarios input from an external forecast file can only be executed for dynamic models currently.** However, a workaround method is available for static models and is explained in this issue: [Unable to run a forecast_file scenario on a static model (#6)](https://code.usgs.gov/water/stats/rsparrow/-/issues/6) · Issues · Water / stats / RSPARROW · GitLab The method entails use of the procedures described in section 4.4.9.2.4. The method must be executed from the control file command lines, as scenario inputs from an external forecast file in the DSS Rshiny mapper is not currently allowed for static models.

The CSV file should be located in the user’s RSPARROW *“…/data”* directory, the most secure location to avoid the overwriting of files. A copy of the file is stored in the *“…/results/scenarios/(scenario_name)/”* directory when the scenario is executed, but may be overwritten if a scenario with the same name is subsequently executed. We recommend that user’s adopt file naming conventions that identify unique features of the scenario or changed model variables. 

The variables in the external CSV file should have either the *sparrowNames* or *data1UserNames* listed as the column headers, with the corresponding user selection entered for the `use_sparrowNames` control setting (i.e., TRUE or FALSE).  Both the names and the value units of the explanatory variables in the external CSV file must be identical to those of the variables that are specified in the *dataDictionary.csv* for the calibrated model.

**RSPARROW automatically executes the following tasks on the data input from an external CSV forecast file** (as executed by the *readForecast.R* routine):  

* Reaches with missing values (`“NA”`) are assigned the original value of the explanatory variable for the *waterid*, such that a “no change” condition will be assumed and shown in map displays and output files; 

* Values of the land-to-water delivery factor variables are mean-adjusted if this adjustment procedure was used in the calibrated model (i.e., `if_mean_adjust_delivery_vars <- "yes"`);

* The contents (i.e., records) of the external CSV file are internally sorted according to the *waterid* variable to ensure proper matching with the existing data calibration and prediction records;

* Variables in the external CSV file that are not used in a calibrated model (as specified in the *parameters.csv* file) are ignored (thus, these variables can be present in the CSV file without causing any execution errors). 

**Dynamic model settings**

For dynamic models, we recommend that the name of the external CSV file should identify the time (e.g., short- or long-term forecast) or other relevant information associated with the timestep of the changed base/reference variable. 

Also, the display of the mapped change-scenario results in the HTML *plotly* output files (or RShiny DSS map display) requires users to select a timestep ("year" and/or "season") during the model calibration period, which can be used as a base or reference period condition to compare with changed future conditions and their associated predictions of water quality. The timestep of this reference period must be specified in a consistent manner for both the map display settings and the external CSV forecasting file (with changed explanatory variables) that is used for the scenario model simulations:  

* The records of the external CSV forecasting file should contain values of the *waterid* index variable that are associated with the calibration period timestep for the base or reference time period relevant to the scenario. 

* The *"year"* and/or *"season"* settings selected for the DSS map display **“Dynamic Mapping Display Settings”** (or control script timestep display settings `map_years` and `map_seasons`) should be consistent with the timestep included in the *waterid* index variable in the forecasting CSV file. This will ensure that the map display shows a year and/or season that is consistent with the scenario simulation results. If using the control script timestep settings `map_years` and `map_seasons`, these must be specified along with the page formatting settings `mapsPerPage` and `mapPageGroupBy` (see Chapter section 4.4.8.4; section 8 of the sparrow control file).  Note that additional year(s) and/or season(s) may also be selected for display in the HTML map output (or RShiny DSS display) along with the change in the base/reference conditions if the user wants to compare other calibration time period results with those for the change scenario simulations of model predictions in the same map display.

For additional details on use of the external CSV forecasting file, see illustrations of the change scenarios for the dynamic tutorial model in Chapter subsection 7.2.2.2).
...
# Indicate the watershed locations where the scenarios will be applied to either 
#    "all reaches" or "selected reaches"
# Options: 
# NA executes the scenarios for all non-terminal reaches
# [waterid] executes scenarios for a single watershed defined by and including 
#           the outlet [waterid] reach
...
* Alternatively, users may apply the management-change scenarios to a single or multiple watersheds by listing the watershed outlet reach identifier (i.e., value of the *waterid* system variable). RSPARROW automatically defines the watershed reaches, inclusive of the outlet reach and all upstream hydrologically connected reaches. 
     ...
     * The RShiny interactive mode allows users to optionally identify watershed outlet reaches in the *flag_targetReachWatersheds.csv* file and input these reaches to scenarios. Users enter a "1" in the column named *Flag* either directly from the RShiny session or prior to enabling an RShiny session. This method is preferred in cases where users have a large number of outlet reach locations. A copy of the edited file is saved to the user's `scenario_name` subdirectory.  
     

##### Set scenario source and/or delivery variable conditions for "all reaches" in user-defined watersheds 
...
# NOTE: The sources must be expressed in mass units (e.g. fertilizer) or areal units
#     (e.g. land use or land cover)
scenario_sources <- c("logAirTemp", "logPPT")
...
# Source/delivery variable adjustment factors (increase or decrease) applied to 
#   "all reaches" in the user-specified watersheds. Enter a factor of 0.1 or 1.1 to 
#   obtain a 10% reduction or increase in a source, respectively. Order must be 
#   consistent with the 'scenario_sources'.
scenario_factors <- c(0.20, 0.10)
    
```
Management-change scenarios applied to all reaches (`select_scenarioReachAreas<-"all reaches"`) in user-specified watersheds require control settings to define the sources and/or delivery variables and the magnitude of change in the explanatory variables for the scenario.

The `scenario_sources` setting identifies one or more source and/or delivery variables in the SPARROW model that are to be evaluated in the management-change scenario. The sources can be expressed in mass units (e.g., fertilizer use) or areal units (e.g., land use/cover). Note that the `scenario_sources` setting is also applicable for the "selected reaches" option described in the next subsection 4.4.9.3. 
...
##### Set scenario source and/or delivery variable conditions for "selected reaches" in user-defined watersheds 
...
Users are free to devise more elaborate R statements to create scenarios that include a more diverse range of sources and/or delivery variable, change factors, and land conversion types. These could, for example, allow different change factors to be applied across different reaches. Alternatively, execution of management-change scenarios with the RShiny option provides considerable flexibility and ease of use. 
...
The `scenario_name` setting allows users to specify a name for each evaluated scenario. This creates a separate parallel subdirectory in the "(run_id)/scenarios" directory for each named scenario (see Fig. 1 in Chapter 1). 
...
Three RSPARROW control settings use the *iseed* value to initiate the following random selection methods:
  ...
  * `if_validate <- "yes"` and `pvalidate` > 0 - Enables the random selection of calibration and validation sites, using the R random sample function `sample`, with the no sample replacement option (`replace==FALSE`). See subsection 4.4.6 for details on the selection of validation sites. 
...
#   Outputs the model coefficients and bias-retransformation correction factors 
#   for model residuals of each bootstrap iteration.
# Options: 
...
#   Outputs reach-level bias-corrected mean predictions and their uncertainties for 
#   mass-related metrics (e.g. total load), including source-share related mass metrics.
# Note: Requires bootstrap model parameters (if_boot_estimate) to be executed, based on
#   current or previous RStudio session
# Options: 
...
* Bias-corrected mean - The bootstrap proportional bias-transformation corrected mean is computed as the ratio of the squared NLLS mean prediction (inclusive of the mean exponentiated error correction) and the average of the *biters* mean bootstrap predictions (inclusive of the mean exponentiated error correction for each of the *biters* iterations), as described in equation 1.138 (Schwarz et al., 2006). Note that the estimates of the bootstrap proportional mean are bias corrected individually for each mass-based prediction metric and reach location, without any restrictions on the network-wide mass balance. Thus, the mean estimates do not retain a spatial mass balance within the river network. 
    ...
    * For analyses of model predictions that require mass balance restrictions, the standard or non-bootstrap estimates for the unconditioned predictions should be used (Chapter subsection 4.4.7), based on use of the control setting `if_predict` as described in section 7 of the control script (output is directed to the *(run\_id)\_predicts_load.csv* and *(run\_id)\_predicts_yield.csv* files). The non-bootstrap unconditioned predictions of load and yield receive a single, network-wide estimate of the bias-correction factor (mean exponentiated model error), thereby conforming with the mass balance constraints.   
...
In RSPARROW, the conditional methods are used to calculate the prediction variables for load (*mean_mpload_total*, *mean_mpload_(sources)*) and yield (*mean_myield_total*, *mean_myield_(sources)*) as described in subsection 5.2.2.1. 
...
```{r, eval=FALSE}
#-------------------------------------------------------------------------------
# Add relative or absolute path name to the directory containing all RSPARROW source code
# NOTE: If there are spaces in the file path to the source code then errors may occur
  path_master <- "./RSPARROW_master"
...
Users should specify names for the three required subdirectories in the User Directory (no spaces are allowed!); the names shown above are examples and are not required. The subdirectories must be located parallel to one another (see Figure \ref{directory1}). Multiple "results" directories and/or other non-RSPARROW directories can also exist parallel to the required three directories. 

Executing the *sparrow_control.R* script in RStudio automatically determines the location (path name) of the "results" directory for the model, which is internally stored in R and will allow the creation of subdirectories for saving model output files. 
...
* Overwrite the active control script (and all input control files) in the "User Directory/results" subdirectory with the prior versions of these files. Before the files are overwritten, the user will be asked to close all control files in the results directory, including the sparrow_control.R script (but not RStudio). This will allow these files to be overwritten. Failing to close the *sparrow_control.R* file will cause an error in the `copy_PriorModelFiles` functionality, which could result in a mixture of old and new control files in the results directory. Note that the user's path name specified for the `path_master` setting in the active control script will be retained and is not overwritten.
...
* Users should set `if_estimate<-"no"` (and `if_estimate_simulation<-"no"`) to use prior estimated (or simulated) model results to generate new predictions, maps, and management scenarios, or to enable the RShiny interactive mapper. Thus, no model re-estimation is necessary in this case. 
...
# Options
# c([run_id], ...) to compare current model to previously run models found in 
...
```{r, eval=FALSE}
#-------------------------------------------------------------------------------
# Enable RSPARROW error handling. This allows for custom messaging of RSPARROW errors 
#   printed to the console, and when 'batch_mode' is "yes" the messages will be saved 
#   to an error log on the users system.
# NOTE: When "no", the user can view a full error traceback by executing 'traceback(2)' 
#   in the console. However, errors that occur when 'batch_mode' is "yes" will not be 
#   output or saved, making traceback impossible. When "yes" and an error occurs, to 
#   restore original user error options, execute 'options(backupOptions)' in the console.
...
```{r, eval=FALSE}
#-------------------------------------------------------------------------------
# WARNING: It is highly recommended the code below remain unedited. Use caution if 
#          making changes.

# Install required packages
# NOTE: This is a one time process unless a new version of R is installed or more 
#   recent packages are found on CRAN. Packages previously installed by user will 
#   be skipped.
...
The initial execution of the RSPARROW control script (`devtools::install_deps` function) automatically installs the most recent versions of the required RSPARROW library packages and functions in the user’s default R library as specified in RStudio. In subsequent executions of the control script, no updates are made to the RSPARROW library dependencies (i.e., `upgrade="never"`). 

RSPARROW 2.1 was tested with R-4.4.2 for compatibility with versions of the required R library functions available at the time of public release that are listed in the DESCRIPTION file. RSPARROW may operate with more current versions of the libraries, but full compatibility of RSPARROW with newer versions is not guaranteed.

When opening and executing the RSPARROW control script in RStudio, users should be attentive to messages and prompts associated with the R package installations. See Chapter subsection 1.3.5 for details on messages and prompts associated with the installation and updating of the RSPARROW library dependencies. 
...
**3. Use the instructions in Chapter 4.2 to guide the setup of the control script, data Dictionary, and parameter control input files for the model.** The instructions include a detailed checklist in subsection 4.2.4 for the setup and testing of the system settings and new models. This includes instructions for using the control script in an RStudio session to set path names, verify the RSPARROW library installation, set data import options, setup mapping shape files, verify the river reach connectivity, and establish control settings for model execution, prediction, and mapping. 

**4. To execute the model in RSPARROW in an RStudio session (step 6 in the checklist in subsection 4.2.4), the following settings and data statements should be applied in the *sparrow_control.R* script:**  
...
* To evaluate alternative model specifications with different explanatory variables, consult subsection 4.4.4 for guidance on model estimation in RSPARROW. 
...
This binary file is a modified version of the *data1.csv*, with the contents defined by the *sparrowNames* variables in the *dataDictionary.csv* file (see Chapter subsection 3.2 for a detailed description). 

Modification of the number of reach records of the *data1.csv* is on based on user-defined filters, using the control setting `filter_data1_conditions` (section 2 of the control script). The contents of the *sparrowNames* variables is subject to modification by user-defined R statements in the *userModifyData.R* script (see subsection 3.5).
...
For reaches where the RSPARROW newly-derived total drainage area differs from the user-supplied *demtarea* by more than 1 percent, a CSV file is output with selected reach attribute information; the output of a HTML file with mapped reach attributes is optional. 
...
The control setting `if_verify_demtarea_maps<-"yes"` produces the plots and maps for the following reach attributes:
...
Note that the following message will appear in the RStudio Console: "Running Check Drainage Area Errors Maps in batch mode." Batch mode execution (*batchMaps_checkDrain.R* in the "RSPARROW_master/batch" directory) significantly decreases the processing time of stream maps for large watersheds and very detailed stream networks. RSPARROW execution will not proceed until all network connectivity output is complete.

A *"plotly"* interactive plot is automatically output in the browser-accessible HTML file. The `enable_plotlyMaps<-"yes"` setting (see section 8 of the *sparrow_control.R* script and Chapter subsection 4.4.8.4) produces interactive maps. **Note that the setting `enable_plotlyMaps<-"yes"` produces HTML files that are very large in size; users should expect some delays when opening the HTML file content in a browser.** The setting `enable_plotlyMaps<-"no"` outputs non-interactive maps in the HTML file.  
...
*Network connectivity output for dynamic models*

For dynamic models, users have the option to produce seasonal and/or annual subsets of the various maps described above. The maps are output to three separate HTML files, with self explanatory labels in the file name, in the subdirectory *".../estimate/diagnostic_darea_mismatches_maps"*. 

Five control script settings (section 8) govern the format of the output of annual and/or seasonal maps:  `map_years`, `map_seasons`, `mapsPerPage`, `mapPageGroupBy`, and `diagnosticPlots_timestep`. See Chapter subsection 4.4.8.4 for a detailed description of the use of the control settings to create plots and maps by season and/or year. 
...
The object contains variables associated with the user-defined selection of parameters for their SPARROW model. The object is created by the *selectParmValues.R* function. The source of the information is the *parameters.csv* file (see Chapter subsection 3.3 and Table 5 for details on the variables in the CSV file). 
...
The files described below in eight subsections are output when a model is estimated (control setting `if_estimate<-"yes"`) or executed in simulation model (`if_estimate_simulation<-"yes"`). 
...
* Subsection 5.2.2.6 describes a prediction summary that additionally requires use of the setting `if_predict<-"yes"`. 

* Subsection 5.2.2.7 describes graphical output for validation sites when the `if_validate<-"yes"` control setting is used. 

* Subsection 5.2.2.8 summarizes the R binary output files that archive the model estimation results and diagnostic metrics, and allow subsequent use of the estimation results for prediction and mapping without having to re-estimate the model. These also have utility for experienced R users and developers. 
...
#-------------------------------------------------------------------------------
  ...
  Maximum number of calibration stations = 23
                                       
  Calibration time period: 2001 to 2014
  
```
For dynamic SPARROW models, the above summary information is provided for the calibration stations and time period.  
...
```                                              
The model performance metrics as shown above are reported separately for the conditioned (monitoring-adjusted) and unconditioned (simulated) predictions (See Chapter subsection 4.4.7.1 for a discussion of the attributes of conditioned and unconditioned predictions, and see subsection 4.4.4.5 for a discussion of the performance metrics). 
...
#-------------------------------------------------------------------------------
  ...
  MODEL ESTIMATION PERFORMANCE (Monitoring-Adjusted Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
  296 80.86076 0.9171404 0.8397367     7.732644
                                                      
  MODEL SIMULATION PERFORMANCE (Simulated Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
  296 151.9881 0.8442549 0.6987648     32.56892
        
  SPRING
                                                                
  MODEL ESTIMATION PERFORMANCE (Monitoring-Adjusted Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
  314 109.4931 0.8668372 0.7357388     2.189439
                                                      
  MODEL SIMULATION PERFORMANCE (Simulated Predictions)
 MOBS     SSE       RSQ RSQ-YIELD PERCENT BIAS
  314 193.015 0.7652599  0.534159     12.21149
        
  SUMMER
                                                                
  MODEL ESTIMATION PERFORMANCE (Monitoring-Adjusted Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
  314 70.71842 0.9060209 0.7560975      7.89852
                                                      
  MODEL SIMULATION PERFORMANCE (Simulated Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
  314 123.8388 0.8354282 0.5728894     23.07624
      
  FALL
                                                                
  MODEL ESTIMATION PERFORMANCE (Monitoring-Adjusted Predictions)
 MOBS     SSE      RSQ RSQ-YIELD PERCENT BIAS
  296 98.4571 0.904984 0.7868273     6.438845
                                                      
  MODEL SIMULATION PERFORMANCE (Simulated Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
  296 203.6626 0.8034555 0.5590436      21.3355
 
```
For dynamic seasonal SPARROW models, the model performance summary metrics as shown above are reported separately for the conditioned (monitoring-adjusted) and unconditioned (simulated) predictions for each season.
...
**CooksDpvalue** = the p-value associated with the Cook's D statistic, computed from an F distribution with degrees of freedom of the number of model parameters plus 1 and the number of observations minus the number of model parameters (Helsel and Hirsch, 2002). 
...
**year** = Numeric identifier for the year time period (dynamic SPARROW models only).

**season** = Character identifier of the season time step (‘winter’,’spring’,’summer’,’fall) (dynamic SPARROW models only).
...
**pPBisas** = Percent Bias, computed as the ratio of the sum of the model residuals (observed load - unconditioned predicted load) to the sum of the observed load across all calibration sites, and multiplied by 100 (see Moriasi et al., 2007).


#### (run_id)_ANOVAdynamic.list (R binary object) (dynamic models only)

&nbsp;

The object is output for estimated seasonal and annual dynamic models (`if_estimate <- "yes"`), and contains the following model performance-related variables for the calibration sites:

**dynsites** = Total number monitoring calibration sites. 

**dynyearmin** = Minimum year available in the calibration record.

**dynyearmax** = Maximum year available in the calibration record. 

**cyear** = Numeric year identifier for each of the calibration observations.

The following variables are reported for seasonal models only:

**cseason** = Character season identifier for each of the calibration observations. 

**smobs** =  Total number of seasonal observations for each season used in the model calibration. 

**sSSE** = Sum of Squares of Error, calculated as the sum of the squares of the weighted residuals for each season.

**sRSQ** = R-Squared, computed as the ratio of the sum of the model residuals to the total variance in the log transformed observed loads for each season.

**sRSQ_YLD** = Yield R-Squared for each season, expressed as the RSQ adjusted for the mean log drainage area. Much of the variation in load is associated with drainage area size, which is typically highly correlated with the contaminant sources variables. Thus, high RSQ values do not necessarily indicate the explanatory strength of the model. RSQ-YIELD provides a scale-independent measure of model fit that provides an improved measure of the process interpretability of the model.

**sPBias** = Percent Bias for each season, computed as the ratio of the sum of the model residuals (observed load - predicted load) to the sum of the observed load across all calibration sites, and multiplied by 100 (see Moriasi et al., 2007). Positive values indicate prediction underestimation bias, whereas negative values indicate prediction overestimation bias.

**spSSE** = Sum of Squares of Error for each season, calculated as the sum of the squares of the weighted residuals, based on use of the unconditioned predictions.

**spRSQ** = R-Squared for each season, computed as the ratio of the sum of the model residuals (based on the unconditioned predictions) to the total variance in the log transformed observed loads.

**spRSQ_YLD** = Yield R-Squared for each season, expressed as the *pRSQ* adjusted for the mean log drainage area. 

**spPBisas** = Percent Bias for each season, computed as the ratio of the sum of the model residuals (observed load - unconditioned predicted load) to the sum of the observed load across all calibration sites, and multiplied by 100 (see Moriasi et al., 2007).
...
The metrics for evaluating model performance are reported according to four topical areas as described in this subsection:  

1. Maps of optional user-selected attributes for the calibration sites

2. Diagnostic plots of model estimation performance metrics

3. Diagnostic plots of model simulation performance metrics

4. Diagnostic maps of model residuals and observed to predicted ratios for the calibration sites

The model performance diagnostics are reported separately based on the use of conditioned and unconditioned model predictions (see Chapter subsection 4.4.7.1 for full explanation). These provide two informative perspectives on model performance for *estimation* and *simulation*.
...
Note that *plotly* **interactive plots** are automatically output in the browser-accessible model diagnostics file, *(run\_id)\_Diagnostic_Plots.html* in the "…/(run_id)/estimate" subdirectory. 

Users can optionally output *plotly* **interactive maps** of the station attributes and model residuals (section 4.4.8.3) by selecting the `enable_plotlyMaps<-"yes"` setting (see section 8 of the *sparrow_control.R* script and Chapter subsection 4.4.8.4 for details). The maps are output to the HTML files *(run\_id)\_Calibration_Site_Attribute_Maps.html*, *(run\_id)\_Diagnostics_Residuals_Maps.html* in the subdirectories  "…/(run_id)/estimate/mapSiteAttributes" and "…/(run_id)/estimate/residualMaps", respectively. Alternatively, the setting `enable_plotlyMaps<-"no"` outputs non-interactive maps in the model diagnostics file, *(run\_id)\_Diagnostic_Plots.html* in the "…/(run_id)/estimate" subdirectory. 

**Note that the setting `enable_plotlyMaps<-"yes"` produces HTML files that are very large in size; users should expect some delays when opening the HTML file content in a browser.**  

Each interactive plot and map displays an R *"plotly"* tool box banner, located in the upper right portion of each plot or map (e.g., Fig. \ref{PlotlyInteractiveMap1}), that allows users to interactively control various features of each image. The interactive tool box provides users with a standard set of control options for the display image, including the ability to pan/zoom, box select areas of the image, rescale plot axes, sub-select features from the map legend for display, download 'png' files, and to display the data associated with plot and map features by hovering the mouse cursor over these features (i.e., "hover-text"). The "hover-text" feature allows users to display the data associated with individual data points in plots and the monitoring sites and stream segments in maps. 
...
Maps of user-selected site attributes, as shown in the `map_siteAttributes.list` control setting (see section 8 of the control script; section 4.4.8.3.1 above) are output in one of two possible HTML files according to the `enable_plotlyMaps` setting: 

* `enable_plotlyMaps<-"yes"` - creates *plotly* interactive maps in the file *(run\_id)\_Calibration_Site_ Attribute_Maps.html* in the "…/(run_id)/estimate/mapSiteAttributes" subdirectory.

* `enable_plotlyMaps<-"no"` - creates non-interactive maps in the file *(run\_id)\_Diagnostic_Plots.html* in the "…/(run_id)/estimate" subdirectory.

The map for each user-selected site attribute is output to a separate tab in the HTML file.
...
The following plots are output to the *(run\_id)\_Diagnostic_Plots.html* file (organized by section number labels 2.1 to 2.8 in the file):

1. Four-plot panel:  observed vs. predicted loads, observed vs. predicted yields, log residuals vs. predicted loads, and log residuals vs. predicted yields. These plots can be useful to identify outliers, prediction biases, and cases where the model residuals are not identically distributed (i.e., heteroscedastic).
   
2. Four-plot panel:  boxplot of log residuals, boxplot of observed to predicted ratios, normal quantile plot of the standardized residuals, and plot of squared residuals vs. predicted loads. These plots can be used to identify cases where the model residuals are not identically distributed (i.e., heteroscedastic).
   
3. Plot of conditioned (monitoring-adjusted) load predictions vs. unconditioned (simulated) load predictions. The plot provides complementary information about the model accuracy, related to the performance of the model in *simulation* mode when monitoring conditioning effects are removed (see Chapter subsection 4.4.4.5 on measures of model performance). The plot can be used to identify differences in conditioned and unconditioned predictions along the prediction gradient but especially in larger streams where monitoring adjustment may improve prediction accuracy. Cases where the unconditioned predictions display systematic differences from the conditioned predictions may indicate prediction biases and limitations in the accuracy of the model when applied in simulation mode independently of monitoring load adjustments. Evidence of general agreement in the conditioned and unconditioned predictions, with relatively uniform scatter along the line of equivalence, indicates that the simulated predictions are generally unbiased in relation to the conditioned predictions. 
...
Using the unconditioned predictions, the plots described in the above subsection (5.2.4.10.2) are replicated and output to section 3 of the *(run\_id)\_Diagnostic_Plots.html* file (NOTE that the third plot described above for conditioned vs. unconditioned loads is not reported for the simulation diagnostic plots).  
...
The maps are output in one of two possible HTML files according to the `enable_plotlyMaps` setting: 

* `enable_plotlyMaps<-"yes"` - creates *plotly* interactive maps in the file *(run\_id)\_Diagnostics_Residuals_ Maps.html* in the directory "…/(run_id)/estimate/residualMaps".

* `enable_plotlyMaps<-"no"` - creates non-interactive maps in the file *(run\_id)\_Diagnostic_Plots.html* in the "…/(run_id)/estimate" subdirectory.

The interactive and non-interactive maps are reported for both the conditioned (*estimation*) and unconditioned (*simulation*) predictions for the following five metrics (organized by section number labels 4.1 to 4.5 in the file):

1. Model estimation log residuals, based on monitoring conditioned predictions 
   
2. Model simulation log residuals, based on the unconditioned predictions 
   
3. Model estimation standardized residuals based on the conditioned predictions
   
4. Model estimation ratios of observed to predicted loads for the conditioned predictions 
   
5. Model simulation ratios of observed to predicted load for the unconditioned predictions 

Three maps are reported for each metric using tab features with the labels "threshold-above", "threshold-below", and "all". The "all" tab displays the metric for all calibration sites, whereas the "threshold" tabs allow the user to display the metric only for calibration sites above and below the midpoint of the user-defined breakpoints, based on the control settings `residual_map_breakpoints` and `ratio_map_breakpoints` as specified in section 8 of the control script. 


##### Model diagnostic output for dynamic models

&nbsp;

For dynamic models, diagnostic plots are output for all observations to the *(run\_id)\_Diagnostic_Plots.html* file, identical to the plots that are output for static models as described in the subsections above. Additionally, users have the option to produce seasonal or annual subsets of these diagnostic plots, according to the `diagnosticPlots_timestep` control setting (see Chapter subsection 4.4.8.3.2). The selection of *"season"* or *"year"* for the control setting outputs separate HTML files in the subdirectory *".../estimate/diagnosticPlotsNLLS_dynamic"*. The filenames have labels corresponding to the type of diagnostic plot, with the contents of each file arranged chronologically by season or year. 

Site attribute and residual maps can also be optionally output for seasonal and/or annual time periods using the control settings described in Chapter subsection 4.4.8.4 (i.e., `map_years`, `map_seasons`, `mapsPerPage`, `mapPageGroupBy`). Site attribute maps are output to a single HTML file *(run\_id)\_Calibration_Site_Attribute_ Maps.html* in the subdirectory *"/diagnosticPlotsNLLS_dynamic/mapSiteAttributes"*, organized using yearly-denoted tabs consisting of four-panel seasonal map displays. Residual maps are output to separate HTML files in the subdirectory *"/diagnosticPlotsNLLS_dynamic/residualMaps"*, with the filename labels corresponding to the type of residual metric; within each HTML file, the residual metric maps are organized using yearly-denoted tabs consisting of four-panel seasonal map displays.  

Additionally, users have the option to output time series plots for the calibration stations. These are governed by the control script setting `diagnostic_timeSeriesPlots` (Options: “yes”, “no”)--see Section 8 of the control script. The plots are output to the file *Obs_v_Pred_TimeSeriesPlots.html*, located in the *".../estimate/diagnosticPlotsNLLS_dynamic"* subdirectory. 

See Table 2 in Chapter subsection 2.3 for a complete listing of the HTML files for dynamic models. See the prior Chapter subsections for static models for a complete explanation of the metrics displayed in the plots and maps. 
...
**Additional optional sensitivity output for dynamic models**

For dynamic models, sensitivity plots are output for all observations to the *(run\_id)\_diagnostic_sensitivity.html* file, identical to the plots that are output for static models as described in the subsection above. Additionally, users have the option to produce seasonal or annual subsets of these plots, according to the `diagnosticPlots_timestep` control setting (see Chapter subsection 4.4.8.3.2). The selection of *"season"* or *"year"* for the control setting outputs separate HTML files in the subdirectory *".../estimate/diagnostic_sensitivity_dynamic"*. The filenames have labels corresponding to the type of sensitivity plot, with the contents of each file arranged chronologically by season or year. See Table 2 in Chapter subsection 2.3 for a complete listing of the HTML files for dynamic models.
...
In cases where the control setting `if_validate <- "yes"` is specified, the model performance diagnostics for the validation sites are reported in two major sections.
 ...
 (a) Model simulation diagnostics are reported for the unconditioned (simulated) predictions, and include the following plots output to the *(run\_id)\_Validation_Plots.html* file (organized by section number labels 1.1 to 1.7 in the file):

1. Four-plot panel:  observed vs. predicted loads, observed vs. predicted yields, log residuals vs. predicted loads, and log residuals vs. predicted yields. These plots can be useful to identify outliers, prediction biases, and cases where the model residuals are not identically distributed (i.e., heteroscedastic).
   
2. Four-plot panel:  boxplot of log residuals, boxplot of observed to predicted ratios, normal quantile plot of the standardized residuals, and plot of squared residuals vs. predicted loads. These plots can be used to identify cases where the model residuals are not identically distributed (i.e., heteroscedastic). 

3. Plots of the observed to predicted ratio vs. the area-weighted mean values of the user-selected explanatory variables for the incremental areas between calibration sites. The plots provide visual information about the accuracy of the model specification for each of the explanatory variables; in general, no correlation should be visible between the ratios and the mean values of the explanatory variable if the model is properly specified.  The plots are output only if the control setting `if_corrExplanVars<-"yes"` (section 5 of the control script) is selected and only for the modeled variables where a value of "1" is entered in the *parmCorrGroup* labeled column in the *parameters.csv* file.

4. Boxplots of the ratio of the observed to predicted loads vs. the decile classes of the total drainage area for the sites. The boxplots provide a measure of the general bias (symmetry of the interquartile range about a value of one) and precision (length of the interquartile range) of the model predictions across different sized drainages. 
   
5. Boxplots of the ratio of the observed to predicted loads vs. the contiguous spatial classes specified by users in the control setting `classvar` (e.g., HUC-4). The boxplots provide a measure of the general bias (symmetry of the interquartile range about a value of one) and precision (length of the interquartile range) of the model predictions across sub-regions of the modeled spatial domain. The number of plots (pages) is equal to number of user-specified variables. 
   
6. Boxplots of the ratio of the observed to predicted loads vs. the deciles of the land-use class variable specified by users in the control setting `class_landuse`, with the land-use classes expressed as a percentage of the incremental drainage area extending from the calibration site to the nearest upstream site locations. The boxplots provide a measure of the general bias (symmetry of the interquartile range about a value of one) and precision (length of the interquartile range) of the model predictions across different land-cover/use types.
   
7. Four-plot panels are reported separately for each of the contiguous spatial classes specified for the first element of the control setting `classvar`:  observed vs. predicted loads, observed vs. predicted yields, log residuals vs. predicted loads, and log residuals vs. predicted yields. The panels provide a more detailed look at the model residuals in relation to loads and yields across sub-regions of the modeled spatial domain, according to the user-specified contiguous spatial classes. 
   
(b) Maps of the residuals and the observed to predicted ratios for validation sites:
   
   * Log residuals, based on the unconditioned (*simulation*) predictions
   
   * Ratio of observed to predicted load for the unconditioned (*simulation*) predictions
   
Three maps are reported for each metric using tab features with the labels "threshold-above", "threshold-below", and "all". The "all" tab displays the metric for all calibration sites, whereas the "threshold" tabs allow the user to display the metric only for calibration sites above and below the midpoint of the user-defined breakpoints, based on the control settings `residual_map_breakpoints` and `ratio_map_breakpoints` as specified in section 8 of the control script.

Note that *plotly* **interactive plots** are automatically output for the model simulation diagnostics in the browser-accessible file *(run\_id)\_Validation_Plots.html* in the "…/(run_id)/estimate" subdirectory. 

Users can optionally output *plotly* **interactive maps** of the validation model residuals (section 4.4.8.3) by selecting the `enable_plotlyMaps<-"yes"` setting (see section 8 of the *sparrow_control.R* script and Chapter subsection 4.4.8.4 for details). The maps are output to the HTML file *(run\_id)\_Validation_Residuals_Maps.html* in the subdirectory "…/(run_id)/estimate/residualMaps". Alternatively, the setting `enable_plotlyMaps<-"no"` outputs non-interactive maps in the model diagnostics file, *(run\_id)\_Validation_Plots.html* in the "…/(run_id)/estimate" subdirectory. 
...
**Additional optional validation diagnostic output for dynamic models**

For dynamic models, diagnostic plots are output for all observations to the *(run\_id)\_Validation_Plots.html* file, identical to the plots that are output for static models as described in the subsection above. Additionally, users have the option to produce seasonal or annual subsets of these diagnostic plots, according to the `diagnosticPlots_timestep` control setting (see Chapter subsection 4.4.8.3.2). The selection of *"season"* or *"year"* for the control setting outputs separate HTML files in the subdirectory *".../estimate/validation_plots_dynamic"*. The filenames have labels corresponding to the type of diagnostic plot, with the contents of each file arranged chronologically by season or year. 

Residual maps can also be optionally output for seasonal and/or annual time periods using the control settings described in Chapter subsection 4.4.8.4 (i.e., `map_years`, `map_seasons`, `mapsPerPage`, `mapPageGroupBy`). Residual maps are output to separate HTML files in the subdirectory *"/validation_plots_dynamic/residualMaps"*, with the filename labels corresponding to the type of residual metric; within each HTML file, the residual metric maps are organized using yearly-denoted tabs consisting of four-panel seasonal map displays.  
...
Results from the application of the Moran's I test (executed by the `if_spatialAutoCorr<-"yes"` setting) are reported in the two files described below (see Chapter subsection 4.4.5.1 for additional details). 
...
**Additional optional spatial autocorrelation output for dynamic models**

For dynamic models, autocorrelation plots are output for all observations to the *(run\_id)\_diagnostic_spatialau tocor.html* file, identical to the plots that are output for static models as described in the subsection above. Additionally, users have the option to produce seasonal or annual subsets of these plots, according to the `diagnosticPlots_timestep` control setting (see Chapter subsection 4.4.8.3.2). The selection of *"season"* or *"year"* for the control setting outputs separate HTML files in the subdirectory *".../estimate/diagnostic_spatialautocor_dynamic"*. The filenames have labels corresponding to the type of diagnostic plot, with the contents of each file arranged chronologically by season or year. See Table 2 in Chapter subsection 2.3 for a complete listing of the HTML files for dynamic models.
...
This file is output by the execution of the parametric bootstrap uncertainty estimation (see Chapter subsection 4.4.10) using the control setting `if_boot_estimate <- "yes"` (section 10 of the control script). The file contains the following elements:
...
For dynamic models, the variables *year* and/or *season* are automatically output to the load and yield CSV files. 
...
The prediction mean estimates and uncertainties as described in Chapter subsection 4.4.10 are output to the CSV files described below. Each file contains the following standard list of *sparrowNames* from the *dataDictionary.csv* file (see Table 4): *waterid*, *originalWaterid*, *rchname*, *rchtype*, *headflag*, *termflag*, *demtarea*, *demiarea*, *meanq*, *fnode*, *tnode*, *hydseq*, *frac*, *iftran*, *staid*. 
...
Note that the estimates of the bootstrap mean are bias corrected individually for each mass-based prediction metric and reach location, without any restrictions on the network-wide mass balance. Thus, the mean estimates do not retain a spatial mass balance within the river network (see Chapter subsection 4.4.10.4 for details).
...
The file is created using the control setting `if_boot_predict <- "yes"` (section 10 of the control script) and contains parametric bootstrap predictions for load and yield (see Chapter subsection 4.4.10 for details on the methods and prediction metrics). Note that the estimates of the bootstrap mean are bias corrected individually for each mass-based prediction metric and reach location, without any restrictions on the network-wide mass balance. Thus, the mean estimates do not retain a spatial mass balance within the river network.

The object contents include:

**boparmlist** = a character vector with the names of the variables associated with the model load predictions. The base variable names are shown above in Chapter subsection 5.3.2.1 and include for each load prediction metric the standard error and the lower and upper bounds for the 90 percent confidence intervals, using the prefixes of **se_**, **ci_lo_**, and **ci_hi_**, respectively. 
...
The maps in the following files are controlled by user settings in sections 8 and 9 of the control script (also see Chapter subsection 4.4.8 and 4.4.9 for details). 
...
Users should note that if a large number of variables are specified for output, especially for a model with a large spatial domain (e.g., more than 300,000 reaches), the execution time for map production will be lengthy. We advise using the RShiny interactive mapper (`enable_shinyMaps<-"yes"` in section 9 of the control script) to output multiple variables in batch mode. This will produce one map per file, which will allow more efficient opening of the output file. Additionally, note that stream maps require much less time to produce than catchment maps.
...
### Mapping subdirectory: (run_id)/maps/Stream

The stream reach maps contain values of the prediction metrics or variables from the *dataDictionary* that are listed in the control setting `master_map_list` in section 8 of the control script. A separate file is generated for each variable, with the name of the variable embedded in the file name:  *(run_id)*_*(variable_name).html*.  The `enable_plotlyMaps` setting (Chapter subsection 4.4.8.4) controls the generation of HTML files with either interactive (`yes`) or non-interactive (`no`) features. The object binary file *batch_(time).RData* is also created if the maps are saved in batch mode.

### Mapping subdirectory: (run_id)/maps/Catchment
...
### Mapping subdirectory:  (run_id)/maps/Interactive

This directory is created if maps are saved (using the *"Save Map"* button) from an RShiny interactive DSS mapping session (i.e., `enable_ShinyApp<-"yes"`; section 9 of the control script; see Chapter subsection 4.4.9 for details).  

There are three subdirectories created with the names "Stream", "Catchment", and "SiteAttributes", based on the types of maps (*"Map Type"*) that are output, depending on the selected map type. 

The output format is controlled by the *"Output Map Format"* setting in the RShiny mapper interface, with a PDF file output for the *"static"* option (with non-interactive features) and a HTML file output (with interactive features) for the *"plotly"* and *"leaflet"* options. The *"plotly"* map option is not available for the display of catchment metrics because of the lengthy rendering time.

The naming convention for the output files is *(run_id)*_*(variable_name).pdf* and *(run_id)*_*(variable_name).html*. 



### Mapping subdirectory:  (run_id)/maps/ESRI_ShapeFiles

There are as many as three subdirectories created, depending on the user selections for the control setting `outputESRImaps` (section 8 of the control script), with the names "prediction", "residuals", and "siteAttributes". 

In the "prediction" subdirectory, the following ESRI files are created:  "lineShape.(dbf,prj,shp,shx)"", and "polyShape.(dbf,prj,shp,shx)", corresponding to stream and catchment maps respectively. 

In the "residuals" subdirectory, the files "residShape.(dbf,prj,shp,shx)" are created for the calibration monitoring sites and populated with data from the *residuals.csv* file. 

In the "siteAttributes" sub-directory, the files "siteAttrshape.(dbf,prj,shp,shx)" are created for the calibration monitoring sites and populated with variables selected using the `mapSiteAttributes` setting.
...
## Management-change scenarios directory:  (run_id)/scenarios

The following files are output to each subdirectory for the user-specified scenario name as given in the *scenario_name* control setting or specified in the RShiny interface. See Chapter subsection 4.4.9 for details on the control settings given in section 9 of the control script. 
...
### (scenario_name)\_(run_id)\_(variable_name).html

A separate stream and/or catchment map is output for each of the prediction metrics (absolute and/or relative) listed in the control setting `scenario_map_list`. Stream maps are output according to the `output_map_type` setting and stored in separate subdirectories ("Stream", "Catchment") according to the specified type. A separate file is generated for each variable, with the name of the variable embedded in the file name:  *(scenario_name)*_*(run_id)*_*(variable_name)*.*html*.  The `enable_plotlyMaps` setting (Chapter subsection 4.4.8.4) controls the generation of a HTML file with features that are either interactive ("yes") or non-interactive/static ("no"). 

When using the RShiny interactive DSS mapper (i.e., `enable_ShinyApp<-"yes"`), the maps can be output and saved using the *"Save Map"* button (see section 9 of the control script and Chapter subsection 4.4.9 for details), with the output format controlled by the *"Output Map Format"* setting (note that the *"static"* option saves the map as a PDF file with non-interactive features). The saved files are written to subdirectories ("Stream" and/or "Catchment") within the scenario directory that is named by the user (i.e., entry for the *"Enter Scenario Name"* option).  


### (scenario_name)_(run_id)_scenario_metainfo.txt

The text file records the values for the following control settings for the user scenario: `select_scenarioReachAreas`, `select_targetReachWatersheds`, and `LanduseConversion`. Additionally, the following settings for the RShiny session for the scenario are recorded:  *Source* and *PercentChange*. Reaches with unadjusted prediction metrics are shown in the maps by the category "No Change" and assigned a ratio of 1.0.
...
The  file is created as an archive with key scenario control settings and the load and yield prediction variables (from Chapter subsections 5.5.3 and 5.5.6) that are output from the execution of a source-change scenario evaluation. The object is output to the subdirectory for the user-specified scenario name as given in the *scenario_name* control setting or specified by the user in the RShiny interface. The absolute prediction metrics are corrected for log-retransformation bias as is done for the standard predictions (without changed sources). 
...
**scenario_map_list** = a scenario control setting that specifies the type of predictions to map for stream reaches and output to a HTML file in batch mode.
...
**oparmlist** = a character vector with the names of the variables associated with the model load predictions for management scenarios as described in Chapter subsection 5.5.3. 

**loadunits** = a character vector with the units of the load predictions corresponding to the variable names given in the *oparmlist* vector.

**predmatrix** = a NxR matrix with the values of the load predictions, where N is the number of reaches and R is the number of the load prediction variables corresponding to the names given in the *oparmlist* vector. 

**oyieldlist** = a character vector with the names of the variables associated with the model yield predictions for management scenarios as described in Chapter subsection 5.5.6.
...
The following files are created in a new directory with the name `results\modelComparison_name` in cases where a user selects to compare the results of the executed model with multiple previously executed models, as specified by the control settings for `compare_models` (see Chapter subsection 4.4.11.4): 
    

### (modelComparison_name)_summary.txt

The text file list in sequence for the user-specified models the following metrics from the *(run\_id)\_summary.txt* file (see Chapter subsection 5.2.4.2):
...
The CSV files associated with *(run\_id)\_summary.txt* file output are stored in the "(run_id)/estimate/summaryCSV"" subdirectory and combined in the model comparison. The combined CSV file contains the **parameter estimate metrics** as described above, with an additional column for the user-specified `compare_models`.
...
# Tutorial for static models:  Executing and interpreting a series of models that build in complexity
...
Subsection 6.2 demonstrates this incremental approach to model building. The tutorial presents statistical information on the model diagnostics to illustrate the effects of the additional explanatory variables on model performance and the interpretability of the models. Two of the tutorial models (3, 7) are also presented to illustrate the potential sensitivity of model results to the use of different initial values for explanatory variables that have lower levels of statistical significance (also see subsection 4.4.4.4). 

The model control files can be used to execute any of the eight models demonstrated in the tutorial. Control files are stored in the *"run_id"* subdirectories under the "UserTutorial/results" directory. Model results (text, plots, maps) are only provided for tutorial model 6, based on a prior execution of the model. The other models must be executed to output their results, which are directed to various files in the *"run_id"* subdirectories.

**To execute a model, users have the option to copy the control files (".csv", ".R") from a given *"run_id"* model subdirectory to the "results" directory, followed by a renaming of the files to exclude the model identification prefix and execution of the *sparrow_control.R* script in RStudio. **

**Alternatively, users can load the control settings for any of the eight models into the RStudio active RSPARROW control script (*sparrow_control.R* located in the "UserTutorial/results" directory) according to the following steps:**

* Set the `copy_PriorModelFiles` setting to the model *"run_id"* of choice and enter the user's path name for the `path_master` setting in the active control script (section 11 of the control script; Chapter subsection 4.4.11.3). 

* Execute the control script (i.e., select *Source* in RStudio). This will overwrite the active control script (and all input control files) in the *"results"* directory with the versions of these files from the prior model. Note that the user's path name specified for the `path_master` setting in the active control script will be retained and is not overwritten. 

* Once execution has completed and the prior model settings are copied into the active control script in the RStudio session, other model control settings can be kept or modified to perform various model applications, such as estimation, prediction, mapping, or evaluations of management scenarios in the RShiny interactive mapper. 

Subsection 6.3 is supplementary information that illustrates the use of the tutorial data in the *data1.csv* file to automatically create the model input control files (*parameters.csv*, *design_matrix.csv*). A generic description of the approach is given in step 3 of the checklist in Chapter subsection 4.2.4. The supplementary information in 6.2 is intended to demonstrate the types of Console messages that appear during the execution of these steps.    
...
To execute this model, *'parmInit'* was set to 500 kg/km2/yr (chosen arbitrarily) and *'parmMin'* and *'parmMax'* were set to 0 and a large value (10,000 kg/km2/yr), respectively, in the *parameters.csv* file. Note that if the control setting `edit_Parameters<-"yes"` is selected, then the file will appear as a "popup" on the screen; once entries are completed, users should save the file (*users should consult subsection 4.4.4.3 for details on the selection of parameters and setting of the initial and minimum/maximum values for parameters*). 

Note that the summary statistics for model performance and coefficient estimates and the plots shown in this section for model 1 can be obtained from the *“model1_summary.txt”* and *"Model1_Diagnostic_Plots.html"* files in the *“/results/Model1/estimate”* subdirectory, respectively.  Residual maps with *plotly* interactive features can be accessed in the *"Model1_Diagnostics_Residuals_Maps.html"* file in the *"/estimate/residualMaps"* subdirectory (resulting from the use of the `enable_plotlyMaps <- "yes"` control setting). 

The observed total nitrogen loads at the 708 calibration sites, to which the model is fit, are shown in Fig. \ref{diagnostics_mod1_A}a. The loads span over five orders of magnitude and indicate the combined influence of watershed sources and streamflow (watershed size). High loads are observed across the corn-belt and also in larger rivers, such as the Upper Mississippi and Ohio. Total nitrogen yields (see Fig. \ref{diagnostics_mod1_A}b) are more indicative of watershed sources, and display large values in the corn-belt areas of the Midwest, with much more moderate to low values of yield in the Appalachians and Upper Midwest areas in the states of Wisconsin and Minnesota. These plots are available in the *TN_model1_Diagnostic_Plots.html* file in the *"results/estimate"* subdirectory, based on the use of the setting `map_siteAttributes.list<-c("meanload","meanyield")` in the control script (section 8). Both variables were defined in the *userModifyData.R* script. 
...
\caption{\textbf{Calibration site map of the model residuals for the total nitrogen model 1.} Overpredictions of load are shown by negative residual values and underpredictions of load are shown by positive residual values. }
...
The effect of a highly statistically insignificant variable (`shrubgrass`; *p*=0.973) on the stability of the estimated coefficients and residuals for model 3 is demonstrated through a series of three models:  3a, 3b, and 3c. The results for these models are only discussed here; details for the model summary metrics can be examined in the saved model subdirectories.  
...
\caption{\textbf{Calibration site map for the model residuals for the total nitrogen model 6 with source, land-to-water delivery, and aquatic decay explanatory variables. }  Overpredictions are shown by negative values; underpredictions are shown by positive values. }
...
### Evaluating management-change scenarios

Two versions of the model (model 6 and 3) are used to illustrate the execution of management-change scenarios in the decision support tool using the RShiny interactive mapper. Settings in section 9 of the control script (and the *userModifyData.R* script) can also be used to execute change scenarios, but the RShiny interactive mapper is more efficient and easier to use. The documentation (Chapter 4.4.9) should be consulted for details about the general approach used to identify watersheds and reaches for scenarios and information on the types of prediction metrics available for output for scenario results.

Management-change scenarios can be applied in the interactive mapper (or manually in the control script) to evaluate the effects of changes in source inputs, expressed in units of either *mass* (e.g., fertilizer use) or *land area* (e.g., land cover/use), and/or changes in land-to-water delivery variables on the water-quality loads and concentrations in downstream reaches. For scenarios with area-based land-use sources, users have the option to convert the land area from one land-use type to another, or users can change the land-use source loading per unit area through changes in the mean estimates of the land-use model coefficients (this latter option can only be executed in the RShiny DSS). For scenarios that involve changes in the area of a land-use source, users are required to select a land use source to receive an equivalent areal change in the opposite direction (i.e., land conversion). This ensures that the total drainage area in the scenario is unchanged; for example, linking reductions in forested land with increases in crop land that are equivalent in areal units. 

Management-change scenarios can be executed for user-defined watershed locations, source types, land-to-water delivery variables, and change magnitudes. In the RShiny DSS, users first specify the reach outlets (i.e., targeted reach locations) for watersheds where the source-change scenarios are to be applied. RSPARROW automatically identifies all hydrologically connected reach segments upstream of the outlet reaches. Users then enter tabular information to specify the details of the change scenario attributes, including variable names and conditions for the sources, land-to-water delivery variables, change factors, and reach locations within the targeted watersheds where the scenarios apply. 

Two modes are available to specify changes in land-water delivery variables and/or source variables that are evaluated in a scenario: (1) **“change-percentage scenarios”** —menu entries allow users to specify the magnitude of a hypothetical percentage change in the values of one or more explanatory variables; and (2) **“forecasting file change scenarios”** —users input an external CSV file to the RShiny DSS, which contains new reach-level values of one or more of the model explanatory variables that reflect hypothetical or model-forecasted changes in watershed conditions (this mode should be used to evaluate changes in transformed land-to-water delivery variables—see discussion in section 4.4.9.2.1).
...
\caption{\textbf{RShiny interactive mapper interface with settings to evaluate a 25 percent reduction in the atmospheric deposition nitrogen source in all stream reaches for model 6.}  }
\label{shiny_2}
\end{figure}

\begin{figure}
\includegraphics[width=1.6\linewidth]{figures/training_tutorial/tutorial_figures_shiny3.jpg}
\caption{\textbf{RShiny interactive map of the modeled domain, displaying the relative effects on total nitrogen load of a 25 percent reduction in the atmospheric deposition nitrogen source in all reaches for model 6. The load effects are reported as a ratio of the updated (changed) model prediction of load to the baseline model load prediction. The RShiny control settings for the evaluated scenario conditions are shown in the previous figure. } }
\label{shiny_3}
\end{figure}
...
\caption{\textbf{RShiny interactive mapper interface with settings to evaluate a 25 percent reduction in the atmospheric deposition nitrogen source in stream reaches of the Ohio River Basin (huc2=5) for model 6.}  }
\label{shiny_6}
\end{figure}
...
\caption{\textbf{RShiny interactive map of the modeled domain, displaying the relative effects on total nitrogen load of a 25 percent reduction in the atmospheric deposition nitrogen source in stream reaches in the Ohio River Basin (huc2=5) for model 6. The load effects are reported as a ratio of the updated (changed) model prediction of load to the baseline model load prediction. The RShiny control settings for the evaluated scenario conditions are shown in the previous figure.} }
\label{shiny_7}
\end{figure}
...
\caption{\textbf{RShiny interactive mapper interface with settings to evaluate a 25 percent reduction in the atmospheric deposition nitrogen source in stream reaches of the Wabash River (waterid=15531) and New River (waterid=13747) Basins for model 6.}  }
\label{shiny_4}
\end{figure}
...
\caption{\textbf{RShiny interactive map of the modeled domain, displaying the relative effects on total nitrogen load of a 25 percent reduction in the atmospheric deposition nitrogen source in stream reaches in the Wabash River (waterid=15531) and New River (waterid=13747) Basins for model 6. The load effects are reported as a ratio of the updated (changed) model prediction of load to the baseline model load prediction. The RShiny control settings for the evaluated scenario conditions are shown in the previous figure.} }
\label{shiny_5}
\end{figure}
...
\caption{\textbf{RShiny interactive mapper interface with settings to evaluate multiple sources, source-change percentages, and reach locations for the midwestern total nitrogen model 6.}  }
\label{shiny_8}
\end{figure}

\begin{figure}
\includegraphics[width=1.6\linewidth]{figures/training_tutorial/tutorial_figures_shiny9.jpg}
\caption{\textbf{RShiny interactive map of the modeled domain, displaying the relative effects on total nitrogen load of hypothetical reductions in multiple sources and reach locations. The load effects are reported as a ratio of the updated (changed) model prediction of load to the baseline model load prediction. The RShiny control settings for the evaluated scenario conditions are shown in the previous figure. } }
\label{shiny_9}
\end{figure}
...
Using the land-use model 3 for the Midwest, the scenario applies a 30% reduction in crop land area (`crops`), with the affected crop land areas converted to a `shrubgrass` cover by specifying the *LanduseConversion* land cover type as `shrubgrass`. RSPARROW ensures that the total drainage area is unchanged in the scenario simulation, with the area reduction in crop land (associated with the 30% reduction) corresponding to an equivalent increase in the area of shrub-grass land cover.  
...
\caption{\textbf{RShiny interactive mapper interface with settings to evaluate a 30 percent reduction in the cropland area source in stream reaches of the Upper Mississippi River Basin (huc2=5) for model 3.}  }
\label{shiny_10}
\end{figure}
...
\caption{\textbf{RShiny interactive map of the modeled domain, displaying the relative effects on total nitrogen load of a 30 percent reduction in the cropland area source in stream reaches in the Upper Mississippi River basin for model 3. The load effects are reported as a ratio of the updated (changed) model prediction of load to the baseline model load prediction. The RShiny control settings for the evaluated scenario conditions are shown in the previous figure. } }
\label{shiny_11}
\end{figure}
...
As in the previous example (subsection 6.2.9.3), the scenario is only applied to reaches in the watersheds of the Upper Mississippi regional basin (i.e., huc2=7), with the terminal reaches used to identify the targeted reach watersheds. The following selections were made to enable the reach targeting:
...
\caption{\textbf{RShiny interactive mapper interface with settings to evaluate a 30 percent reduction in export of nitrogen from the pasture land source in stream reaches of the Upper Mississippi River Basin (huc2=5) for model 3.}  }
\label{shiny_12}
\end{figure}
...
\caption{\textbf{RShiny interactive map of the modeled domain, displaying the relative effects on total nitrogen load of a 30 percent reduction in nitrogen load from the pasture land source in stream reaches in the Upper Mississippi River basin for model 3. The load effects are reported as a ratio of the updated (changed) model prediction of load to the baseline model load prediction. The RShiny control settings for the evaluated scenario conditions are shown in the previous figure. } }
\label{shiny_13}
\end{figure}
...
Note that the data and parameter input control files for the SPARROW tutorial model are provided to users in final form and require no setup using the automated control settings illustrated in this section. To execute the steps below, users will need to temporarily place the existing final copies of the *dataDictionary.csv*, *parameters.csv*, and *design_matrix.csv* files in another directory and delete the files in the *User Directory/results* subdirectory. 

Users should also review step 3 in the checklist in Chapter subsection 4.2.4 for details on the setup of the input control files.
...
# Tutorial for dynamic models:  Illustrating the calibration, diagnostics, and applications of the model

## Introduction

The tutorial uses a preliminary test-version of a SPARROW dynamic model, developed to simulate total phosphorus (TP) loads in streams of the Tampa Bay watershed, a 5,700 square kilometer basin on the central western coast of the state of Florida that drains to the Gulf of Mexico. The model uses non-propriety data from the USGS (R. Smith, written comm., 2022) and NASA (J. Srikishen, written comm., 2022) that are currently being evaluated with various versions of a SPARROW dynamic TP model for the Tampa Bay watershed. One statistical dynamic version of the TP model is presented here. Note that the model and data are used for illustration purposes only to demonstrate the features and capabilities of RSPARROW. The model and supporting data should not be used for actual water resource evaluations of TP loads in the Tampa Bay watershed or to support related water management decisions for the watershed or estuary.  

The dynamic model calibration uses seasonal mean estimates of TP, computed from the instantaneous TP measurements and mean daily streamflow values using the WRTDS method (Hirsch et al., 2010). The seasonal mean values are available for the period Spring 2001 to Summer 2014 at 23 monitoring sites. The calibrated model is applied to 2,780 stream reaches in watersheds draining to the Tampa Bay estuary, using the NHD medium resolution (1:100,000 scale) digital stream network data. 

The estimated TP model includes five source variables, five land-to-water delivery variables, and one stream decay variable. Temporally varying phosphorus inputs are estimated for two sources:  fertilizer (`FertP_Seas`) and urban runoff (`urban_Seas`), whereas the remaining three sources are represented by constant seasonal phosphorus inputs:  municipal wastewater treatment (`FacWW_P`), phosphorus in the runoff from active and inactive mines (`Pmines_v17`), and natural geologic sources of phosphorus (`GeolP`).  All of the land-to-water climate-related variables vary by year and season, including total precipitation (`logPPT`), mean air temperature (`logAirTemp`), soil moisture (`PrevSurfSoMoi`), and enhanced vegetation index (EVI; `PrevEVIMean`). The remaining land-to-water delivery variable is a seasonally constant soil erodibility index (`lkfactup`). The stream decay variable (`rchtot`) is expressed as the mean annual reach time of travel per depth of streamflow, with no seasonal variation. 

The tutorial, presented in three subsections, uses the Tampa Bay TP model to illustrate the use of various settings and display capabilities for dynamic SPARROW models, including the use of the RShiny DSS. The first subsection (7.2) uses the multi-year seasonal SPARROW model (**TP_season**) to describe the model calibration and selected model-performance diagnostics as well as examples of the use of the RShiny DSS to execute five management-change scenarios. The model is calibrated using mean seasonal observations at 23 monitoring sites (n=1220 observations; slightly shorter records are available at 10 sites). 

The subsequent major chapter subsections illustrate the required control settings for the execution of dynamic models with different timesteps, including models of mean annual and long-term mean seasonal stream loads:  subsection 7.3 presents a TP model of mean annual loads (**TP_MeanAnnual**; n=296), whereas subsection 7.4 presents a TP model of long-term mean seasonal loads (**TP_MeanSeason**; n=92). The loads in these models are computed as annual or long-term seasonal averages of the original multi-year observations of mean seasonal TP  (**TP_season**) used in the Tampa Bay model (i.e., 2002 to 2014 time period at 23 monitoring sites), with the explanatory data averaged for the timesteps in a similar manner. 

The input control files for the tutorial models are stored in the *“run_id”* subdirectories (**TP_season**, **TP_MeanAnnual**, **TP_MeanSeason**) under the *“UserTutorial_Dynamic/results”* directory. Model results are only provided for the tutorial model **TP_season**, based on a prior execution of the model. The other dynamic models must be executed to output their results, which are directed to various files in the *"run_id"* subdirectories. For dynamic models, users have the option to output selected time-dimensioned diagnostic plots and maps, based on settings in section 8 of the sparrow control file.

The model performance and diagnostic results presented in this chapter can be viewed for the **TP_MeanSeason** model (and for other models following their execution) by accessing files in the *“UserTutorial_Dynamic/results/(run_id)/estimate”* directory and the associated *“…/estimate/diagnosticPlotsNLLS_ dynamic“* subdirectory for each model (see the directory structure diagram in Fig. 1). Output directed to the upper-level of the *“diagnosticPlotsNLLS_dynamic“* subdirectory include separate HTML files for each of the model diagnostic plots from the *“(run\_id)\_Diagnostic_Plots.html“* file, organized in each file by season and/or year according to the user-control settings. In addition, HTML files with seasonal and/or annual displays of the site attributes and model residuals are output to two subdirectories, *“mapSiteAttributes“* and *“residualMaps“*, respectively. A separate *“…/estimate/diagnostic_sensitivity_dynamic“* subdirectory contains three files with the model parameter sensitivity metrics, organized in each file by season and/or year according to the user-control settings.  

Finally, the upper-level *“UserTutorial_Dynamic/results/(run_id)/estimate” * directory contains the identical diagnostic files that are output for a static SPARROW model for all observations (see Chapter subsection 5.2). These include, for example, the *“(run\_id)\_summary.txt“* and  *“(run\_id)\_Diagnostic_Plots.html“* files that provide basic tabular and graphic information on the SPARROW model estimation and performance diagnostics. 


## Mean seasonal model for years 2001 to 2014 (TP_season)

###  Model estimation and diagnostics

Execution of the provided RSPARROW control script (*sparrow_control.R*) in RStudio (after installing the required software and setting RStudio defaults--see subsections 1.3.3 and 1.3.4) will run the SPARROW tutorial model "TP_season", with results written to the "TP_season_User" directory. To verify the software installation and model execution, users can compare the results with those provided in the "TP_season" directory. 

During execution of the control script in RStudio, a variety of warning messages will appear in the Console window related to data checks for missing or zero values. This is expected for the TP model data set and does not adversely affect the execution of the model or generation of the results. 

The model performance metrics for the estimated model as shown below are output to the *(run\_id)\_summary.txt* file.  These reflect the aggregate performance of the model across all seasons and years for the 1220 seasonal observations at 23 monitoring sites. The R-square is 0.916 for loads and 0.846 for yields for the estimated (monitoring-adjusted) model with a prediction bias of 6.9%. The model performance in simulation mode for unconditioned predictions (i.e., without any adjustment for the monitoring data) provides a more accurate measure of the predictive skill of the model, independent of the monitored data; model predictions in simulation mode frequently show somewhat lower prediction accuracy than model predictions from the estimated model using monitoring-adjusted loads. Accordingly, the simulation model performance shows a somewhat lower R-square of 0.842 for loads and 0.713 for yields, with a much higher prediction bias of 22.8%.  
...
```{r, eval=FALSE}
  ...
  MODEL ESTIMATION PERFORMANCE (Monitoring-Adjusted Predictions)
 MOBS NPARM   DF      SSE       MSE      RMSE       RSQ RSQ-ADJUST RSQ-YIELD PERCENT BIAS
 1220    11 1209 359.3834 0.2972567 0.5452125 0.9156663  0.9149687 0.8463134     6.997145
                                                      
  MODEL SIMULATION PERFORMANCE (Simulated Predictions)
 MOBS NPARM   DF      SSE      MSE      RMSE     RSQ RSQ-ADJUST RSQ-YIELD PERCENT BIAS
 1220    11 1209 672.2426 0.556032 0.7456755 0.84225  0.8409453 0.7125223     22.77574
...
A summary is also provided in the *(run\_id)\_summary.txt* file of the model performance metrics for each of the seasons, as shown below.  These results show generally acceptable and consistent measures of model fit across the different seasons, suggesting that the model performs reasonably well in all of the seasons. A generally better fit to the data is observed for the winter season, based on the yield R-squared metrics (0.838 and 0.696 for monitoring-adjusted and simulated predictions, respectively), although the improved overall fit in this season is accompanied by a somewhat larger prediction bias (i.e., over-prediction), especially for the simulated (unconditioned) predictions (i.e., 32.8%).  
...
```{r, eval=FALSE}
  ...
  MODEL ESTIMATION PERFORMANCE (Monitoring-Adjusted Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
  296 81.83259 0.9161446 0.8378106      7.90121
                                                      
  MODEL SIMULATION PERFORMANCE (Simulated Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
  296 153.1812 0.8430322    0.6964     32.79161
        
  SPRING
                                                                
  MODEL ESTIMATION PERFORMANCE (Monitoring-Adjusted Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
  314 108.8262 0.8676483 0.7373484     2.491845
                                                      
  MODEL SIMULATION PERFORMANCE (Simulated Predictions)
 MOBS     SSE       RSQ RSQ-YIELD PERCENT BIAS
  314 191.572 0.7670148 0.5376416     12.89269
        
  SUMMER
                                                                
  MODEL ESTIMATION PERFORMANCE (Monitoring-Adjusted Predictions)
 MOBS      SSE      RSQ RSQ-YIELD PERCENT BIAS
  314 70.08023 0.906869 0.7582986     8.089529
                                                      
  MODEL SIMULATION PERFORMANCE (Simulated Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
  314 123.4817 0.8359027 0.5741211     23.46295
      
  FALL
                                                                
  MODEL ESTIMATION PERFORMANCE (Monitoring-Adjusted Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
  296 98.64438 0.9048032 0.7864218     6.711744
                                                      
  MODEL SIMULATION PERFORMANCE (Simulated Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
  296 204.0077 0.8031224 0.5582963     22.02364

```


Note that small, uninformed initial values were used to estimate the model: 0.01 for all sources and stream decay and 0.0 for all land-to-water delivery variables. Model convergence was achieved after 28 iterations, with relatively rapid convergence to within about five percent of the final SSE occurring after only five iterations. The very small and highly uninformed values used for the initial parameter values as well as the high degree of statistical significance of the final estimates of the model parameters (all with p<0.002) suggest that the model converged to a minimum SSE. 

The estimated model parameters and their associated standard errors and statistical significance metrics, as shown in the table below, are output to the *(run\_id)\_summary.txt* file in the *".../estimate"* subdirectory.  

```{r, eval=FALSE}
  PARAMETER ESTIMATES
   PARAMETER     PARM TYPE ESTIMATE    SE         T         P-VALUE      VIF             
1  FacWW_P       SOURCE     1.18146674 0.23218738  5.088419 4.182844e-07  1.358623 
2  FertP_Seas    SOURCE     0.08919828 0.01272449  7.009968 3.953504e-12  5.813746                                         
3  Pmines_v17    SOURCE     0.05658318 0.01037863  5.451894 6.036856e-08  7.224750                                            
4  GeolP         SOURCE     0.11853349 0.03654896  3.243142 1.214498e-03 18.405091                                                    
5  urban_Seas    SOURCE    25.97400611 6.26699979  4.144568 3.641769e-05  3.800808            
6  logAirTemp    DELIVF    -0.91599292 0.18458299 -4.962499 7.954342e-07  2.169542                                               
7  logPPT        DELIVF     1.42999059 0.04822198 29.654329 0.000000e+00  2.168088                                                 
8  PrevSurfSoMoi DELIVF     0.37794396 0.12136580  3.114089 1.888497e-03  2.635118                                           
9  PrevEVIMean   DELIVF     2.78090585 0.21767503 12.775493 0.000000e+00  6.083602                                              
10 lkfactup      DELIVF     1.79652141 0.40053434  4.485312 7.973409e-06  3.013900                                                 
11 rchtot        STRM       0.79614057 0.08484248  9.383749 0.000000e+00 16.492245                                      
  
  
  EigenValue Spread  Normal PPCC  SWilks W      P-Value   Mean Exp Weighted Error
           141.4985    0.9914509 0.9862119 2.435671e-09                  1.125879

```

The model specification is designed to support climate-related short-term forecasting, based on a seasonal (i.e., 3-month) NOAA forecast of climatic conditions for precipitation and ambient temperature (see Chapter subsection 7.2.2.2.3 for illustrations of the use of SPARROW with climate forecasts). For this specification, the two climate variables enter the model as contemporaneous seasonal conditions for the historical time periods (Spring 2001 to Summer 2014). Short-term forecast estimates were not available for the other seasonal climate-related variables (i.e., soil moisture, EVI). Therefore, these variables, which have the `Prev` prefix for the name, are used in the model as non-contemporaneous measures of seasonal conditions, based on the value of the variable for the previous season. 

This lagged temporal structure, based on conditions in a previous timestep, is intended to provide supplemental climate-related information to account for the proximate (i.e., prior season) effects on water quality of soil and vegetation properties that are associated with climate processes. This structure can potentially improve the accuracy of water-quality predictions and forecasts by accounting for these additional climate-related soil and vegetation properties. Contaminant storage processes have been simulated in newer dynamic versions of the SAS SPARROW model (R. Smith, USGS, written comm., 2022) by adding a first-order lag process, using one or more estimated model coefficients for temporally lagged sources, or by adding a storage-related source term for baseflow or groundwater discharge that reflects contaminant transit-time distributions. The inclusion of these storage processes in RSPARROW would provide a more realistic and interpretable version of a dynamic model that could be used to evaluate change scenarios. The inclusion of these process components will be considered for future versions of RSPARROW.

Note that one additional non-contemporaneous explanatory variable, evapotranspiration (`PrevET`), was evaluated in a preliminary version of the dynamic model presented here and was found to be statistically insignificant (p=0.9882), and therefore, was eliminated from the final model fit.

Also, note that the selected dynamic model presented here is a preliminary version that lacks any adjustment for the effects of serial correlation in the time series of model residuals on coefficient standard errors and significance. A future version of RSPARROW is being considered that will provide more accurate estimates of the coefficient standard errors and significance levels, which can support final user decisions on the selection of model variables. Adjustments for serial correlation effects would be expected to have the largest influence on decisions about whether the more marginally significant variables (i.e., `GeolP`, `PrevSurfSoMoi`) are included in the final model.
...
Options for the control settings for managing the output of diagnostic plots and residual maps for dynamic models are described in chapter subsections 4.4.8.3.2 and 4.4.8.4 (*plotly* interactive graphics). 

Seasonal diagnostic plots for the predicted loads, yields, and residuals are shown for the **TP_season** model in Figure \ref{dyntutorial_1}. These plots support the conclusion that the model performs similarly in each of the seasons—i.e., relatively similar scatter is observed along the red 1:1 line in the plot of observed vs. predicted values as well as for the model residuals, which are equally dispersed above and below the red horizontal lines in the plots. The plots are output to the *ModEstimation_Obs_v_Pred.html* file in the *"…/estimate/diagnosticPlotsNLLS_dynamic"* subdirectory, based on the control setting `diagnosticPlots_timestep<-“season”`.

Time series plots of the observed and predicted values are also provided for each of the monitoring sites; plots are illustrated for two the 23 monitoring sites in Figure \ref{dyntutorial_2}. The plots are output to the *Obs_v_Pred_TimeSeriesPlots.html* file in the *"…/estimate/diagnosticPlotsNLLS_dynamic"* subdirectory, based on the control setting `diagnostic_timeSeriesPlots<-"yes”`. 

Users can also create additional time series plots of the predictions, residuals, and other monitoring station attributes by using R or other plotting software to access the data output to the *(run\_id)\_residuals.csv* file in the *“UserTutorial_Dynamic/results/(run_id)/estimate”* directory. 

Additional diagnostics are also available to evaluate the geographic distribution of the dynamic model residuals; these can be used to determine if spatial biases exist in the model predictions and whether the assumption of constant variance in the residuals is valid. Figure \ref{dyntutorial_3} shows log residual maps for the TP model for seasons in the years 2001 and 2002; users can output maps for other selected years or all years and seasons using control settings in section 8 of the sparrow control script. the maps are output to the *ModelEstimationLogResiduals_all.html* file in the *"…/estimate/diagnosticPlotsNLLS_dynamic/residualMaps"* subdirectory. The map output format is controlled by four settings as described in Chapter subsection 4.4.8.4.


\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_1.jpg}
\caption{\textbf{Seasonal diagnostic plots for the RSPARROW Tampa Bay seasonal total phosphorus model.} The diagnostics are based on monitoring-adjusted (conditioned) predictions. The 4-panel plots for each season include:  (a) observed load vs. predicted load (upper left); (b) observed yield vs. predicted yield (upper right); (c) log residuals vs. predicted load (lower left); and (d) log residuals vs. predicted yield (lower right). }
\label{dyntutorial_1}
\end{figure}
...
\begin{figure}
\includegraphics[width=1.3\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_2.jpg}
\caption{\textbf{Time series plots of the observed and predicted values for the RSPARROW Tampa Bay seasonal total phosphorus model for the spring 2001 to summer 2014 period.} Plots are shown for two of the 23 monitoring stations used to calibrate the model. }
\label{dyntutorial_2}
\end{figure}
...
\begin{figure}
\includegraphics[width=1.3\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_3.jpg}
\caption{\textbf{Calibration site map for the seasonal residuals for the Tampa Bay seasonal total phosphorus model.} The model residuals are displayed for seasons in the 2001 and 2002 years. }
\label{dyntutorial_3}
\end{figure}
...
###  Use of the RShiny DSS

For dynamic models, the DSS menu interface is identical to that described for static models, with the exception of the additional menu display feature labeled *“Dynamic Mapping Settings”*. This feature allows the user to control the timestep of the data in the map display (note that when executing management scenarios, user selection of the timestep for the model simulation is managed by other DSS menu settings--see details in Chapter subsection 7.2.2.2 below). 

Depending on the timestep associated with the dynamic model, a menu setting, with dropdown check-box entries, appears with the label *“year(s)”* and/or *“season(s)”* (see examples of the dropdown entry options and explanation for these settings in Figures \ref{dyntutorial_4} and \ref{dyntutorial_5}). A maximum of four maps is allowed for display on the DSS page for any single execution (i.e., “Generate Map” button) of user settings from the menu. Therefore, users can select a maximum of four years or seasons for display for a multi-year seasonal model, such as the Tampa Bay TP model. 

Note that the timestep associated with a dynamic model is automatically detected according to the availability of the “year” and/or “season” variable in the *data1.csv* input file. 


####  Map display of *dataDictionary* variables and model predictions

&nbsp;

All of the *dataDictionary* variables and model predictions can be mapped for any of the available timesteps of the model. These can be display for the monitoring stations (i.e., “Site attributes”) or for the streams or catchments. Figure \ref{dyntutorial_4} illustrates the mapping of the seasonal fertilizer use variable (`FertP_Seas`) for each of the four seasons in 2002 at the monitoring sites, whereas Figure \ref{dyntutorial_6} illustrates the mapping of the model predictions of total yield for the same time period in all streams. 

Users also have the option to compute summary metrics for the variables that is selected for display, including *“mean”*, *“median”*, *“min”*, or *“max”*, by using the *“Dynamic Mapping Settings”* for *“year(s)”* and *“season(s)”*. For example, selection of the “mean” check-box option for the “year(s)” setting, as shown in Figure \ref{dyntutorial_5}, computes a mean across all years for each season that is check-box selected (up to the maximum of four seasons) for “season(s)” setting.  Four seasons are displayed in Figure \ref{dyntutorial_5} because all seasons were selected as check-box options for the “season(s)” setting.

By contrast, if a user were to select the *“mean”* check-box option for the *“season(s)”* setting, this would compute a mean across all seasons for each year that is check-box selected in the *“year(s)”* setting, up to the maximum of an allowed four years (i.e., the allowable maximum of four maps for the DSS page display). 

Note that the following text is provided in the *“Dynamic Mapping Settings”* section of the RShiny DSS menu to explain the use of the *“year(s)”* and *“season(s)”* options for selecting timesteps for the map display: 

&nbsp;

```{r, eval=FALSE}
RShiny DSS Menu:  Explanation of the Dynamic Mapping Settings

A maximum of 4 maps can be displayed. The maps are based on selections of individual 
“year(s)” and/or “season(s)”, or alternatively, the selection of a summary metric 
(e.g., “mean”, “min”), which is either computed across all “year(s)” for each “season(s)” 
selection (up to a maximum of 4 seasons) or computed across all “season(s)” for each 
“year(s)” selection. Note that the selection of a summary metric (e.g., mean) for both 
“year(s)” and “season(s)” will display a single map, based on the computation of the 
metric across all available observations for the period of record.  

```

See Table 8 in Chapter subsection 4.4.8.4 for additional guidance and examples of the map display options based on the settings available in the sparrow control script. There is no limit placed on the number of maps that can be output using the control script settings.  


\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_4.jpg}
\caption{\textbf{RShiny DSS interactive mapper interface for the Tampa Bay seasonal total phosphorus model, displaying fertilizer use at the 23 monitoring sites during four seasons of 2002. } The “Map Type” selection of “Site Attributes” produces a map of the monitoring sites for the user-selected variable. }
\label{dyntutorial_4}
\end{figure}


\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_5.jpg}
\caption{\textbf{RShiny DSS interactive mapper interface for the Tampa Bay seasonal total phosphorus model, displaying the mean fertilizer use by season for the 2001 to 2014 period.  } Each seasonal mean is computed as an average of the annual values of fertilizer use for the 2001 to 2014 period. The check-box settings in the DSS menu for “year(s)” are “mean” and for “season(s)” are each of the four seasons (“winter”, “spring”, “summer”, “fall”). }
\label{dyntutorial_5}
\end{figure}


\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_6.jpg}
\caption{\textbf{RShiny DSS interactive mapper interface for the Tampa Bay seasonal total phosphorus model, displaying the model predictions of total yield for the four seasons in 2002.  } The check-box settings in the DSS menu for “year(s)” are “2002” and for “season(s)” are each of the four seasons (“winter”, “spring”, “summer”, “fall”). }
\label{dyntutorial_6}
\end{figure}
...
Five management-change scenarios are illustrated below, using the Tampa Bay seasonal TP model. The scenarios illustrate the two modes available for evaluating management scenarios (see the next subsection 7.2.2.2.1), including one **“change-percentage scenario”**, evaluating a hypothetical percentage reduction in fertilizer use, and four **“forecasting file change scenarios”**, evaluating short- (spring, 2022) and long-term (year 2030) changes in climate variables, a long-term (2030) change in land cover, and a long-term (2030) simultaneous change in both land cover and climate.

Illustrations of the forecasting file change scenarios use external CSV files that contain a *waterid* variable associated with the last year (and either the spring season or all seasons during the last year) for the calibration period (i.e., 2014). Therefore, the map displays of relative changes in model predictions in the scenarios reflect forecasted future changes in relation to the water-quality predictions for watershed conditions at the end of the calibration record. A user could select other years and/or seasons during the calibration period (e.g., a timestep that might be more representative of normal or extreme conditions during the calibration period) to compare with future conditions, provided the user ensures that the timestep during the calibration period is reflected in the choice of the values of the *waterid* variable listed in the external CSV file (See section 4.4.9.2.1 for details on the forecast CSV file setup). 



##### RShiny DSS menu settings for executing scenarios

&nbsp;

Scenarios are executed by selecting the *“Scenarios for Changes in Sources and/or Delivery Variables”* option for the *“Map Type”* menu item. Two modes are available to specify the land-water delivery variables and/or source variables for which natural or managed changes in their values are to be evaluated in a scenario (note that detailed explanations for executing these scenarios are provided in each of two subsequent subsections 7.2.2.2.2 and 7.2.2.2.3):  

* **“Change-percentage scenarios”** - Menu entries allow users to specify the magnitude of a hypothetical percentage change in the values of one or more explanatory variables relative to the values for a base or reference timestep during the calibration period. A percentage change can also be applied to one or more model coefficients associated with explanatory variable values for a base or reference timestep (see Chapter subsection 4.4.9.1.2 for details). Note that the change-percentage scenario option should not be applied to transformed (e.g., logarithmic) land-to-water delivery variables because this implicitly assumes a disproportionate change in the original units of the variable relative to the user-specified percentage; this can result in erroneous interpretations of the magnitudes of the simulated changes in stream water-quality loads. 

* **“Forecasting file change scenarios”** - Users input an external CSV file to the DSS, which contains new (i.e., changed) reach-level values of one or more of the model explanatory variables associated with a “future” timestep (i.e., that extends beyond the last timestep of the model calibration period). The future values may reflect hypothetical or model-forecasted changes in contaminant sources and/or land-to-water delivery variables. This option also allows the evaluation of scenarios that entail changes in land-water delivery variables that are transformed in the calibrated model (e.g., logarithmic). In these cases, a relative or absolute change can be externally applied to the untransformed delivery variable by the user, with the variable subsequently transformed and stored in the external CSV file prior to input to the RSPARROW DSS. 

**For both scenario modes, the *“Dynamic Mapping Display Settings”* for the “year(s)” and/or “season(s)” only control the mapping display of the scenario results.** Users should ensure that the mapping display timestep (year and/or season) is consistent with the model simulation timestep for the scenario. The simulation timestep is associated with the base or reference conditions during the calibration time period that are either altered (i.e., as in **“change-percentage scenarios”**) or selected to enable comparisons with future hypothetical or forecasted water-quality predictions (i.e., as in **“forecasting file change scenarios”**). 

For **“change-percentage scenarios” **, users should select a mapping display timestep (year and/or season) that is consistent with the user-specified model simulation timestep, which defines the base or reference conditions of the explanatory variables during the calibration time period that receive the percentage adjustment. The simulation timestep (year and/or season) is entered as part of the reach selection criteria in the section of the RShiny menu entitled *“Select Target Reach Watersheds”*, which appears when a *“no”* response is selected for the *“Run Scenario using forecasting file input”* option. Tabular entries of the simulation timestep can be made when the option *“selected reaches”* is specified for the menu setting labeled *“Select reaches for applying the scenario (within targeted watersheds)”*.  

For **“forecasting file change scenarios”** (activated by a *“yes”* response to the *“Run Scenario using forecasting file input”* option), users should select a mapping display year and/or season associated with the base or reference timestep during the calibration period that are associated with the values of the future changed inputs, as recorded in the external CSV file.  This base/reference timestep is implicitly represented in the values of the required *waterid* variable in the CSV file. The *waterid* variable is a spatial and temporal index, with values that are numerically unique for each stream reach and timestep in the model calibration period. The temporal index component allows future water-quality predictions (for change scenarios) to be simulated, and subsequently compared with the water-quality predictions during the calibration timestep, thereby permitting the computation and display of change magnitudes in the predictions (i.e., see the display option entry *“Relative Change in Load”* for the *“Mapping Variable Type”* in the RShiny menu). See section 4.4.9.2.1 for details on the format requirements to setup the external CSV file (this section also describes the tasks that are automatically executed by the RSPARROW code to ensure that the forecast values are properly linked to the stream reaches and timesteps for the calibrated SPARROW model).


##### Change-percentage scenario:  Change in the base condition of a source variable

&nbsp;

This **“change-percentage scenario”** illustrates the effects on TP stream loads of a 10 percent reduction in seasonal fertilizer use (`FertP_Seas`). The DSS menu settings and the map display of the reach-level effects of fertilizer reduction on TP load are shown in Figures \ref{dyntutorial_7a} and \ref{dyntutorial_7b}. The base year and season for application of the scenario is Summer 2014, the most recent season and year available in the record used to calibrate the model. The check-box *“Dynamic Mapping Display Settings”* are *“2014”* for *“year(s)”* and *“summer”* for *“season(s)”*, which will generate a map in the DSS display that is consistent with the simulation timestep. 

To specify the timestep (year and/or season) for application of the scenario simulation (as illustrated in Figures \ref{dyntutorial_7a}), the following menu entries are required: 

* Select the option *“selected reaches”* for the menu setting labeled *“Select reaches for applying the scenario (within targeted watersheds)”*. Note that selection of the option *“all reaches”* for this setting does not allow the user to enter values for the timestep. 

* Select the option *“yes”* or *“no”* for the menu setting labeled *“Apply same reach selection criteria to all selected changed variables (yes/no)”*. 

  * A *“yes”* entry, as illustrated in Figure \ref{dyntutorial_7a}, displays two tables in the menu. The first table allows users to select the changed model variable (`FertP_Seas`) and enter the percentage quantity for application of the percentage change (see table label *“Select Model Explanatory Variables and Percent Change Factors”*). The second table allows users to specify additional criteria for the scenario, including the timestep entries (see table label *“Reach Selection Criteria”*), which apply to all model explanatory variables (i.e., sources and/or delivery variables). As shown in Figure \ref{dyntutorial_7a}, the year and season were entered into the second table, using the *“AND”* separator to ensure that both conditions are applied to all of the reaches. 

  *	A *“no”* entry results in the display of only one table. This option provides greater flexibility, allowing users to apply different reach (and timestep) selection criteria and change percentages to each model variable. To enter the timestep for a seasonal model, two rows are required, one for the year and a second for the season, with the *“AND”* separator selected for the first row. Note that the changed model variable (e.g., seasonal fertilizer) and the change percentage (e.g., -10) must be entered on both rows for the table entries to be accepted (*this does NOT cause any replication of the application of the change percentage*). 
  
\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_7a.jpg}
\caption{\textbf{RShiny DSS interactive mapper interface for the Tampa Bay seasonal total phosphorus model, with settings to evaluate a percentage-change scenario for a 10 percent reduction in the seasonal fertilizer source variable. The table outlined in blue illustrates how to add a second row to the table to select for season timestep in the reach selection criteria. } }
\label{dyntutorial_7a}
\end{figure}

\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_7b.jpg}
\caption{\textbf{RShiny DSS interactive mapper display results for the Tampa Bay seasonal total phosphorus model, with settings to evaluate a percentage-change scenario for a 10 percent reduction in the seasonal fertilizer source variable. } }
\label{dyntutorial_7b}
\end{figure}

The results of the scenario simulation as shown in Figure \ref{dyntutorial_7b} display the relative change in TP load in streams.  These show that a 10 percent reduction in seasonal fertilizer would be expected to result in reductions in stream TP load of less than about 8 percent, with most changes less than 2 percent. Predicted reductions of substantially less than 10 percent in many of the Tampa Bay streams is an indication that other sources that are unchanged in the scenario contribute considerably to stream phosphorus loads. 



##### Change scenarios input from a user's forecasting file:  Changes in future climate and land-cover conditions  

&nbsp;

Four scenarios are presented in this subsection that illustrate the effects of a changes in climate and land-cover conditions on stream TP loads. The model explanatory variables evaluated in these scenarios include the seasonal mean precipitation (`logPPT`), mean temperature (`logAirTemp`), and fertilizer use (`FertP_Seas`).  The DSS menu settings and the map display of the reach-level effects of changes in the explanatory variables on TP load are shown in Figures \ref{dyntutorial_8a} to \ref{dyntutorial_11b}. A base year of 2014 is used in the scenarios to facilitate comparisons with the predictions for future years; 2014 is the most recent year available in the record that was used to calibrate the model. The methods and references used to derive the forecasts of future climate and land-cover conditions associated with the explanatory model variables are described in the next subsection (7.2.2.2.4).

As we noted in section 1.2, users are cautioned that the lack of contaminant storage components in the statistical dynamic model limits the interpretability of the outcomes of change scenarios, which do not precisely quantify the effects of changed model variables on the timing of future changes in stream water quality. A more physically based representation of storage processes in the dynamic model, such as lagged sources or groundwater discharge with contaminant transit times, would be necessary (and is being considered for future RSPARROW versions) to provide a more realistic forecast of future changes in stream water quality in response to managed or natural changes in watershed sources, land use, or climate.

The temporal menu selections for the *“Dynamic Mapping Display Settings”*, which control the map display of the results, include *“2014”* for *“year(s)”* for all of the scenarios; the *“season(s)”* setting is *“spring”* for the first scenario (Figures \ref{dyntutorial_8a} and \ref{dyntutorial_8b}), whereas all four seasons are selected for display for the subsequent scenarios (Figures \ref{dyntutorial_9a} to \ref{dyntutorial_11b}). For each of the scenarios, the *“year”* and *“season”* settings generate prediction maps in the DSS display that are consistent with the simulation timestep that is implicitly reflected in the values of the *waterid* variable (unique reach and timestep variable) in the external CSV file. A pair of figures are shown for each scenario example, with the first displaying the menu settings and the second displaying the mapped results for the model simulations. 

To input the external CSV forecasting files to the DSS, the following menu entries are required, as shown in the figures:

* Enter *“yes”* for the menu setting labeled *“Run Scenario using forecasting file input”*. 

* Enter the name of the file for the menu setting labeled *“Enter forecast scenario filename”*. The external CSV file must be located in the user’s RSPARROW *“…/data”* directory, the most secure location to avoid the overwriting of files. A copy of the file is stored in the *“…/results/scenarios/(scenario_name)/”* directory when the scenario is executed, but may be overwritten if a scenario with the same name is subsequently executed.

* Using the check-box entry adjacent to the CSV file name, select the type of header labels that are used in the CSV file for the column with the unique reach and timestep variable—i.e., *data1UserNames* (e.g., *time_arcid*) or *sparrowNames* (e.g., *waterid*). The *sparrowNames* are used in the CSV files for all of the scenarios illustrated here. 


&nbsp;&nbsp;

*Short-term change in climate conditions (Figures \ref{dyntutorial_8a} and \ref{dyntutorial_8b})*

The simulation predictions display the percentage change in TP load in streams that result from changes in the seasonal mean precipitation (`logPPT`) and mean temperature (`logAirTemp`) from the spring of 2014 to the spring of 2022 (*Tampa_NMME_2022_02_Spring.csv* forecast file).  These simulations are based on short-term NMME forecasts of seasonal climate conditions for time periods extending 6 months into the future (Kirtman et al., 2014), as obtained from the RSAPPT tool (White et al., 2022). 

Forecasted changes in seasonal precipitation and seasonal temperature would be expected to result in changes in stream TP load ranging from about -80% to +170%, with the changes in most stream reaches falling between a 33% increase and 56% decrease. Most increases in TP load would be expected to occur in the central portion of the Tampa Bay watershed, whereas most decreases would occur in the northern and southern portions of the watershed. 

When executing this scenario, two warning messages will appear in the DOS command prompt window, indicating that two variables (PPT30MEAN, meanTemp), which are not used as explanatory variables in the TP model, were found in the forecasting file and removed from use in the forecasting scenario. This resulted from an RSPARROW quality control check, which ensures that only SPARROW model explanatory variables are used in the forecasting simulations.  

\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_8a.jpg}
\caption{\textbf{RShiny DSS interactive mapper interface for the Tampa Bay seasonal total phosphorus model, with settings to evaluate a forecasting file change scenario, involving a change in the mean precipitation and temperature conditions from the spring 2014 to the spring 2022.   } }
\label{dyntutorial_8a}
\end{figure}

\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_8b.jpg}
\caption{\textbf{RShiny DSS interactive mapper display results for the Tampa Bay seasonal total phosphorus model, with settings to evaluate a forecasting file change scenario, involving a change in the mean precipitation and temperature conditions from the spring 2014 to the spring 2022.   } }
\label{dyntutorial_8b}
\end{figure}

&nbsp;&nbsp;

*Long-term change in climate conditions (Figures \ref{dyntutorial_9a} and \ref{dyntutorial_9b})*

The simulation predictions display the percentage change in TP load in streams that result from changes in the seasonal mean precipitation (`logPPT`) and mean temperature (`logAirTemp`) from 2014 to 2030 for all seasons of the year (*Tampa_CMIP5_2030_rcp45.csv* forecast file).  These simulations are based on long-term MACA (Multivariate Adaptive Constructed Analogs) climate predictions, which are statistically downscaled from the seasonal forecasts from an ensemble of seven global climate models, as obtained from the RSAPPT tool (White et al., 2022).

Forecasted changes in seasonal precipitation and seasonal temperature would be expected to result in changes in stream TP load ranging from -80% to +170%, with the changes in most stream reaches falling between a 18% increase and 38% decrease. During the winter and fall seasons, most reaches display decreases in TP load of from -6% to -38%. TP loads during the spring and summer seasons are predominantly higher within streams, with the largest increases observed in the middle portion of the Tampa Bay watershed in the spring season. 


\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_9a.jpg}
\caption{\textbf{RShiny DSS interactive mapper interface for the Tampa Bay seasonal total phosphorus model, with settings to evaluate a forecasting file change scenario, involving a change in the mean precipitation and temperature conditions from 2014 to 2030.  Results are shown for all seasons of the year.  } }
\label{dyntutorial_9a}
\end{figure}

\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_9b.jpg}
\caption{\textbf{RShiny DSS interactive mapper display results for the Tampa Bay seasonal total phosphorus model, with settings to evaluate a forecasting file change scenario, involving a change in the mean precipitation and temperature conditions from 2014 to 2030.  Results are shown for all seasons of the year.  } }
\label{dyntutorial_9b}
\end{figure}

&nbsp;&nbsp;

*Long-term change in land-cover conditions (Figures \ref{dyntutorial_10a} and \ref{dyntutorial_10b})*

The simulation predictions display the percentage change in TP load in streams that result from changes in seasonal mean fertilizer use (`FertP_Seas`) associated with changes in crop lands from 2014 to 2030 for all seasons of the year (*RSPARROW_LU_Tampa_Tp.csv* forecast file).  These simulations are based on projections of fertilizer use derived from forecasts of land-cover changes (M. Estes, 2022). 

Forecasted changes in seasonal fertilizer use would be expected to result in changes in stream TP load ranging from -80% to a maximum of about 9,000%, with the vast majority of reaches showing increases in TP load in all seasons. The increases in TP load typically range from about 0.1% to 54% and are generally nonuniformly distributed throughout the watershed in each season. 


\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_10a.jpg}
\caption{\textbf{RShiny DSS interactive mapper interface for the Tampa Bay seasonal total phosphorus model, with settings to evaluate a forecasting file change scenario, involving a change in the fertilizer use from 2014 to 2030.  } Results are shown for all seasons of the year. }
\label{dyntutorial_10a}
\end{figure}

\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_10b.jpg}
\caption{\textbf{RShiny DSS interactive mapper display results for the Tampa Bay seasonal total phosphorus model, with settings to evaluate a forecasting file change scenario, involving a change in the fertilizer use from 2014 to 2030.  } Results are shown for all seasons of the year. }
\label{dyntutorial_10b}
\end{figure}

&nbsp;&nbsp;

*Long-term change in land-cover and climate conditions (Figures \ref{dyntutorial_11a} and \ref{dyntutorial_11b})*

The simulation predictions display the percentage change in TP load in streams that result from changes in the seasonal mean precipitation (`logPPT`) and mean temperature (`logAirTemp`), and seasonal mean fertilizer use (`FertP_Seas`) from 2014 to 2030 for all seasons of the year (*RSPARROW_LuClimate_Tampa_Tp.csv* forecast file).  These simulations are based on long-term MACA (Multivariate Adaptive Constructed Analogs) climate predictions, which are statistically downscaled from the seasonal forecasts from an ensemble of seven global climate models, as obtained from the RSAPPT tool (White et al., 2022), as well as projections of fertilizer use derived from forecasts of land-cover changes (M. Estes, 2022).

Forecasted changes in seasonal precipitation, temperature, and fertilizer use would be expected to result in changes in stream TP load ranging from -80% to a maximum of about 14,000%, with the changes in most stream reaches falling within an interval ranging from a 45% increase to a 34% decrease. The predominant increases in TP load, which are projected to result from increases in fertilizer use in 2030 (as displayed in Fig. \ref{dyntutorial_10b}), are moderated by the forecasted changes in climate conditions for 2030 (see Fig. \ref{dyntutorial_11b}). This gives rise to a greater balance in the number of stream reaches that would be expected to show increases and decreases in TP load.  Some seasonal differences in the direction of change are projected to occur:  During the winter and fall seasons, more streams tend to display decreases in TP loads, whereas TP loads during the spring and summer seasons show somewhat larger numbers of stream reaches with increases.



\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_11a.jpg}
\caption{\textbf{RShiny DSS interactive mapper interface for the Tampa Bay seasonal total phosphorus model, with settings to evaluate a forecasting file change scenario, involving a change in the mean precipitation, mean temperature, and fertilizer use from 2014 to 2030.  }  Results are shown for all seasons of the year. }
\label{dyntutorial_11a}
\end{figure}

\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_11b.jpg}
\caption{\textbf{RShiny DSS interactive mapper display results for the Tampa Bay seasonal total phosphorus model, with settings to evaluate a forecasting file change scenario, involving a change in the mean precipitation, mean temperature, and fertilizer use from 2014 to 2030.  }  Results are shown for all seasons of the year. }
\label{dyntutorial_11b}
\end{figure}



##### Forecasts of future climate and land-cover conditions:  Summary of the data used in the scenarios

&nbsp;

Predictions of future climate and land-cover conditions, as evaluated in the scenarios in the previous subsection, were obtained using a climate forecasting tool (RSAPPT; White and others, 2022) and projections of annual land-cover changes to the year 2040 from FORE-SCE (M. Estes, written communication, National Space Science Technology Center, Aug. 2022; Sohl and Sayler, 2008), respectively. Projections of fertilizer use, as required for the simulations with the calibrated TP SPARROW model, were derived from forecasts of land-cover changes (M. Estes, 2022) with assumed areal fertilizer usage rates for crop lands.  

The RSAPPT climate tool is an open-source python script that downloads climate model output for the southeastern USA and computes seasonal and annual climate metrics (e.g., mean precipitation and temperature) for future years (download the application zip file here: https://po.ncche.olemiss.edu/sharing/5RNbVR5qB). The tool is customized to provide geospatial referencing of the climate metrics to SPARROW National Hydrological Data stream reach segments for three watersheds (Mobile Bay, AL; Tampa Bay, FL; and Winyah Bay, S.C.) in the southeastern USA. The tool outputs the unique reach and timestep identification number associated with the last year and season of the SPARROW calibration model and the climate metrics for temperature (deg. F.) and precipitation (mm/season). 

The RSAPPT tool provides two types of climate metrics for input and evaluation in the RSPARROW DSS. The first consists of long-term forecasts of annual climate conditions for years 2006-2099 (https://climate.northwestknowledge.net/MACA/data_portal.php).  The metrics are generated from MACA (Multivariate Adaptive Constructed Analogs) data, which are statistically downscaled from the seasonal forecasts of an ensemble of seven global climate models. The second type of RSAPPT metrics consists of short-term forecasts of seasonal conditions for time periods extending 6 months into the future (https://www.cpc.ncep.noaa.gov/products/NMME/). These metrics are generated from NMME (North American Multimodel Ensemble) climate model output of mean monthly and seasonal climate conditions (Kirtman et al., 2014).



\break

## Mean annual model for years 2002 to 2014 (TP_MeanAnnual)

For purposes of illustrating a mean annual model, the total phosphorus loads were computed as annual averages of the original multi-year observations of mean seasonal TP that were used in the Tampa Bay seasonal model (i.e., **TP_season**; 2002 to 2014 time period at 23 monitoring sites). This resulted in 296 total observations in the model, corresponding to the number of monitoring stations with annual data for the period 2002 to 2014 (note that the load observations and explanatory data for the year 2001 were removed because data were available for only three of the seasons). 

Additionally, the explanatory data were averaged for the timesteps in a similar manner for each unique stream reach (*comid*).  All response and explanatory variable names are unchanged from those used in the mean seasonal model. Temporal variables in the input data file were modified so that only *“year”* variable is retained, with no *“season”* variable appearing in the file. 

The following settings were changed in the RSPARROW control file to execute the model:
```{r, eval=FALSE}

# Section 1
  input_data_fileName <- "mean_annual.Tampa.data1.csv"

# Section 8
  map_seasons<-NA    
  map_years<-c(2002,2003)  # Users can select any calibration years of interest
  diagnosticPlots_timestep<-"year"     
  mapPageGroupBy<- "year" 

# Section 11
  run_id <- "TP_MeanAnnual" 

```

Finally, for a mean annual model any occurrences of the *“season”* variable should be removed from the *dataDictionary* and the *userModifyData.R* for continuity.  

&nbsp;&nbsp;

*Model calibration and performance*

The estimated model parameters and their associated standard errors and statistical significance metrics are output to the *(run\_id)\_summary.txt* file, as shown below.

Eight of the 11 parameters are statistically significant (p<=0.05), with non-significant coefficients estimated for two source variables (geologic P, urban lands) and one land-to-water delivery variables (temperature). The statistically insignificant variables were kept in the model for illustration purposes. 

```{r, eval=FALSE}

  PARAMETER ESTIMATES
   PARAMETER     PARM TYPE ESTIMATE   SE         T          P-VALUE      VIF             
1  FacWW_P       SOURCE    0.59471312 0.26547485  2.2401863 2.584954e-02  1.352610
2  FertP_Seas    SOURCE    0.09126731 0.02679581  3.4060297 7.539607e-04 15.394797                                         
3  Pmines_v17    SOURCE    0.07195110 0.02326026  3.0933058 2.175815e-03  8.156347                                               
4  GeolP         SOURCE    0.05223248 0.03411481  1.5310793 1.268590e-01 14.661352                                                    
5  urban_Seas    SOURCE    1.42343223 3.71579221  0.3830764 7.019487e-01  5.357853            
6  logAirTemp    DELIVF    0.11512141 0.71439139  0.1611461 8.720925e-01  1.116813                                              
7  logPPT        DELIVF    1.33709887 0.10053359 13.3000209 0.000000e+00  1.086455                                                
8  PrevSurfSoMoi DELIVF    1.94970592 0.28551011  6.8288508 5.163470e-11  4.166696                                          
9  PrevEVIMean   DELIVF    2.94435446 0.75396335  3.9051692 1.176246e-04 14.666963                                             
10 lkfactup      DELIVF    1.39459524 0.83744937  1.6652890 9.695393e-02  3.641468                                                
11 rchtot        STRM      0.82230687 0.19825607  4.1477009 4.436052e-05 17.830544                                     
  
  
  EigenValue Spread  Normal PPCC  SWilks W     P-Value   Mean Exp Weighted Error
           144.7482    0.9882826 0.9828834 0.001318519                  1.078047

```

The model performance metrics for the estimated model are shown below as output to the *(run\_id)\_summary.txt* file.  These reflect the aggregate performance of the model across all years for the 296 annual observations at 23 monitoring sites. The R-square is 0.929 for loads and 0.863 for yields for the estimated (monitoring-adjusted) model with a prediction bias of 4.81%. The model performance in simulation mode for unconditioned predictions (i.e., without any adjustment for the monitoring data) provides a more accurate measure of the predictive skill of the model, independent of the monitored data; model predictions in simulation mode frequently show somewhat lower prediction accuracy than model predictions from the estimated model using monitoring-adjusted loads. Accordingly, the simulation model performance shows a somewhat lower R-square of 0.881 for loads and 0.770 for yields, with a much higher prediction bias of 20.7%. 

```{r, eval=FALSE}

  MODEL NAME: TP_MeanAnnual
                                                                                                                          
  DYNAMIC MODEL SUMMARY
                                             
  Maximum number of calibration stations = 23
                                       
  Calibration time period: 2002 to 2014
  
                                                                
  MODEL ESTIMATION PERFORMANCE (Monitoring-Adjusted Predictions)
 MOBS NPARM  DF      SSE       MSE      RMSE       RSQ RSQ-ADJUST RSQ-YIELD PERCENT BIAS
  296    11 285 68.94857 0.2419248 0.4918585 0.9293471   0.926868 0.8633463     4.807367
                                                      
  MODEL SIMULATION PERFORMANCE (Simulated Predictions)
 MOBS NPARM  DF      SSE       MSE      RMSE       RSQ RSQ-ADJUST RSQ-YIELD PERCENT BIAS
  296    11 285 115.9047 0.4066831 0.6377171 0.8812302  0.8770629 0.7702808     20.70475
...
## Long-term mean seasonal model  (TP_MeanSeason)

For purposes of illustrating a long-term mean seasonal model, the total phosphorus loads were computed as annual averages of the original multi-year observations of mean seasonal TP that were used in the Tampa Bay seasonal model (i.e., **TP_season**; 2002 to 2014 time period at 23 monitoring sites). This resulted in 92 seasonal observations in the model, with 23 observations (i.e., the number of monitoring stations) available in each season for the period 2002 to 2014 (note that the load observations and explanatory data for the year 2001 were removed because data were available for only three of the seasons).

Additionally, the explanatory data were averaged for the timesteps in a similar manner for each unique stream reach (*comid*).  All response and explanatory variable names are unchanged from those used in the mean seasonal model. Temporal variables in the input data file were modified so that only *“season”* variable is retained, with no *“year”* variable appearing in the file. 
...
# Section 1
  input_data_fileName <- "mean_seasonal.Tampa.data1.csv"

# Section 8
  map_seasons<-"all"    
  map_years<-NA
  diagnosticPlots_timestep<-"season"     
  mapPageGroupBy<- "season" 

# Section 11
  run_id <- "TP_MeanSeason" 

```

Finally, for a long-term mean seasonal model any occurrences of the *“year”* variable should be removed from the *dataDictionary* and the *userModifyData.R* for continuity.  
...
Only eight of the original 13 parameters were statistically significant (p<=0.05) for the model estimation. Two parameters (previous soil moisture; soil erodibility factor) had very small estimated coefficients, which interfered with the model estimation and were eliminated. The remaining statistically insignificant variables included two source variables (manure waste, geologic P) and one land-to-water delivery variable (precipitation). The statistically insignificant variables were kept in the model for illustration purposes.  

```{r, eval=FALSE}

  PARAMETER ESTIMATES
   PARAMETER     PARM TYPE ESTIMATE      SE           T             P-VALUE      VIF              
1  FacWW_P       SOURCE     4.5165836520  2.025604614  2.2297459337 0.0285323693  2.382061 
2  FertP_Seas    SOURCE     0.0772543980  0.023376374  3.3048066636 0.0014172709 15.023525                                         
3  Pmines_v17    SOURCE     0.0115743961  0.005802549  1.9947090149 0.0494379518  2.796605                                              
4  GeolP         SOURCE     0.0007981166  1.000000000  0.0007981166 0.9993651575  1.185265                                                  
5  urban_Seas    SOURCE    54.7582258215 14.858929561  3.6852066361 0.0004115245  4.155469            
6  logAirTemp    DELIVF     1.9005741510  1.000000000  1.9005741510 0.0609152231  1.875716                                               
7  logPPT        DELIVF     0.0664247776  1.000000000  0.0664247776 0.9472033242  2.032892                                                  
8  PrevSurfSoMoi DELIVF    -0.0062335722  0.020507474 -0.3039658671 0.7619333781  1.064748                                          
9  PrevEVIMean   DELIVF     6.6922820962  0.288292349 23.2135265618 0.0000000000  5.573968                                             
10 lkfactup      DELIVF     1.7878352990  1.523848370  1.1732370057 0.2441392963  2.052094                                                
11 rchtot        STRM       0.9838480516  0.255516458  3.8504292779 0.0002345165 23.164231                                      
  
  
  EigenValue Spread  Normal PPCC  SWilks W   P-Value   Mean Exp Weighted Error
            137.259    0.9956338 0.9804432 0.1827726                  1.057901

```

The model performance metrics for the estimated model are shown below as output to the *(run\_id)\_summary.txt* file.  These reflect the aggregate performance of the model across all seasons for the 92 seasonal observations at 23 monitoring sites. The R-square is 0.943 for loads and 0.903 for yields for the estimated (monitoring-adjusted) model with a prediction bias of 4.1%. The model performance in simulation mode for unconditioned predictions (i.e., without any adjustment for the monitoring data) provides a more accurate measure of the predictive skill of the model, independent of the monitored data; model predictions in simulation mode frequently show somewhat lower prediction accuracy than model predictions from the estimated model using monitoring-adjusted loads. Accordingly, the simulation model performance shows a somewhat lower R-square of 0.937 for loads and 0.879 for yields, with a much higher prediction bias of 15.8%. 


```{r, eval=FALSE}

MODEL NAME: TP_MeanSeason
                                                                                                                                                 
  DYNAMIC MODEL SUMMARY
                                             
  Maximum number of calibration stations = 23
                                 
  Calibration time period: 0 to 0
  
                                                                
  MODEL ESTIMATION PERFORMANCE (Monitoring-Adjusted Predictions)
 MOBS NPARM DF      SSE       MSE      RMSE      RSQ RSQ-ADJUST RSQ-YIELD PERCENT BIAS
   92    11 81 14.33046 0.1769192 0.4206177 0.949222  0.9429532 0.9031512     4.139282
                                                      
  MODEL SIMULATION PERFORMANCE (Simulated Predictions)
 MOBS NPARM DF      SSE       MSE      RMSE       RSQ RSQ-ADJUST RSQ-YIELD PERCENT BIAS
   92    11 81 17.78666 0.2195884 0.4686026 0.9369755  0.9291947 0.8797933     15.84142
...
A summary is also provided in the *(run\_id)\_summary.txt* file of the model performance metrics for each of the seasons, as shown below.  These results show generally consistent measures of model fit across the different seasons, suggesting that the model performs reasonably well in all of the seasons. A generally better fit to the data is observed for the winter season, based on the yield R-squared metrics (0.848 and 0.796 for monitoring-adjusted and simulated predictions, respectively), although the improved overall fit in this season is accompanied by a somewhat larger prediction bias (i.e., under-prediction), especially for the simulated (unconditioned) predictions (i.e., -36.7%).  
...
```{r, eval=FALSE}
  ...
  MODEL ESTIMATION PERFORMANCE (Monitoring-Adjusted Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
   23 3.430284 0.9341307 0.8483855    -11.77609
                                                      
  MODEL SIMULATION PERFORMANCE (Simulated Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
   23 4.624062 0.9112074 0.7956218    -36.74935
        
  SPRING
                                                                
  MODEL ESTIMATION PERFORMANCE (Monitoring-Adjusted Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
   23 4.159016 0.9023693 0.7675626    -7.057357
                                                      
  MODEL SIMULATION PERFORMANCE (Simulated Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
   23 4.455978 0.8953982  0.750966    -9.247988
        
  SUMMER
                                                                
  MODEL ESTIMATION PERFORMANCE (Monitoring-Adjusted Predictions)
 MOBS     SSE       RSQ RSQ-YIELD PERCENT BIAS
   23 2.40438 0.9415434 0.7469949   -0.4430429
                                                      
  MODEL SIMULATION PERFORMANCE (Simulated Predictions)
 MOBS      SSE      RSQ RSQ-YIELD PERCENT BIAS
   23 2.431132 0.940893 0.7441799     3.440397
      
  FALL
                                                                
  MODEL ESTIMATION PERFORMANCE (Monitoring-Adjusted Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
   23 4.336779 0.9345013  0.755634     12.41409
                                                      
  MODEL SIMULATION PERFORMANCE (Simulated Predictions)
 MOBS      SSE       RSQ RSQ-YIELD PERCENT BIAS
   23 6.275487 0.9052208 0.6463929     38.42569
...
The RSPARROW library dependencies include the standard R packages listed in the DESCRIPTION file located in the *"RSPARROW_master"* directory. The 2.1 version of RSPARROW with an R-4.4.2 installation was tested using the versions of the library dependencies listed in the DESCRIPTION file. 

We recommend installation of R-4.4.2, using the zipped version included with the RSPARROW 2.1 download from the USGS GitLab repository. The included R version contains all R libraries and dependencies that are required to execute this RSPARROW version. 

Advanced R users can alternatively install R for versions >=4.4.2 from [CRAN](https://cran.r-project.org/) and automatically install R dependencies for RSPARROW when executing the RSPARROW control script. Execution of the control script runs the R statement shown below (`devtools::install_deps`), which automatically installs (on the first run of the script) the most recent versions of the required R functions in the user's default R library (as specified in RStudio). The installed R library packages are then loaded into the RStudio session by the R statement `devtools::load_all`. 
...
In subsequent executions of the control script, no updates are made to the library dependencies (see Chapter subsection 4.4.12 for details). Users are not required to update the R packages to versions that are more recent than those listed for the RSPARROW compatibility versions in the DESCRIPTION file. However, when a more recent version of R is installed (>R-4.4.2), the latest versions of the library dependencies will be automatically installed with the initial execution of the control script. Note that RSPARROW compatibility with updated R package versions (more recent than those listed in the DESCRIPTION file) is not guaranteed.

Note that RStudio will print the following message in a yellow banner when the RSPARROW control script is opened in RStudio for the first time with a new R version:  *"Packages devtools and rstudioapi required but are not installed."* Users should ignore this message and instead execute (*"Source"*) the control script, which will install these and other required R packages (**User caution:** Package installation using the yellow banner prompt may exclude certain required packages such as *"backports"*). 

Users are also directed to select *"No"* as the response to the following question, if it appears in a pop-up box in RStudio during the installation of R library dependencies: *"Do you want to install from sources the packages which need compilation?"*. A *"No"* response installs R packages from binary versions, which may not include the most recent version of a package. The selection of *"Yes"* requires that users have previously set up [*Rtools42*](https://cran.r-project.org/bin/windows/Rtools/) (an R developer package) to compile R packages from their source, otherwise the installation of R packages from their source will fail. 
...
There are 205 functions and routines in the RSPARROW system, classified according to 12 functional types (with many of the R functions classified by more than one type). These type categories allow developers to search the functions (and their dependencies) by type when using the developer tools in Section 8.2.2.  Some functions/routines are classified according to more than one category.
...
**sparrowSetup** -  Used to create, import, check, and format control files for SPARROW execution (41).  

**sparrowExec** - Core functional routines that execute SPARROW models (46).

**shiny** - Used by the RShiny interactive decision support tool (28).

**batch** - Used for batch execution of R scripts using the *Rscript.exe* executable (7).

**errorTrap** - Tests control settings and files for critical errors and stops execution if a critical error is detected (22).

**utility** - Used throughout the RSPARROW system to perform minor computational or administrative tasks; these include variable input arguments (67).

**lists** - Creates control setting lists that organize the control settings for import into multiple functions and routines (8).

**rmd** - R Markdown functions for creating HTML output of plots and maps (2).

**fortran** - Fortran subroutines that perform many of the core computational tasks in SPARROW modeling. These tasks typically entail repeated calculations (e.g., accumulation of loads in the reach network) that are more efficiently performed in Fortran. The subroutines are stored as DLL files in the RSPARROW_master/src directory (5). 

**external** - Functions that can be used outside of the RSPARROW system and/or within the *userModifyData.R* control script (8).

**dynamic** - R routines and Markdown (rmd) functions for executing dynamic versions of the SPARROW model (10). 


A tabular listing of some RSPARROW functions and routines classified by type is shown in Figure \ref{functionType1}. The table is also stored in the *RSPARROW_master/inst/tables/funcTypes.csv* file.



\begin{figure}
\includegraphics[width=1.3\linewidth]{figures/FigFunctionTypes1.jpg}
\caption{\textbf{RSPARROW library functions and routine types, organized by 12 functional categories. Users can access this table in the RSPARROW\_master/inst/tables/funcTypes.csv file.} }
\label{functionType1}
\end{figure}
...
The *dataDictionary.csv* file provides an active repository of the system and user-defined variable names (see Chapter subsection 3.2). Access to the contents of this file during an RSPARROW RStudio session streamlines R developer access to SPARROW variables for use in mapping or other types of functions.

The contents of this file (variable names and attributes) are stored in the *data_names* and *subdata* R binary objects during an RStudio session. The *subdata* object is also output to the */data* subdirectory, following the execution of the control script. 


### Parameter classification conventions

The RSPARROW model parameters are assigned different classifications according to the conceptual description of contaminant sources and landscape and aquatic processes in the SPARROW model. The process classifications of the model parameters are described in Chapter subsection 3.3 as SOURCE, DELIVF, STRM, RESV, OTHER. The parameter types are also stored in the *SelParmValues* object. 
...
* **backupOptions** - A list of the saved user options for customized error handling described in subsection 4.4.11.6. 
...
* **Csites.weights.lnload.list** - A named list that is saved internally for the `NLLS_weights<-"lnload"` control setting and contains information on the weights for the calibration monitoring sites. The list is created in the *estimateWeightedErrors.R* function. The contents of the list includes:
   ...
   * *weights_nlr* = the normalized weight, expressed as the reciprocal of variance as a function of the log transformed predicted load. See Chapter subsection 4.4.4.11 for details on the methods.
...
* *Development of the capability to host the RShiny Decision Support System (DSS) mapper and existing SPARROW models on a remote Web server.* This capability would eliminate the need for end-users (e.g., water resource managers) to install and maintain copies of R, RStudio, and the SPARROW data on their personal computer. RSPARROW version 1.1, which executes RShiny from a Web browser that is independent of RStudio, reflects recent changes in the RSPARROW coding architecture that can assist efforts to develop remote server hosting of RSPARROW data and software.  

* *Development of a more efficient R-based SPARROW NLLS model estimation algorithm.* The RSPARROW function employs Fortran looping to sum loads within the reach network, as done in the SAS SPARROW Fortran subroutine. However, SAS execution of the SPARROW NLLS function is more than two times faster than that of the RSPARROW NLLS function, *estimateFeval.R*. Evaluations of alternative approaches for performing matrix calculations in R yielded negligible differences in execution times.
...
* *Testing and development of the RSPARROW functions and control script for use on IOS (Apple) and Linux systems.* Two features of the current RSPARROW functions have Windows dependencies: the automated pop-up of CSV control files and the batch-mode operation. 

* *Estimation of contaminant storage in watersheds in dynamic SPARROW models.* The dynamic model presented in RSPARROW 2.0 is a simplified statistical dynamic specification of a SPARROW model for simulating temporally varying stream water-quality loads. Although the model empirically incorporates the implicit effects of contaminant storage processes into the model predictions and the estimated coefficients, it lacks structural components (i.e., coefficients or input data) that would provide a quantifiable physical representation of storage processes (e.g., in soils and groundwater). Contaminant storage processes have been simulated in newer dynamic versions of the SAS SPARROW model (Schmadel et al., 2024; Miller et al., 2024) by adding a first-order lag process, using one or more estimated model coefficients for temporally lagged sources, or by adding a storage-related source term for baseflow or groundwater discharge that reflects contaminant transit-time distributions. The inclusion of these storage processes in a future version of RSPARROW would provide a more realistic and interpretable dynamic model that could be used to evaluate change scenarios. The inclusion of these process components will be considered for future versions of RSPARROW. The change-scenario features and the temporal data structure in RSPARROW versions >=2.0 have been designed to be fully compatible with future implementations of a more physically based representation of storage in the model.

* *Adjustment of the model coefficient standard errors of dynamic models for serial correlation.* Serial correlation can exist in the time series of the residuals of dynamic SPARROW models, which can cause somewhat less accurate estimates of the model coefficient standard errors and their statistical significance (e.g., Ator et al., 2019; Schmadel et al., 2021, 2024). Ancillary SAS SPARROW code is currently available to adjust the standard errors of the coefficients of dynamic models for serial correlation (G. Schwarz, USGS, written comm., 2022; Schmadel et al., 2024). These methods are being considered for inclusion in a future version of RSPARROW. 
...
Alexander, R. B., and Gorman Sanisaca, L. (2019). [RSPARROW: An R system for SPARROW modeling](https://code.usgs.gov/water/stats/RSPARROW) [Software release]. U.S. Geological Survey. DOI: https://doi.org/10.5066/P9UAZ6FO 
...
Ator, S.W., Garcia, A.M., Schwarz, G.E., Blomquist, J.D., and Sekellick, A.J. (2019), [Toward Explaining Nitrogen and Phosphorus Trends in Chesapeake Bay Tributaries, 1992–2012](https://onlinelibrary.wiley.com/doi/10.1111/1752-1688.12756), Journal of the American Water Resources Association, 55(5), 1149-1168.
...
Gorman Sanisaca, Lillian, and Alexander, Richard B.  (2024) [RSPARROW 2.0: An R system for SPARROW modeling](https://code.usgs.gov/water/stats/RSPARROW) [Software release]. U.S. Geological Survey. DOI:  https://doi.org/10.5066/P9G64CAN 


Harris, S., Elliott, S., McBride, G., Shankar, U., Quinn, J., Wheeler, D., Wedderburn, L., Hewitt, A., Gibb, R., Parfitt, R.,  Clothier, B., Green, S., Montes de Oca Munguía, O., Dake, C., and Rys, G. (2009), [Integrated assessment of the environmental, economic and social impacts of land use change using a GIS format – the CLUES model](http://ageconsearch.umn.edu/bitstream/97166/2/2009_21_Impacts%20of%20land%20use%20change%20CLUES%20model_Harris%20etal.pdf), New Zealand Agricultural and Resource Economics Society Conference Proceedings, Nelson, New Zealand, August 27-28, 2009. DOI: 10.22004/ag.econ.97166
...
Hirsch, R. M., Moyer, D.L., and Archfield, S.A. (2010), [Weighted Regressions on Time, Discharge, and Season (WRTDS), With an application to Chesapeake Bay river inputs](https://onlinelibrary.wiley.com/doi/full/10.1111/j.1752-1688.2010.00482.x), Journal of the American Water Resources Association (JAWRA) 46(5):857‐880. doi: 10.1111/j.1752‐1688.2010.00482.x
...
Hoos, A.B., and Roland, V.L. II (2019), [Spatially referenced models of streamflow and nitrogen, phosphorus, and suspended-sediment loads in the Southeastern United States](https://doi.org/10.3133/sir20195135), U.S. Geological Survey Scientific Investigations Report 2019–5135, 91 p.
...
Kirtman, B.P. and others (2014), North American Multimodel Ensemble: Phase-1 Seasonal-to-Interannual Prediction; Phase-2 toward Developing Intraseasonal Prediction, Bulletin American Meteorological Society, v. 95 (4), DOI: https://doi.org/10.1175/BAMS-D-12-00050.1
...
Lee, C.J., Hirsch, R.M., and Crawford, C.G. (2019), [An evaluation of methods for computing annual water-quality loads](https://doi.org/10.3133/sir20195084), U.S. Geological Survey Scientific Investigations Report 2019–5084, 59 p. doi: https://doi.org/10.3133/sir20195084.
...
Miller, M.P., Capel, P.D., Garcia, A.M., and Ator, S.W. (2019), [Response of nitrogen loading to the Chesapeake Bay to source reduction and land use change scenarios:  A SPARROW-informed analysis](https://doi.org/10.1111/1752-1688.12807), Journal of the American Water Resources Association, 56, 100-112. 
...
Miller, O.L., Putman, A.L., Smith, R.A., Schwarz, G.E., Hess, M.D., McDonnell, M.C., and Jones, D. (2024). [Temporal variability in irrigated land and climate influences on salinity loading across the Upper Colorado River Basin, 1986-2017] (https://doi.org/10.1088/1748-9326/ad18dd), Environmental Research Letters, 19. DOI: 10.1088/1748-9326/ad18dd
...
Moore, R.B., C.M. Johnston, K.W. Robinson, and Deacon, J.R. (2004), [Estimation of total nitrogen and phosphorus in New England streams using spatially referenced regression models](https://pubs.usgs.gov/sir/2004/5012/), U.S. Geological Survey Scientific Investigations Report 2004-5012. 
...
Schmadel, N.M., Harvey, J.W., Schwarz, G.E., Alexander, R.B., Gomez‐Velez, J.D.,Scott, D., and Ator, S.W. (2019), [Small ponds in headwater catchments are a dominant influence on regional nutrient and sediment budgets](https://doi.org/10.1029/2019GL083937), Geophysical Research Letters. DOI: 10.1029/2019GL083937

Schmadel, N.M., Harvey, J.W., Schwarz, G.E., (2021), [Seasonally dynamic nutrient modeling quantifies storage lags and time-varying reactivity across large river basins](https://iopscience.iop.org/article/10.1088/1748-9326/ac1af4), Environmental Research Letters, 16(9).  DOI: 10.1088/1748-9326/ac1af4

Schmadel, N.M., Miller, O.L., Ator, S., Miller, M.P., Schwarz, G.E., Robertson, D.M., Sekellick, A., Skinner, K., and Saad, D.A. (2024),
[Seasonally varying contributions of contemporaneous and lagged sources of instream total nitrogen and phosphorus load across the Illinois River basin](https://doi.org/10.1016/j.scitotenv.2024.176816),Science of the Total Environment, 955. DOI:10.1016/j.scitotenv.2024.176816 
...
Sohl, Terry and Sayler, Kristi, (2008), "Using the FORE-SCE model to project land-cover change in the southeastern United States". 572. https://digitalcommons.unl.edu/usgsstaffpub/572
...
Wellen, C., Arhonditsis, G. B., Labencki, T., & Boyd, D. (2014), [Application of the SPARROW model in watersheds with limited information: a Bayesian assessment of the model uncertainty and the value of additional monitoring. Hydrological Processes](https://doi.org/10.1002/hyp.9614), 28(3), 1260–1283. https://doi.org/10.1002/hyp.9614

White, C., M. Barik, J. Srikishen, M. Al-Hamdan, and W. Crosson (2022), [“RSPARROW Atmospheric Post Processing Tool”](https://po.ncche.olemiss.edu/sharing/5RNbVR5qB), Universities Space Research Association and University of Mississippi. 


# ACKNOWLEDGMENTS

We thank the following individuals and institutions for their contributions to the development of the 1.0 versions of RSPARROW. Matt Miller and Bradley Huffman of the U.S. Geological Survey (USGS) provided reviews and suggestions on the documentation and R code. Additional suggestions and testing of RSPARROW were provided by Ana Garcia, Laura Nagy, Victor Roland, Kristina Hopkins, David Saad, and Allison Appling of the USGS, Marcelo Luiz de Souza and Alexandre de Amorim of the National Water Agency of Brazil (ANA), Eber Josér Anda and Lucus Goncalves of the Geological Survey of Brazil (CPRM), and Jayanthi Srikishen and Mohammad Alhamdan of the Universities Space Research Association at NASA (National Aeronautics and Space Administration). Additional support was provided by Gregory Schwarz, Laura De Cicco, Anne Hoos, and Mike Ierardi of the USGS. Reviews and suggestions were also provided on the RShiny interface and decision-support tool by Steve Preston, David Wolock, and Michael Woodside of the USGS. We also thank Marcus Beck at the Tampa Bay Estuary Program, who contributed R code for the RShiny *leaflet* interactive maps in the 1.1.0 version. We also thank Marcus Beck for his review and comments on the 2.1 RSPARROW version. 

For their contributions to the development of the RSPARROW 2.0 version, we thank Marcus Beck (Tampa Bay Estuary Program) and Jiayu Fang (University of Mississippi), who served as beta testers of the R code. The 2.0 updates also benefited from helpful review comments and suggestions that were provided by Marcus Beck and Noah Schmadel (U.S. Geological Survey) on the R code and documentation, respectively. 

We extend our appreciation to the participants in SPARROW training workshops, held in Brasilia, Brazil (May 29-June 2, 2017; sponsored by ANA and CPRM) and Baltimore, Maryland USA (September 13, 2018; sponsored by the USGS), for their feedback on preliminary versions of the RSPARROW 1.0 version software. 
...
\textcolor{red}{Gorman Sanisaca, Lillian, Hurley, Kyle, and Alexander, Richard B.  (2025) [RSPARROW 2.1: An R system for SPARROW modeling](https://code.usgs.gov/water/stats/RSPARROW) [Software release]. U.S. Geological Survey. DOI:  https://doi.org/10.5066/P1Y6RUBU }
