...
addVars <- function(file.output.list) {
  ...
  # import design_matrix (uses row.names to preserve SOURCE row labels)
  ...
  if (nrow(test_data_names) != 0) { # missing from dataDictionary
    for (t in test_data_names$sparrowNames) {
      message(paste0(
        "ERROR: ", t, " PARAMETER NOT FOUND IN dataDictonary.csv\n ", t,
...
SET run_dataImport<-'yes' AND load_previousDataImport<-'no'\n \n"
      ...
      new_data_names <- data.frame(
        varType = test_data_names[which(test_data_names$sparrowNames == t), ]$parmType,
        sparrowNames = t,
        data1UserNames = NA,
        varunits = NA,
    ...
    # save dataDictionary
    fwrite(data_names,
      file = fileDic, row.names = F, append = F, showProgress = FALSE,
  ...
  if (nrow(test_design) != 0) { # missing from design_matrix
    ...
    for (t in test_design$sparrowNames) {
      if (test_design[which(test_design$sparrowNames == t), ]$parmType == "SOURCE") {
        message(paste0(
          "ERROR: ", t, " PARAMETER NOT FOUND IN design_matrix.csv as SOURCE\n ", t,
...
SET run_dataImport<-'yes' AND load_previousDataImport<-'no'\n \n"
        ...
        message(paste0(
          "ERROR: ", t, " PARAMETER NOT FOUND IN design_matrix.csv as DELIVF\n ", t,
          " HAS BEEN ADDED TO design_matrix.csv\n USER MUST EDIT design_matrix.csv, userModifyData.R\n
TO ALLOW FOR NEW PARAMETER\n \nDATA IMPORT MUST BE RE_RUN\n
SET run_dataImport<-'yes' AND load_previousDataImport<-'no'\n \n"
        ))
    ...
    fwrite(dmatrixin,
      file = fileDesign, row.names = F, append = F, showProgress = FALSE,
      dec = csv_decimalSeparator, sep = csv_columnSeparator, col.names = TRUE, na = "NA"
    )
  } # end if test_design
  ...
  if (nrow(test_design) != 0 | nrow(test_data_names) != 0) {
    ...
    stop("Error in addVars.R. Run execution terminated.")
  }
} # end function
