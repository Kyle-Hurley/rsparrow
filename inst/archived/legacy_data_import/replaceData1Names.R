...
#' @return `data1` data.frame containing input data (data1) with data1UserNames replaced
#' with sparrowNames
#' @keywords internal
#' @noRd
  ...
  for (i in 1:length(datalstreq)) {
    ...
    for (j in 1:length(data1_names)) {
      if (!is.na(datalstin[i])) {
        if (datalstin[i] == data1_names[j]) {
          ick <- ick + 1
        } # compare required name in DATA1 file with actual DATA1 names
      }
    }
    if (ick == 0) {
      data1[[datalstreq[i]]] <- NA
    }
  }
