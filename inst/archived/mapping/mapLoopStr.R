...
#' 
#' @description 
#' Creates list of map objects with number of panels equal to RSPARROW user 
#' setting mapsPerPage.
#' 
#' Executed By: \itemize{
#'              \item goShinyPlot.R, 
...
#' 
#' @return named list of plot objects for multi-panel maps
#' @keywords internal
#' @noRd
...
mapLoopStr <- function(mapType, mapLoopInput.list) {
  ...
  unPackList(
    lists = list(mapLoopInput.list = mapLoopInput.list),
    parentObj = list(NA)
  )
  ...
  unPackList(
    lists = list(
      mapping.input.list = mapping.input.list[
        !names(mapping.input.list) %in% names(mapLoopInput.list)
      ]
    ),
    parentObj = list(NA)
  )
  ...
  if (Rshiny & mapType == "site") {
    ...
    unPackList(
      lists = list(mapping.input.list = mapping.input.list),
      parentObj = list(NA)
    )
  ...
  for (i in unique(plots$plotKey)) {
    ...
    for (j in 1:nrow(plotSub)) {
      ...
      if (!is.na(y[1]) & !is.na(s[1])) {
        ...
        # Subtitle always year - season for resid maps
        if (mapType == "resid") {
          subTitle <- paste(y, s)
        }
        ...
        if (existGeoLines) {
          if (enable_plotlyMaps == "no" | enable_plotlyMaps == "static") {
            ...
            if (mapType != "resid") {
              p <- p %>% add_sf(
                data = GeoLines, mode = "lines", type = "scatter",
        ...
        # NOTE: predictMaps_single, mapSiteAttributes, diagnosticMaps removed in Plan 05A.
        # Map rendering is disabled; functions will be restored in Plan 05D.
        p.list <- list(p = NULL, usedColors = usedColors)
    ...
    eval(parse(text = paste0(
      "map_loop.list$p", letters[which(unique(plots$plotKey) == i)],
