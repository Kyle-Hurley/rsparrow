...
#' tabular{ll}{
#' `allSourcesDFno` \tab data.frame with the following columns ModelVariables, PercentChange,
#' ChangeCoefficient, SelectionVariable, Min, Max, Equals, Like, LanduseConversion,
#' and Separator for Rshiny scenarios where different changes are applied to different
#' parameters in different areas. \cr
#' `sourceRed` \tab data.frame with the following columns ModelVariables, PercentChange,
#' ChangeCoefficient, and LanduseConversion for Rshiny scenarios where all changes are
#' applied to the same parameters, but with the option to select different areas in which
#' the changes can be applied. \cr
#' `allSourcesDF` \tab data.frame with the following columns SelectionVariable, Min, Max,
#' Equals, Like,  and Separator for Rshiny scenarios where all changes are
#' applied to the same parameters, but with the option to select different areas in which
#' the changes can be applied. \cr
#' `cosmeticPred` \tab data.frame with the following columns type, setting, and
#' settingValue with cosmetic mapping settings to control map appearance (color,
#' line width, etc.) for prediction maps. \cr
#' `cosmeticSite` \tab data.frame with the following columns type, setting, and
#' settingValue with cosmetic mapping settings to control map appearance (color,
#' line width, etc.) for site attribute maps. \cr
#' `cosmeticScen` \tab data.frame with the following columns type, setting, and
#' settingValue with cosmetic mapping settings to control map appearance (color,
#' line width, etc.) for scenario maps. \cr
#' }
...
createRTables <- function(selectSources, data_names, mapping.input.list) {
  ...
  allSourcesDFno <- data.frame(
    ModelVariables = factor(as.character(""), levels = selectSources),
