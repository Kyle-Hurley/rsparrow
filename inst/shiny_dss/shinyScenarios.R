...
shinyScenarios <- function(id, input, choices, sitedata, add_plotlyVars, scenario.input.list) {
  unPackList(
    lists = list(scenario.input.list = scenario.input.list),
    parentObj = list(NA)
  )
  ...
  conditionalPanel(
    ...
    # scenario_name
    fluidRow(
      column(width = 9, textInput(ns("scenarioName"),
    ...
    conditionalPanel(
      condition = paste0("input['", ns("forecastScenario"), "']=='yes'"),
      ...
      fluidRow(
        column(width = 9, textInput(ns("forecast_filename"),
          label = "",
          forecast_filename
        )),
    ...
    conditionalPanel(
      condition = paste0("input['", ns("forecastScenario"), "']=='no'"),
      HTML("<br>"),
