        ...
        sidebarLayout(
          sidebarPanel(
            ...
            if (checkDynamic(subdata)) {
              conditionalPanel(
                "input.mapType!='Scenarios for Changes in Sources and/or Delivery Variables (e.g., Climate, Land use)'",
    ...
    server = shinyServer(function(input, output, session) {
      ...
      observe({
        ...
        if (input$mapType == "Catchment" & input$batch == "Interactive") {
          ...
          output$plotlyExplanation <- renderText({
            "Plotly not available for catchment maps in Interactive mode due to long processing time\n to get interactive catchment maps select Batch mode and enable plotly"
          })
      ...
      observe({
        if (input$batch == "Batch") {
          if (input$mapType %in% c("Stream", "Catchment")) {
            ...
            ratioChoices <- data.frame(
              category = c("Relative Change in Load", "Relative Change in Load"),
      ...
      observe({
        if (grepl("Scenarios", input$mapType)) {
          ...
          testRow <- testCosmetic(input, output, session,
            ...
            scenario.input.list, mapping.input.list
          )$rowNums
      ...
      p1 <- eventReactive(input$goPlot, {
        ...
        tryCatch({
          ...
          errMsg <- NA
          if (input$mapType %in% c("Stream", "Catchment")) {
            badSettings <- testCosmetic(input, output, session,
              DF = as.data.frame(scenarioRtables$cosmeticPred), mapType = input$mapType,
              scenario.input.list, mapping.input.list
            )$badSettings
          } else if (input$mapType == "Site Attributes") {
            ...
            badSettings <- testCosmetic(input, output, session,
              DF = as.data.frame(scenarioRtables$cosmeticScen), "Source Change Scenarios",
              scenario.input.list, mapping.input.list
            )$badSettings
          }


          currentP <- goShinyPlot(
            input, output, session, choices, "goPlot", badSettings, errMsg,
            file.output.list, map_uncertainties, BootUncertainties,
            data_names, mapping.input.list,
            # predict.list,
            subdata, SelParmValues,
        ...
        tryCatch(
          {
            ...
            goShinyPlot(
              input, output, session, choices, "savePDF", badSettings, errMsg,
              file.output.list, map_uncertainties, BootUncertainties,
              data_names, mapping.input.list,
              subdata, SelParmValues,
              # site attr
              sitedata, estimate.list, estimate.input.list,
              # scenarios
              JacobResults,
              ConcFactor, DataMatrix.list, dlvdsgn,
              reach_decay_specification, reservoir_decay_specification,
      ...
      p3 <- eventReactive(input$batchPlot, {
        ...
        goShinyPlot(
          input, output, session, choices, "batchPlot", badSettings, errMsg,
          file.output.list, map_uncertainties, BootUncertainties,
          data_names, mapping.input.list,
          subdata, SelParmValues,
          # site attr
